.class public final Landroidx/compose/foundation/pager/PagerDefaults;
.super Ljava/lang/Object;
.source "Pager.kt"


# annotations
.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nPager.kt\nKotlin\n*S Kotlin\n*F\n+ 1 Pager.kt\nandroidx/compose/foundation/pager/PagerDefaults\n+ 2 CompositionLocal.kt\nandroidx/compose/runtime/CompositionLocal\n+ 3 Composables.kt\nandroidx/compose/runtime/ComposablesKt\n+ 4 Composer.kt\nandroidx/compose/runtime/ComposerKt\n*L\n1#1,967:1\n74#2:968\n83#3,3:969\n1116#4,6:972\n*S KotlinDebug\n*F\n+ 1 Pager.kt\nandroidx/compose/foundation/pager/PagerDefaults\n*L\n354#1:968\n355#1:969,3\n355#1:972,6\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000N\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\u0008\u0002\n\u0002\u0010\u0008\n\u0000\n\u0002\u0018\u0002\n\u0002\u0010\u0007\n\u0002\u0008\u0003\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0005\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\u0008\u00c7\u0002\u0018\u00002\u00020\u0001B\u0007\u0008\u0002\u00a2\u0006\u0002\u0010\u0002Jh\u0010\n\u001a\u00020\u000b2\u0006\u0010\u000c\u001a\u00020\r2\u0008\u0008\u0002\u0010\u000e\u001a\u00020\u000f2\u000e\u0008\u0002\u0010\u0010\u001a\u0008\u0012\u0004\u0012\u00020\u00070\u00062\u000e\u0008\u0002\u0010\u0011\u001a\u0008\u0012\u0004\u0012\u00020\u00070\u00122\u000e\u0008\u0002\u0010\u0013\u001a\u0008\u0012\u0004\u0012\u00020\u00070\u00062\u0008\u0008\u0002\u0010\u0014\u001a\u00020\u00152\u0008\u0008\u0002\u0010\u0016\u001a\u00020\u0007H\u0007\u00f8\u0001\u0000\u00a2\u0006\u0004\u0008\u0017\u0010\u0018JY\u0010\n\u001a\u00020\u000b2\u0006\u0010\u000c\u001a\u00020\r2\u0008\u0008\u0002\u0010\u000e\u001a\u00020\u000f2\u000e\u0008\u0002\u0010\u0010\u001a\u0008\u0012\u0004\u0012\u00020\u00070\u00062\u000e\u0008\u0002\u0010\u0011\u001a\u0008\u0012\u0004\u0012\u00020\u00070\u00122\u000e\u0008\u0002\u0010\u0013\u001a\u0008\u0012\u0004\u0012\u00020\u00070\u00062\u0008\u0008\u0003\u0010\u0016\u001a\u00020\u0007H\u0007\u00a2\u0006\u0002\u0010\u0019J\u0016\u0010\u001a\u001a\u00020\u001b2\u0006\u0010\u000c\u001a\u00020\r2\u0006\u0010\u001c\u001a\u00020\u001dR\u000e\u0010\u0003\u001a\u00020\u0004X\u0086T\u00a2\u0006\u0002\n\u0000R\u0017\u0010\u0005\u001a\u0008\u0012\u0004\u0012\u00020\u00070\u0006\u00a2\u0006\u0008\n\u0000\u001a\u0004\u0008\u0008\u0010\t\u0082\u0002\u0007\n\u0005\u0008\u00a1\u001e0\u0001\u00a8\u0006\u001e"
    }
    d2 = {
        "Landroidx/compose/foundation/pager/PagerDefaults;",
        "",
        "()V",
        "BeyondBoundsPageCount",
        "",
        "LowVelocityAnimationSpec",
        "Landroidx/compose/animation/core/AnimationSpec;",
        "",
        "getLowVelocityAnimationSpec",
        "()Landroidx/compose/animation/core/AnimationSpec;",
        "flingBehavior",
        "Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;",
        "state",
        "Landroidx/compose/foundation/pager/PagerState;",
        "pagerSnapDistance",
        "Landroidx/compose/foundation/pager/PagerSnapDistance;",
        "lowVelocityAnimationSpec",
        "highVelocityAnimationSpec",
        "Landroidx/compose/animation/core/DecayAnimationSpec;",
        "snapAnimationSpec",
        "snapVelocityThreshold",
        "Landroidx/compose/ui/unit/Dp;",
        "snapPositionalThreshold",
        "flingBehavior-PfoAEA0",
        "(Landroidx/compose/foundation/pager/PagerState;Landroidx/compose/foundation/pager/PagerSnapDistance;Landroidx/compose/animation/core/AnimationSpec;Landroidx/compose/animation/core/DecayAnimationSpec;Landroidx/compose/animation/core/AnimationSpec;FFLandroidx/compose/runtime/Composer;II)Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;",
        "(Landroidx/compose/foundation/pager/PagerState;Landroidx/compose/foundation/pager/PagerSnapDistance;Landroidx/compose/animation/core/AnimationSpec;Landroidx/compose/animation/core/DecayAnimationSpec;Landroidx/compose/animation/core/AnimationSpec;FLandroidx/compose/runtime/Composer;II)Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;",
        "pageNestedScrollConnection",
        "Landroidx/compose/ui/input/nestedscroll/NestedScrollConnection;",
        "orientation",
        "Landroidx/compose/foundation/gestures/Orientation;",
        "foundation_release"
    }
    k = 0x1
    mv = {
        0x1,
        0x8,
        0x0
    }
    xi = 0x30
.end annotation


# static fields
.field public static final $stable:I

.field public static final BeyondBoundsPageCount:I

.field public static final INSTANCE:Landroidx/compose/foundation/pager/PagerDefaults;

.field private static final LowVelocityAnimationSpec:Landroidx/compose/animation/core/AnimationSpec;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Landroidx/compose/animation/core/AnimationSpec<",
            "Ljava/lang/Float;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method static constructor <clinit>()V
    .locals 5

    new-instance v0, Landroidx/compose/foundation/pager/PagerDefaults;

    invoke-direct {v0}, Landroidx/compose/foundation/pager/PagerDefaults;-><init>()V

    sput-object v0, Landroidx/compose/foundation/pager/PagerDefaults;->INSTANCE:Landroidx/compose/foundation/pager/PagerDefaults;

    .line 384
    invoke-static {}, Landroidx/compose/animation/core/EasingKt;->getLinearEasing()Landroidx/compose/animation/core/Easing;

    move-result-object v0

    const/4 v1, 0x2

    const/4 v2, 0x0

    const/16 v3, 0x1f4

    const/4 v4, 0x0

    invoke-static {v3, v4, v0, v1, v2}, Landroidx/compose/animation/core/AnimationSpecKt;->tween$default(IILandroidx/compose/animation/core/Easing;ILjava/lang/Object;)Landroidx/compose/animation/core/TweenSpec;

    move-result-object v0

    check-cast v0, Landroidx/compose/animation/core/AnimationSpec;

    sput-object v0, Landroidx/compose/foundation/pager/PagerDefaults;->LowVelocityAnimationSpec:Landroidx/compose/animation/core/AnimationSpec;

    const/16 v0, 0x8

    sput v0, Landroidx/compose/foundation/pager/PagerDefaults;->$stable:I

    return-void
.end method

.method private constructor <init>()V
    .locals 0

    .line 279
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method


# virtual methods
.method public final flingBehavior(Landroidx/compose/foundation/pager/PagerState;Landroidx/compose/foundation/pager/PagerSnapDistance;Landroidx/compose/animation/core/AnimationSpec;Landroidx/compose/animation/core/DecayAnimationSpec;Landroidx/compose/animation/core/AnimationSpec;FLandroidx/compose/runtime/Composer;II)Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;
    .locals 17
    .param p1, "state"    # Landroidx/compose/foundation/pager/PagerState;
    .param p2, "pagerSnapDistance"    # Landroidx/compose/foundation/pager/PagerSnapDistance;
    .param p3, "lowVelocityAnimationSpec"    # Landroidx/compose/animation/core/AnimationSpec;
    .param p4, "highVelocityAnimationSpec"    # Landroidx/compose/animation/core/DecayAnimationSpec;
    .param p5, "snapAnimationSpec"    # Landroidx/compose/animation/core/AnimationSpec;
    .param p6, "snapPositionalThreshold"    # F
    .param p7, "$composer"    # Landroidx/compose/runtime/Composer;
    .param p8, "$changed"    # I
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroidx/compose/foundation/pager/PagerState;",
            "Landroidx/compose/foundation/pager/PagerSnapDistance;",
            "Landroidx/compose/animation/core/AnimationSpec<",
            "Ljava/lang/Float;",
            ">;",
            "Landroidx/compose/animation/core/DecayAnimationSpec<",
            "Ljava/lang/Float;",
            ">;",
            "Landroidx/compose/animation/core/AnimationSpec<",
            "Ljava/lang/Float;",
            ">;F",
            "Landroidx/compose/runtime/Composer;",
            "II)",
            "Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;"
        }
    .end annotation

    move-object/from16 v0, p7

    const v1, -0xb9132f0

    invoke-interface {v0, v1}, Landroidx/compose/runtime/Composer;->startReplaceableGroup(I)V

    const-string v2, "C(flingBehavior)P(5,2,1)345@18344L26,353@18811L7,354@18834L770:Pager.kt#g6yjnt"

    invoke-static {v0, v2}, Landroidx/compose/runtime/ComposerKt;->sourceInformation(Landroidx/compose/runtime/Composer;Ljava/lang/String;)V

    and-int/lit8 v2, p9, 0x2

    const/4 v3, 0x1

    if-eqz v2, :cond_0

    .line 344
    sget-object v2, Landroidx/compose/foundation/pager/PagerSnapDistance;->Companion:Landroidx/compose/foundation/pager/PagerSnapDistance$Companion;

    invoke-virtual {v2, v3}, Landroidx/compose/foundation/pager/PagerSnapDistance$Companion;->atMost(I)Landroidx/compose/foundation/pager/PagerSnapDistance;

    move-result-object v2

    .end local p2    # "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    .local v2, "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    goto :goto_0

    .line 0
    .end local v2    # "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    .restart local p2    # "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    :cond_0
    move-object/from16 v2, p2

    .line 344
    .end local p2    # "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    .restart local v2    # "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    :goto_0
    and-int/lit8 v4, p9, 0x4

    if-eqz v4, :cond_1

    .line 345
    sget-object v4, Landroidx/compose/foundation/pager/PagerDefaults;->LowVelocityAnimationSpec:Landroidx/compose/animation/core/AnimationSpec;

    move-object v10, v4

    .end local p3    # "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .local v4, "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    goto :goto_1

    .line 344
    .end local v4    # "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .restart local p3    # "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    :cond_1
    move-object/from16 v10, p3

    .line 345
    .end local p3    # "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .local v10, "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    :goto_1
    and-int/lit8 v4, p9, 0x8

    const/4 v11, 0x0

    if-eqz v4, :cond_2

    .line 346
    invoke-static {v0, v11}, Landroidx/compose/animation/SplineBasedFloatDecayAnimationSpec_androidKt;->rememberSplineBasedDecay(Landroidx/compose/runtime/Composer;I)Landroidx/compose/animation/core/DecayAnimationSpec;

    move-result-object v4

    move-object v12, v4

    .end local p4    # "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    .local v4, "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    goto :goto_2

    .line 345
    .end local v4    # "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    .restart local p4    # "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    :cond_2
    move-object/from16 v12, p4

    .line 346
    .end local p4    # "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    .local v12, "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    :goto_2
    and-int/lit8 v4, p9, 0x10

    const/4 v5, 0x0

    if-eqz v4, :cond_3

    .line 347
    const/high16 v4, 0x43c80000    # 400.0f

    const/4 v6, 0x5

    const/4 v7, 0x0

    invoke-static {v5, v4, v7, v6, v7}, Landroidx/compose/animation/core/AnimationSpecKt;->spring$default(FFLjava/lang/Object;ILjava/lang/Object;)Landroidx/compose/animation/core/SpringSpec;

    move-result-object v4

    check-cast v4, Landroidx/compose/animation/core/AnimationSpec;

    move-object v13, v4

    .end local p5    # "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .local v4, "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    goto :goto_3

    .line 346
    .end local v4    # "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .restart local p5    # "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    :cond_3
    move-object/from16 v13, p5

    .line 347
    .end local p5    # "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .local v13, "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    :goto_3
    and-int/lit8 v4, p9, 0x20

    if-eqz v4, :cond_4

    .line 348
    const/high16 v4, 0x3f000000    # 0.5f

    move v14, v4

    .end local p6    # "snapPositionalThreshold":F
    .local v4, "snapPositionalThreshold":F
    goto :goto_4

    .line 347
    .end local v4    # "snapPositionalThreshold":F
    .restart local p6    # "snapPositionalThreshold":F
    :cond_4
    move/from16 v14, p6

    .line 348
    .end local p6    # "snapPositionalThreshold":F
    .local v14, "snapPositionalThreshold":F
    :goto_4
    invoke-static {}, Landroidx/compose/runtime/ComposerKt;->isTraceInProgress()Z

    move-result v4

    if-eqz v4, :cond_5

    .line 349
    const/4 v4, -0x1

    const-string/jumbo v6, "androidx.compose.foundation.pager.PagerDefaults.flingBehavior (Pager.kt:348)"

    move/from16 v15, p8

    invoke-static {v1, v15, v4, v6}, Landroidx/compose/runtime/ComposerKt;->traceEventStart(IIILjava/lang/String;)V

    goto :goto_5

    .line 348
    :cond_5
    move/from16 v15, p8

    .line 350
    :goto_5
    cmpg-float v1, v5, v14

    if-gtz v1, :cond_6

    const/high16 v1, 0x3f800000    # 1.0f

    cmpg-float v1, v14, v1

    if-gtz v1, :cond_6

    goto :goto_6

    :cond_6
    move v3, v11

    :goto_6
    if-eqz v3, :cond_b

    .line 354
    invoke-static {}, Landroidx/compose/ui/platform/CompositionLocalsKt;->getLocalDensity()Landroidx/compose/runtime/ProvidableCompositionLocal;

    move-result-object v1

    check-cast v1, Landroidx/compose/runtime/CompositionLocal;

    .local v1, "this_$iv":Landroidx/compose/runtime/CompositionLocal;
    const/4 v3, 0x0

    .local v3, "$changed$iv":I
    const/4 v4, 0x0

    .line 968
    .local v4, "$i$f$getCurrent":I
    const v5, 0x789c5f52

    const-string v6, "CC:CompositionLocal.kt#9igjgp"

    invoke-static {v0, v5, v6}, Landroidx/compose/runtime/ComposerKt;->sourceInformationMarkerStart(Landroidx/compose/runtime/Composer;ILjava/lang/String;)V

    invoke-interface {v0, v1}, Landroidx/compose/runtime/Composer;->consume(Landroidx/compose/runtime/CompositionLocal;)Ljava/lang/Object;

    move-result-object v5

    invoke-static/range {p7 .. p7}, Landroidx/compose/runtime/ComposerKt;->sourceInformationMarkerEnd(Landroidx/compose/runtime/Composer;)V

    .line 354
    .end local v1    # "this_$iv":Landroidx/compose/runtime/CompositionLocal;
    .end local v3    # "$changed$iv":I
    .end local v4    # "$i$f$getCurrent":I
    move-object v1, v5

    check-cast v1, Landroidx/compose/ui/unit/Density;

    .line 356
    .local v1, "density":Landroidx/compose/ui/unit/Density;
    nop

    .line 357
    nop

    .line 356
    nop

    .line 358
    nop

    .line 356
    nop

    .line 359
    nop

    .line 356
    nop

    .line 360
    nop

    .line 356
    nop

    .line 361
    move-object/from16 v4, p1

    move-object v5, v10

    move-object v6, v12

    move-object v7, v13

    move-object v8, v2

    move-object v9, v1

    filled-new-array/range {v4 .. v9}, [Ljava/lang/Object;

    move-result-object v3

    .line 356
    nop

    .line 355
    nop

    .local v3, "keys$iv":[Ljava/lang/Object;
    const/16 v4, 0x8

    .local v4, "$changed$iv":I
    const/4 v5, 0x0

    .local v5, "$i$f$remember":I
    const v6, -0x21de6e89

    invoke-interface {v0, v6}, Landroidx/compose/runtime/Composer;->startReplaceableGroup(I)V

    const-string v6, "CC(remember)P(1):Composables.kt#9igjgp"

    invoke-static {v0, v6}, Landroidx/compose/runtime/ComposerKt;->sourceInformation(Landroidx/compose/runtime/Composer;Ljava/lang/String;)V

    .line 969
    const/4 v6, 0x0

    .line 970
    .local v6, "invalid$iv":Z
    array-length v7, v3

    :goto_7
    if-ge v11, v7, :cond_7

    aget-object v8, v3, v11

    .local v8, "key$iv":Ljava/lang/Object;
    invoke-interface {v0, v8}, Landroidx/compose/runtime/Composer;->changed(Ljava/lang/Object;)Z

    move-result v9

    or-int/2addr v6, v9

    .end local v8    # "key$iv":Ljava/lang/Object;
    add-int/lit8 v11, v11, 0x1

    goto :goto_7

    .line 971
    :cond_7
    move-object/from16 v7, p7

    .local v7, "$this$cache$iv$iv":Landroidx/compose/runtime/Composer;
    const/4 v8, 0x0

    .line 972
    .local v8, "$i$f$cache":I
    invoke-interface {v7}, Landroidx/compose/runtime/Composer;->rememberedValue()Ljava/lang/Object;

    move-result-object v9

    .local v9, "it$iv$iv":Ljava/lang/Object;
    const/4 v11, 0x0

    .line 973
    .local v11, "$i$a$-let-ComposerKt$cache$1$iv$iv":I
    if-nez v6, :cond_9

    sget-object v16, Landroidx/compose/runtime/Composer;->Companion:Landroidx/compose/runtime/Composer$Companion;

    invoke-virtual/range {v16 .. v16}, Landroidx/compose/runtime/Composer$Companion;->getEmpty()Ljava/lang/Object;

    move-result-object v0

    if-ne v9, v0, :cond_8

    goto :goto_8

    .line 977
    :cond_8
    move-object/from16 p4, v1

    move-object v0, v9

    goto :goto_9

    .line 974
    :cond_9
    :goto_8
    const/4 v0, 0x0

    .line 365
    .local v0, "$i$a$-remember-PagerDefaults$flingBehavior$2":I
    nop

    .line 366
    nop

    .line 367
    nop

    .line 368
    nop

    .line 364
    move/from16 p2, v0

    move-object/from16 v0, p1

    .end local v0    # "$i$a$-remember-PagerDefaults$flingBehavior$2":I
    .local p2, "$i$a$-remember-PagerDefaults$flingBehavior$2":I
    invoke-static {v0, v2, v12, v14}, Landroidx/compose/foundation/pager/PagerKt;->access$SnapLayoutInfoProvider(Landroidx/compose/foundation/pager/PagerState;Landroidx/compose/foundation/pager/PagerSnapDistance;Landroidx/compose/animation/core/DecayAnimationSpec;F)Landroidx/compose/foundation/gestures/snapping/SnapLayoutInfoProvider;

    move-result-object v16

    .line 363
    move-object/from16 p3, v16

    .line 371
    .local p3, "snapLayoutInfoProvider":Landroidx/compose/foundation/gestures/snapping/SnapLayoutInfoProvider;
    new-instance v0, Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;

    .line 372
    nop

    .line 373
    nop

    .line 374
    nop

    .line 375
    nop

    .line 371
    move-object/from16 p4, v1

    move-object/from16 v1, p3

    .end local p3    # "snapLayoutInfoProvider":Landroidx/compose/foundation/gestures/snapping/SnapLayoutInfoProvider;
    .local v1, "snapLayoutInfoProvider":Landroidx/compose/foundation/gestures/snapping/SnapLayoutInfoProvider;
    .local p4, "density":Landroidx/compose/ui/unit/Density;
    invoke-direct {v0, v1, v10, v12, v13}, Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;-><init>(Landroidx/compose/foundation/gestures/snapping/SnapLayoutInfoProvider;Landroidx/compose/animation/core/AnimationSpec;Landroidx/compose/animation/core/DecayAnimationSpec;Landroidx/compose/animation/core/AnimationSpec;)V

    .line 974
    .end local v1    # "snapLayoutInfoProvider":Landroidx/compose/foundation/gestures/snapping/SnapLayoutInfoProvider;
    .end local p2    # "$i$a$-remember-PagerDefaults$flingBehavior$2":I
    nop

    .line 975
    .local v0, "value$iv$iv":Ljava/lang/Object;
    invoke-interface {v7, v0}, Landroidx/compose/runtime/Composer;->updateRememberedValue(Ljava/lang/Object;)V

    .line 976
    nop

    .line 973
    .end local v0    # "value$iv$iv":Ljava/lang/Object;
    :goto_9
    nop

    .line 972
    .end local v9    # "it$iv$iv":Ljava/lang/Object;
    .end local v11    # "$i$a$-let-ComposerKt$cache$1$iv$iv":I
    nop

    .line 971
    .end local v7    # "$this$cache$iv$iv":Landroidx/compose/runtime/Composer;
    .end local v8    # "$i$f$cache":I
    invoke-interface/range {p7 .. p7}, Landroidx/compose/runtime/Composer;->endReplaceableGroup()V

    .line 355
    .end local v3    # "keys$iv":[Ljava/lang/Object;
    .end local v4    # "$changed$iv":I
    .end local v5    # "$i$f$remember":I
    .end local v6    # "invalid$iv":Z
    check-cast v0, Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;

    invoke-static {}, Landroidx/compose/runtime/ComposerKt;->isTraceInProgress()Z

    move-result v1

    if-eqz v1, :cond_a

    invoke-static {}, Landroidx/compose/runtime/ComposerKt;->traceEventEnd()V

    :cond_a
    invoke-interface/range {p7 .. p7}, Landroidx/compose/runtime/Composer;->endReplaceableGroup()V

    return-object v0

    .line 350
    .end local p4    # "density":Landroidx/compose/ui/unit/Density;
    :cond_b
    const/4 v0, 0x0

    .line 351
    .local v0, "$i$a$-require-PagerDefaults$flingBehavior$1":I
    new-instance v1, Ljava/lang/StringBuilder;

    invoke-direct {v1}, Ljava/lang/StringBuilder;-><init>()V

    const-string/jumbo v3, "snapPositionalThreshold should be a number between 0 and 1. You\'ve specified "

    invoke-virtual {v1, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v1

    .line 352
    nop

    .line 351
    invoke-virtual {v1, v14}, Ljava/lang/StringBuilder;->append(F)Ljava/lang/StringBuilder;

    move-result-object v1

    invoke-virtual {v1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    .line 350
    .end local v0    # "$i$a$-require-PagerDefaults$flingBehavior$1":I
    new-instance v1, Ljava/lang/IllegalArgumentException;

    invoke-virtual {v0}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v0

    invoke-direct {v1, v0}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v1
.end method

.method public final flingBehavior-PfoAEA0(Landroidx/compose/foundation/pager/PagerState;Landroidx/compose/foundation/pager/PagerSnapDistance;Landroidx/compose/animation/core/AnimationSpec;Landroidx/compose/animation/core/DecayAnimationSpec;Landroidx/compose/animation/core/AnimationSpec;FFLandroidx/compose/runtime/Composer;II)Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;
    .locals 18
    .param p1, "state"    # Landroidx/compose/foundation/pager/PagerState;
    .param p2, "pagerSnapDistance"    # Landroidx/compose/foundation/pager/PagerSnapDistance;
    .param p3, "lowVelocityAnimationSpec"    # Landroidx/compose/animation/core/AnimationSpec;
    .param p4, "highVelocityAnimationSpec"    # Landroidx/compose/animation/core/DecayAnimationSpec;
    .param p5, "snapAnimationSpec"    # Landroidx/compose/animation/core/AnimationSpec;
    .param p6, "snapVelocityThreshold"    # F
    .param p7, "snapPositionalThreshold"    # F
    .param p8, "$composer"    # Landroidx/compose/runtime/Composer;
    .param p9, "$changed"    # I
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroidx/compose/foundation/pager/PagerState;",
            "Landroidx/compose/foundation/pager/PagerSnapDistance;",
            "Landroidx/compose/animation/core/AnimationSpec<",
            "Ljava/lang/Float;",
            ">;",
            "Landroidx/compose/animation/core/DecayAnimationSpec<",
            "Ljava/lang/Float;",
            ">;",
            "Landroidx/compose/animation/core/AnimationSpec<",
            "Ljava/lang/Float;",
            ">;FF",
            "Landroidx/compose/runtime/Composer;",
            "II)",
            "Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;"
        }
    .end annotation

    .annotation runtime Lkotlin/Deprecated;
        message = "Please use the overload without snapVelocityThreshold. For Pagerthe functionalities provided by snapVelocityThreshold were already being provided byother APIS."
    .end annotation

    move-object/from16 v10, p8

    move/from16 v11, p9

    const v0, -0x2a0b3802

    invoke-interface {v10, v0}, Landroidx/compose/runtime/Composer;->startReplaceableGroup(I)V

    const-string v1, "C(flingBehavior)P(6,2,1!2,5:c#ui.unit.Dp)466@25585L26,470@25820L190:Pager.kt#g6yjnt"

    invoke-static {v10, v1}, Landroidx/compose/runtime/ComposerKt;->sourceInformation(Landroidx/compose/runtime/Composer;Ljava/lang/String;)V

    and-int/lit8 v1, p10, 0x2

    if-eqz v1, :cond_0

    .line 462
    sget-object v1, Landroidx/compose/foundation/pager/PagerSnapDistance;->Companion:Landroidx/compose/foundation/pager/PagerSnapDistance$Companion;

    const/4 v2, 0x1

    invoke-virtual {v1, v2}, Landroidx/compose/foundation/pager/PagerSnapDistance$Companion;->atMost(I)Landroidx/compose/foundation/pager/PagerSnapDistance;

    move-result-object v1

    move-object v12, v1

    .end local p2    # "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    .local v1, "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    goto :goto_0

    .line 0
    .end local v1    # "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    .restart local p2    # "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    :cond_0
    move-object/from16 v12, p2

    .line 462
    .end local p2    # "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    .local v12, "pagerSnapDistance":Landroidx/compose/foundation/pager/PagerSnapDistance;
    :goto_0
    and-int/lit8 v1, p10, 0x4

    const/4 v2, 0x0

    const/4 v3, 0x0

    if-eqz v1, :cond_1

    .line 463
    nop

    .line 464
    invoke-static {}, Landroidx/compose/animation/core/EasingKt;->getLinearEasing()Landroidx/compose/animation/core/Easing;

    move-result-object v1

    .line 465
    nop

    .line 463
    nop

    .line 464
    nop

    .line 463
    const/16 v4, 0x1f4

    const/4 v5, 0x2

    invoke-static {v4, v2, v1, v5, v3}, Landroidx/compose/animation/core/AnimationSpecKt;->tween$default(IILandroidx/compose/animation/core/Easing;ILjava/lang/Object;)Landroidx/compose/animation/core/TweenSpec;

    move-result-object v1

    check-cast v1, Landroidx/compose/animation/core/AnimationSpec;

    move-object v13, v1

    .end local p3    # "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .local v1, "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    goto :goto_1

    .line 462
    .end local v1    # "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .restart local p3    # "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    :cond_1
    move-object/from16 v13, p3

    .line 463
    .end local p3    # "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .local v13, "lowVelocityAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    :goto_1
    and-int/lit8 v1, p10, 0x8

    if-eqz v1, :cond_2

    .line 467
    invoke-static {v10, v2}, Landroidx/compose/animation/SplineBasedFloatDecayAnimationSpec_androidKt;->rememberSplineBasedDecay(Landroidx/compose/runtime/Composer;I)Landroidx/compose/animation/core/DecayAnimationSpec;

    move-result-object v1

    move-object v14, v1

    .end local p4    # "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    .local v1, "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    goto :goto_2

    .line 463
    .end local v1    # "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    .restart local p4    # "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    :cond_2
    move-object/from16 v14, p4

    .line 467
    .end local p4    # "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    .local v14, "highVelocityAnimationSpec":Landroidx/compose/animation/core/DecayAnimationSpec;
    :goto_2
    and-int/lit8 v1, p10, 0x10

    if-eqz v1, :cond_3

    .line 468
    const/high16 v1, 0x43c80000    # 400.0f

    const/4 v2, 0x5

    const/4 v4, 0x0

    invoke-static {v4, v1, v3, v2, v3}, Landroidx/compose/animation/core/AnimationSpecKt;->spring$default(FFLjava/lang/Object;ILjava/lang/Object;)Landroidx/compose/animation/core/SpringSpec;

    move-result-object v1

    check-cast v1, Landroidx/compose/animation/core/AnimationSpec;

    move-object v15, v1

    .end local p5    # "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .local v1, "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    goto :goto_3

    .line 467
    .end local v1    # "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .restart local p5    # "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    :cond_3
    move-object/from16 v15, p5

    .line 468
    .end local p5    # "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    .local v15, "snapAnimationSpec":Landroidx/compose/animation/core/AnimationSpec;
    :goto_3
    and-int/lit8 v1, p10, 0x20

    if-eqz v1, :cond_4

    .line 469
    invoke-static {}, Landroidx/compose/foundation/gestures/snapping/SnapFlingBehaviorKt;->getMinFlingVelocityDp()F

    move-result v1

    move/from16 v16, v1

    .end local p6    # "snapVelocityThreshold":F
    .local v1, "snapVelocityThreshold":F
    goto :goto_4

    .line 468
    .end local v1    # "snapVelocityThreshold":F
    .restart local p6    # "snapVelocityThreshold":F
    :cond_4
    move/from16 v16, p6

    .line 469
    .end local p6    # "snapVelocityThreshold":F
    .local v16, "snapVelocityThreshold":F
    :goto_4
    and-int/lit8 v1, p10, 0x40

    if-eqz v1, :cond_5

    .line 470
    const/high16 v1, 0x3f000000    # 0.5f

    move/from16 v17, v1

    .end local p7    # "snapPositionalThreshold":F
    .local v1, "snapPositionalThreshold":F
    goto :goto_5

    .line 469
    .end local v1    # "snapPositionalThreshold":F
    .restart local p7    # "snapPositionalThreshold":F
    :cond_5
    move/from16 v17, p7

    .line 470
    .end local p7    # "snapPositionalThreshold":F
    .local v17, "snapPositionalThreshold":F
    :goto_5
    invoke-static {}, Landroidx/compose/runtime/ComposerKt;->isTraceInProgress()Z

    move-result v1

    if-eqz v1, :cond_6

    .line 471
    const/4 v1, -0x1

    const-string/jumbo v2, "androidx.compose.foundation.pager.PagerDefaults.flingBehavior (Pager.kt:470)"

    invoke-static {v0, v11, v1, v2}, Landroidx/compose/runtime/ComposerKt;->traceEventStart(IIILjava/lang/String;)V

    .line 472
    :cond_6
    nop

    .line 473
    nop

    .line 474
    nop

    .line 475
    nop

    .line 476
    nop

    .line 477
    and-int/lit8 v0, v11, 0xe

    const v1, 0x209200

    or-int/2addr v0, v1

    and-int/lit8 v1, v11, 0x70

    or-int/2addr v0, v1

    shr-int/lit8 v1, v11, 0x3

    const/high16 v2, 0x70000

    and-int/2addr v1, v2

    or-int v8, v0, v1

    const/4 v9, 0x0

    .line 471
    move-object/from16 v0, p0

    move-object/from16 v1, p1

    move-object v2, v12

    move-object v3, v13

    move-object v4, v14

    move-object v5, v15

    move/from16 v6, v17

    move-object/from16 v7, p8

    invoke-virtual/range {v0 .. v9}, Landroidx/compose/foundation/pager/PagerDefaults;->flingBehavior(Landroidx/compose/foundation/pager/PagerState;Landroidx/compose/foundation/pager/PagerSnapDistance;Landroidx/compose/animation/core/AnimationSpec;Landroidx/compose/animation/core/DecayAnimationSpec;Landroidx/compose/animation/core/AnimationSpec;FLandroidx/compose/runtime/Composer;II)Landroidx/compose/foundation/gestures/snapping/SnapFlingBehavior;

    move-result-object v0

    invoke-static {}, Landroidx/compose/runtime/ComposerKt;->isTraceInProgress()Z

    move-result v1

    if-eqz v1, :cond_7

    invoke-static {}, Landroidx/compose/runtime/ComposerKt;->traceEventEnd()V

    :cond_7
    invoke-interface/range {p8 .. p8}, Landroidx/compose/runtime/Composer;->endReplaceableGroup()V

    return-object v0
.end method

.method public final getLowVelocityAnimationSpec()Landroidx/compose/animation/core/AnimationSpec;
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()",
            "Landroidx/compose/animation/core/AnimationSpec<",
            "Ljava/lang/Float;",
            ">;"
        }
    .end annotation

    .line 383
    sget-object v0, Landroidx/compose/foundation/pager/PagerDefaults;->LowVelocityAnimationSpec:Landroidx/compose/animation/core/AnimationSpec;

    return-object v0
.end method

.method public final pageNestedScrollConnection(Landroidx/compose/foundation/pager/PagerState;Landroidx/compose/foundation/gestures/Orientation;)Landroidx/compose/ui/input/nestedscroll/NestedScrollConnection;
    .locals 1
    .param p1, "state"    # Landroidx/compose/foundation/pager/PagerState;
    .param p2, "orientation"    # Landroidx/compose/foundation/gestures/Orientation;

    .line 491
    new-instance v0, Landroidx/compose/foundation/pager/DefaultPagerNestedScrollConnection;

    invoke-direct {v0, p1, p2}, Landroidx/compose/foundation/pager/DefaultPagerNestedScrollConnection;-><init>(Landroidx/compose/foundation/pager/PagerState;Landroidx/compose/foundation/gestures/Orientation;)V

    check-cast v0, Landroidx/compose/ui/input/nestedscroll/NestedScrollConnection;

    return-object v0
.end method
