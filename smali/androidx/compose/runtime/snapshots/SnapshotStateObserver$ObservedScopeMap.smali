.class final Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;
.super Ljava/lang/Object;
.source "SnapshotStateObserver.kt"


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Landroidx/compose/runtime/snapshots/SnapshotStateObserver;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x1a
    name = "ObservedScopeMap"
.end annotation

.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nSnapshotStateObserver.kt\nKotlin\n*S Kotlin\n*F\n+ 1 SnapshotStateObserver.kt\nandroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap\n+ 2 MutableVector.kt\nandroidx/compose/runtime/collection/MutableVectorKt\n+ 3 ObjectIntMap.kt\nandroidx/collection/ObjectIntMap\n+ 4 ScatterMap.kt\nandroidx/collection/ScatterMapKt\n+ 5 StateObjectImpl.kt\nandroidx/compose/runtime/snapshots/ReaderKind$Companion\n+ 6 DerivedState.kt\nandroidx/compose/runtime/SnapshotStateKt__DerivedStateKt\n+ 7 MutableVector.kt\nandroidx/compose/runtime/collection/MutableVector\n+ 8 ObjectIntMap.kt\nandroidx/collection/MutableObjectIntMap\n+ 9 ScatterMap.kt\nandroidx/collection/MutableScatterMap\n+ 10 ScatterMap.kt\nandroidx/collection/ScatterMap\n+ 11 IdentityArraySet.kt\nandroidx/compose/runtime/collection/IdentityArraySetKt\n+ 12 IdentityArraySet.kt\nandroidx/compose/runtime/collection/IdentityArraySet\n+ 13 ScopeMap.kt\nandroidx/compose/runtime/collection/ScopeMap\n+ 14 ScatterSet.kt\nandroidx/collection/ScatterSet\n+ 15 _Collections.kt\nkotlin/collections/CollectionsKt___CollectionsKt\n*L\n1#1,652:1\n1208#2:653\n1187#2,2:654\n415#3,3:656\n373#3,6:659\n383#3,3:666\n386#3,2:670\n419#3:672\n420#3:674\n389#3,6:675\n421#3:681\n373#3,6:693\n383#3,3:700\n386#3,2:704\n389#3,6:710\n401#3,4:717\n373#3,6:721\n383#3,3:728\n386#3,2:732\n406#3,2:734\n389#3,6:736\n408#3:742\n401#3,4:758\n373#3,6:762\n383#3,3:769\n386#3,2:773\n406#3,2:775\n389#3,6:777\n408#3:783\n1828#4:665\n1690#4:669\n1828#4:699\n1690#4:703\n1828#4:727\n1690#4:731\n1828#4:750\n1690#4:754\n1828#4:768\n1690#4:772\n1828#4:818\n1690#4:822\n1828#4:842\n1690#4:846\n1828#4:887\n1690#4:891\n1828#4:924\n1690#4:928\n1690#4:961\n1828#4:981\n1690#4:985\n1828#4:1026\n1690#4:1030\n1828#4:1063\n1690#4:1067\n1828#4:1112\n1690#4:1116\n1828#4:1143\n1690#4:1147\n51#5:673\n51#5:682\n51#5:801\n388#6,6:683\n394#6,2:690\n48#7:689\n460#7,11:1085\n843#8:692\n845#8,4:706\n849#8:716\n1050#9:743\n1052#9:757\n1053#9,3:784\n1056#9:793\n363#10,6:744\n373#10,3:751\n376#10,2:755\n379#10,6:787\n396#11,2:794\n399#11:945\n401#11:1084\n108#12,5:796\n114#12:944\n70#13,5:802\n70#13,5:826\n78#13,4:858\n78#13:870\n70#13,5:871\n78#13,4:903\n81#13:907\n70#13,5:908\n78#13,4:940\n70#13,5:965\n78#13,4:997\n78#13:1009\n70#13,5:1010\n78#13,4:1042\n81#13:1046\n70#13,5:1047\n78#13,4:1079\n70#13,5:1096\n78#13,4:1128\n267#14,4:807\n237#14,7:811\n248#14,3:819\n251#14,2:823\n272#14:825\n267#14,4:831\n237#14,7:835\n248#14,3:843\n251#14,2:847\n272#14,2:849\n254#14,6:851\n274#14:857\n273#14:862\n254#14,6:863\n274#14:869\n267#14,4:876\n237#14,7:880\n248#14,3:888\n251#14,2:892\n272#14,2:894\n254#14,6:896\n274#14:902\n267#14,4:913\n237#14,7:917\n248#14,3:925\n251#14,2:929\n272#14,2:931\n254#14,6:933\n274#14:939\n237#14,14:947\n251#14,2:962\n272#14:964\n267#14,4:970\n237#14,7:974\n248#14,3:982\n251#14,2:986\n272#14,2:988\n254#14,6:990\n274#14:996\n273#14:1001\n254#14,6:1002\n274#14:1008\n267#14,4:1015\n237#14,7:1019\n248#14,3:1027\n251#14,2:1031\n272#14,2:1033\n254#14,6:1035\n274#14:1041\n267#14,4:1052\n237#14,7:1056\n248#14,3:1064\n251#14,2:1068\n272#14,2:1070\n254#14,6:1072\n274#14:1078\n267#14,4:1101\n237#14,7:1105\n248#14,3:1113\n251#14,2:1117\n272#14,2:1119\n254#14,6:1121\n274#14:1127\n267#14,4:1132\n237#14,7:1136\n248#14,3:1144\n251#14,2:1148\n272#14,2:1150\n254#14,6:1152\n274#14:1158\n1855#15:946\n1856#15:1083\n*S KotlinDebug\n*F\n+ 1 SnapshotStateObserver.kt\nandroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap\n*L\n397#1:653\n397#1:654,2\n469#1:656,3\n469#1:659,6\n469#1:666,3\n469#1:670,2\n469#1:672\n469#1:674\n469#1:675,6\n469#1:681\n512#1:693,6\n512#1:700,3\n512#1:704,2\n512#1:710,6\n526#1:717,4\n526#1:721,6\n526#1:728,3\n526#1:732,2\n526#1:734,2\n526#1:736,6\n526#1:742\n538#1:758,4\n538#1:762,6\n538#1:769,3\n538#1:773,2\n538#1:775,2\n538#1:777,6\n538#1:783\n469#1:665\n469#1:669\n512#1:699\n512#1:703\n526#1:727\n526#1:731\n535#1:750\n535#1:754\n538#1:768\n538#1:772\n588#1:818\n588#1:822\n599#1:842\n599#1:846\n599#1:887\n599#1:891\n610#1:924\n610#1:928\n588#1:961\n599#1:981\n599#1:985\n599#1:1026\n599#1:1030\n610#1:1063\n610#1:1067\n630#1:1112\n630#1:1116\n647#1:1143\n647#1:1147\n471#1:673\n479#1:682\n581#1:801\n499#1:683,6\n499#1:690,2\n499#1:689\n617#1:1085,11\n512#1:692\n512#1:706,4\n512#1:716\n535#1:743\n535#1:757\n535#1:784,3\n535#1:793\n535#1:744,6\n535#1:751,3\n535#1:755,2\n535#1:787,6\n579#1:794,2\n579#1:945\n579#1:1084\n579#1:796,5\n579#1:944\n588#1:802,5\n599#1:826,5\n599#1:858,4\n588#1:870\n599#1:871,5\n599#1:903,4\n588#1:907\n610#1:908,5\n610#1:940,4\n599#1:965,5\n599#1:997,4\n588#1:1009\n599#1:1010,5\n599#1:1042,4\n588#1:1046\n610#1:1047,5\n610#1:1079,4\n630#1:1096,5\n630#1:1128,4\n588#1:807,4\n588#1:811,7\n588#1:819,3\n588#1:823,2\n588#1:825\n599#1:831,4\n599#1:835,7\n599#1:843,3\n599#1:847,2\n599#1:849,2\n599#1:851,6\n599#1:857\n588#1:862\n588#1:863,6\n588#1:869\n599#1:876,4\n599#1:880,7\n599#1:888,3\n599#1:892,2\n599#1:894,2\n599#1:896,6\n599#1:902\n610#1:913,4\n610#1:917,7\n610#1:925,3\n610#1:929,2\n610#1:931,2\n610#1:933,6\n610#1:939\n588#1:947,14\n588#1:962,2\n588#1:964\n599#1:970,4\n599#1:974,7\n599#1:982,3\n599#1:986,2\n599#1:988,2\n599#1:990,6\n599#1:996\n588#1:1001\n588#1:1002,6\n588#1:1008\n599#1:1015,4\n599#1:1019,7\n599#1:1027,3\n599#1:1031,2\n599#1:1033,2\n599#1:1035,6\n599#1:1041\n610#1:1052,4\n610#1:1056,7\n610#1:1064,3\n610#1:1068,2\n610#1:1070,2\n610#1:1072,6\n610#1:1078\n630#1:1101,4\n630#1:1105,7\n630#1:1113,3\n630#1:1117,2\n630#1:1119,2\n630#1:1121,6\n630#1:1127\n647#1:1132,4\n647#1:1136,7\n647#1:1144,3\n647#1:1148,2\n647#1:1150,2\n647#1:1152,6\n647#1:1158\n579#1:946\n579#1:1083\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000v\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0000\n\u0002\u0018\u0002\n\u0002\u0010\u0002\n\u0002\u0008\u0003\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0008\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0003\n\u0002\u0018\u0002\n\u0002\u0008\u0003\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0006\n\u0002\u0010\u000b\n\u0002\u0008\u0004\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0010\"\n\u0002\u0008\u0006\n\u0002\u0018\u0002\n\u0002\u0008\u0004\u0008\u0002\u0018\u00002\u00020\u0001B\u0019\u0012\u0012\u0010\u0002\u001a\u000e\u0012\u0004\u0012\u00020\u0001\u0012\u0004\u0012\u00020\u00040\u0003\u00a2\u0006\u0002\u0010\u0005J\u0006\u0010\u001f\u001a\u00020\u0004J\u0010\u0010 \u001a\u00020\u00042\u0006\u0010!\u001a\u00020\u0001H\u0002J\u000e\u0010\"\u001a\u00020\u00042\u0006\u0010!\u001a\u00020\u0001J\u0006\u0010#\u001a\u00020$J\u0006\u0010%\u001a\u00020\u0004J0\u0010&\u001a\u00020\u00042\u0006\u0010!\u001a\u00020\u00012\u0012\u0010\'\u001a\u000e\u0012\u0004\u0012\u00020\u0001\u0012\u0004\u0012\u00020\u00040\u00032\u000c\u0010(\u001a\u0008\u0012\u0004\u0012\u00020\u00040)J\u0014\u0010*\u001a\u00020$2\u000c\u0010+\u001a\u0008\u0012\u0004\u0012\u00020\u00010,J\u000e\u0010-\u001a\u00020\u00042\u0006\u0010.\u001a\u00020\u0001J.\u0010-\u001a\u00020\u00042\u0006\u0010.\u001a\u00020\u00012\u0006\u0010\t\u001a\u00020\n2\u0006\u0010\u0006\u001a\u00020\u00012\u000c\u0010/\u001a\u0008\u0012\u0004\u0012\u00020\u00010\u0008H\u0002J\u0018\u00100\u001a\u00020\u00042\u0006\u0010!\u001a\u00020\u00012\u0006\u0010.\u001a\u00020\u0001H\u0002J)\u00101\u001a\u00020\u00042!\u00102\u001a\u001d\u0012\u0013\u0012\u00110\u0001\u00a2\u0006\u000c\u00083\u0012\u0008\u00084\u0012\u0004\u0008\u0008(!\u0012\u0004\u0012\u00020$0\u0003J\u0012\u00105\u001a\u00020\u00042\n\u00106\u001a\u0006\u0012\u0002\u0008\u00030\rR\u0010\u0010\u0006\u001a\u0004\u0018\u00010\u0001X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0016\u0010\u0007\u001a\n\u0012\u0004\u0012\u00020\u0001\u0018\u00010\u0008X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u000e\u0010\t\u001a\u00020\nX\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0018\u0010\u000b\u001a\u000c\u0012\u0008\u0012\u0006\u0012\u0002\u0008\u00030\r0\u000cX\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u000e\u0010\u000e\u001a\u00020\nX\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0011\u0010\u000f\u001a\u00020\u0010\u00a2\u0006\u0008\n\u0000\u001a\u0004\u0008\u0011\u0010\u0012R\u0014\u0010\u0013\u001a\u0008\u0012\u0004\u0012\u00020\u00010\u0014X\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u001d\u0010\u0002\u001a\u000e\u0012\u0004\u0012\u00020\u0001\u0012\u0004\u0012\u00020\u00040\u0003\u00a2\u0006\u0008\n\u0000\u001a\u0004\u0008\u0015\u0010\u0016R6\u0010\u0017\u001a*\u0012\u0008\u0012\u0006\u0012\u0002\u0008\u00030\r\u0012\u0006\u0012\u0004\u0018\u00010\u00010\u0018j\u0014\u0012\u0008\u0012\u0006\u0012\u0002\u0008\u00030\r\u0012\u0006\u0012\u0004\u0018\u00010\u0001`\u0019X\u0082\u0004\u00a2\u0006\u0002\n\u0000R \u0010\u001a\u001a\u0014\u0012\u0004\u0012\u00020\u0001\u0012\n\u0012\u0008\u0012\u0004\u0012\u00020\u00010\u00080\u001bX\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u0018\u0010\u001c\u001a\u000c\u0012\u0008\u0012\u0006\u0012\u0002\u0008\u00030\r0\u001dX\u0082\u0004\u00a2\u0006\u0002\n\u0000R\u0014\u0010\u001e\u001a\u0008\u0012\u0004\u0012\u00020\u00010\u000cX\u0082\u0004\u00a2\u0006\u0002\n\u0000\u00a8\u00067"
    }
    d2 = {
        "Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;",
        "",
        "onChanged",
        "Lkotlin/Function1;",
        "",
        "(Lkotlin/jvm/functions/Function1;)V",
        "currentScope",
        "currentScopeReads",
        "Landroidx/collection/MutableObjectIntMap;",
        "currentToken",
        "",
        "dependencyToDerivedStates",
        "Landroidx/compose/runtime/collection/ScopeMap;",
        "Landroidx/compose/runtime/DerivedState;",
        "deriveStateScopeCount",
        "derivedStateObserver",
        "Landroidx/compose/runtime/DerivedStateObserver;",
        "getDerivedStateObserver",
        "()Landroidx/compose/runtime/DerivedStateObserver;",
        "invalidated",
        "Landroidx/collection/MutableScatterSet;",
        "getOnChanged",
        "()Lkotlin/jvm/functions/Function1;",
        "recordedDerivedStateValues",
        "Ljava/util/HashMap;",
        "Lkotlin/collections/HashMap;",
        "scopeToValues",
        "Landroidx/collection/MutableScatterMap;",
        "statesToReread",
        "Landroidx/compose/runtime/collection/MutableVector;",
        "valueToScopes",
        "clear",
        "clearObsoleteStateReads",
        "scope",
        "clearScopeObservations",
        "hasScopeObservations",
        "",
        "notifyInvalidatedScopes",
        "observe",
        "readObserver",
        "block",
        "Lkotlin/Function0;",
        "recordInvalidation",
        "changes",
        "",
        "recordRead",
        "value",
        "recordedValues",
        "removeObservation",
        "removeScopeIf",
        "predicate",
        "Lkotlin/ParameterName;",
        "name",
        "rereadDerivedState",
        "derivedState",
        "runtime_release"
    }
    k = 0x1
    mv = {
        0x1,
        0x8,
        0x0
    }
    xi = 0x30
.end annotation


# instance fields
.field private currentScope:Ljava/lang/Object;

.field private currentScopeReads:Landroidx/collection/MutableObjectIntMap;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Landroidx/collection/MutableObjectIntMap<",
            "Ljava/lang/Object;",
            ">;"
        }
    .end annotation
.end field

.field private currentToken:I

.field private final dependencyToDerivedStates:Landroidx/compose/runtime/collection/ScopeMap;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Landroidx/compose/runtime/collection/ScopeMap<",
            "Landroidx/compose/runtime/DerivedState<",
            "*>;>;"
        }
    .end annotation
.end field

.field private deriveStateScopeCount:I

.field private final derivedStateObserver:Landroidx/compose/runtime/DerivedStateObserver;

.field private final invalidated:Landroidx/collection/MutableScatterSet;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Landroidx/collection/MutableScatterSet<",
            "Ljava/lang/Object;",
            ">;"
        }
    .end annotation
.end field

.field private final onChanged:Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field private final recordedDerivedStateValues:Ljava/util/HashMap;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/HashMap<",
            "Landroidx/compose/runtime/DerivedState<",
            "*>;",
            "Ljava/lang/Object;",
            ">;"
        }
    .end annotation
.end field

.field private final scopeToValues:Landroidx/collection/MutableScatterMap;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Landroidx/collection/MutableScatterMap<",
            "Ljava/lang/Object;",
            "Landroidx/collection/MutableObjectIntMap<",
            "Ljava/lang/Object;",
            ">;>;"
        }
    .end annotation
.end field

.field private final statesToReread:Landroidx/compose/runtime/collection/MutableVector;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Landroidx/compose/runtime/collection/MutableVector<",
            "Landroidx/compose/runtime/DerivedState<",
            "*>;>;"
        }
    .end annotation
.end field

.field private final valueToScopes:Landroidx/compose/runtime/collection/ScopeMap;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Landroidx/compose/runtime/collection/ScopeMap<",
            "Ljava/lang/Object;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method public constructor <init>(Lkotlin/jvm/functions/Function1;)V
    .locals 6
    .param p1, "onChanged"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;)V"
        }
    .end annotation

    .line 359
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 360
    iput-object p1, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->onChanged:Lkotlin/jvm/functions/Function1;

    .line 376
    const/4 v0, -0x1

    iput v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentToken:I

    .line 381
    new-instance v0, Landroidx/compose/runtime/collection/ScopeMap;

    invoke-direct {v0}, Landroidx/compose/runtime/collection/ScopeMap;-><init>()V

    iput-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->valueToScopes:Landroidx/compose/runtime/collection/ScopeMap;

    .line 387
    new-instance v0, Landroidx/collection/MutableScatterMap;

    const/4 v1, 0x0

    const/4 v2, 0x1

    const/4 v3, 0x0

    invoke-direct {v0, v1, v2, v3}, Landroidx/collection/MutableScatterMap;-><init>(IILkotlin/jvm/internal/DefaultConstructorMarker;)V

    iput-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->scopeToValues:Landroidx/collection/MutableScatterMap;

    .line 392
    new-instance v0, Landroidx/collection/MutableScatterSet;

    invoke-direct {v0, v1, v2, v3}, Landroidx/collection/MutableScatterSet;-><init>(IILkotlin/jvm/internal/DefaultConstructorMarker;)V

    iput-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->invalidated:Landroidx/collection/MutableScatterSet;

    .line 397
    const/4 v0, 0x0

    .line 653
    .local v0, "$i$f$mutableVectorOf":I
    nop

    .line 654
    const/16 v2, 0x10

    .local v2, "capacity$iv$iv":I
    const/4 v3, 0x0

    .line 655
    .local v3, "$i$f$MutableVector":I
    new-instance v4, Landroidx/compose/runtime/collection/MutableVector;

    new-array v5, v2, [Landroidx/compose/runtime/DerivedState;

    invoke-direct {v4, v5, v1}, Landroidx/compose/runtime/collection/MutableVector;-><init>([Ljava/lang/Object;I)V

    .line 653
    .end local v2    # "capacity$iv$iv":I
    .end local v3    # "$i$f$MutableVector":I
    nop

    .line 397
    .end local v0    # "$i$f$mutableVectorOf":I
    iput-object v4, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->statesToReread:Landroidx/compose/runtime/collection/MutableVector;

    .line 404
    new-instance v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap$derivedStateObserver$1;

    invoke-direct {v0, p0}, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap$derivedStateObserver$1;-><init>(Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;)V

    check-cast v0, Landroidx/compose/runtime/DerivedStateObserver;

    iput-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->derivedStateObserver:Landroidx/compose/runtime/DerivedStateObserver;

    .line 425
    new-instance v0, Landroidx/compose/runtime/collection/ScopeMap;

    invoke-direct {v0}, Landroidx/compose/runtime/collection/ScopeMap;-><init>()V

    iput-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->dependencyToDerivedStates:Landroidx/compose/runtime/collection/ScopeMap;

    .line 430
    new-instance v0, Ljava/util/HashMap;

    invoke-direct {v0}, Ljava/util/HashMap;-><init>()V

    iput-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->recordedDerivedStateValues:Ljava/util/HashMap;

    .line 360
    return-void
.end method

.method public static final synthetic access$getDeriveStateScopeCount$p(Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;)I
    .locals 1
    .param p0, "$this"    # Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;

    .line 359
    iget v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->deriveStateScopeCount:I

    return v0
.end method

.method public static final synthetic access$setDeriveStateScopeCount$p(Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;I)V
    .locals 0
    .param p0, "$this"    # Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;
    .param p1, "<set-?>"    # I

    .line 359
    iput p1, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->deriveStateScopeCount:I

    return-void
.end method

.method private final clearObsoleteStateReads(Ljava/lang/Object;)V
    .locals 24
    .param p1, "scope"    # Ljava/lang/Object;

    .line 511
    move-object/from16 v0, p0

    iget v1, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentToken:I

    .line 512
    .local v1, "currentToken":I
    iget-object v2, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScopeReads:Landroidx/collection/MutableObjectIntMap;

    if-eqz v2, :cond_a

    .local v2, "this_$iv":Landroidx/collection/MutableObjectIntMap;
    const/4 v3, 0x0

    .line 692
    .local v3, "$i$f$removeIf":I
    move-object v4, v2

    check-cast v4, Landroidx/collection/ObjectIntMap;

    .local v4, "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    const/4 v5, 0x0

    .line 693
    .local v5, "$i$f$forEachIndexed":I
    iget-object v6, v4, Landroidx/collection/ObjectIntMap;->metadata:[J

    .line 694
    .local v6, "m$iv$iv":[J
    array-length v7, v6

    add-int/lit8 v7, v7, -0x2

    .line 696
    .local v7, "lastIndex$iv$iv":I
    const/4 v8, 0x0

    .local v8, "i$iv$iv":I
    if-gt v8, v7, :cond_7

    .line 697
    :goto_0
    aget-wide v9, v6, v8

    .line 698
    .local v9, "slot$iv$iv":J
    move-wide v11, v9

    .local v11, "$this$maskEmptyOrDeleted$iv$iv$iv":J
    const/4 v13, 0x0

    .line 699
    .local v13, "$i$f$maskEmptyOrDeleted":I
    not-long v14, v11

    const/16 v16, 0x7

    shl-long v14, v14, v16

    and-long/2addr v14, v11

    const-wide v16, -0x7f7f7f7f7f7f7f80L    # -2.937446524422997E-306

    and-long v11, v14, v16

    .line 698
    .end local v11    # "$this$maskEmptyOrDeleted$iv$iv$iv":J
    .end local v13    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v11, v11, v16

    if-eqz v11, :cond_6

    .line 700
    sub-int v11, v8, v7

    not-int v11, v11

    ushr-int/lit8 v11, v11, 0x1f

    const/16 v12, 0x8

    rsub-int/lit8 v11, v11, 0x8

    .line 701
    .local v11, "bitCount$iv$iv":I
    const/4 v13, 0x0

    .local v13, "j$iv$iv":I
    :goto_1
    if-ge v13, v11, :cond_5

    .line 702
    const-wide/16 v14, 0xff

    and-long/2addr v14, v9

    .local v14, "value$iv$iv$iv":J
    const/16 v16, 0x0

    .line 703
    .local v16, "$i$f$isFull":I
    const-wide/16 v17, 0x80

    cmp-long v17, v14, v17

    const/16 v18, 0x0

    const/16 v19, 0x1

    if-gez v17, :cond_0

    move/from16 v14, v19

    goto :goto_2

    :cond_0
    move/from16 v14, v18

    .line 702
    .end local v14    # "value$iv$iv$iv":J
    .end local v16    # "$i$f$isFull":I
    :goto_2
    if-eqz v14, :cond_4

    .line 704
    shl-int/lit8 v14, v8, 0x3

    add-int/2addr v14, v13

    .line 705
    .local v14, "index$iv$iv":I
    move v15, v14

    .local v15, "index$iv":I
    const/16 v16, 0x0

    .line 706
    .local v16, "$i$a$-forEachIndexed-MutableObjectIntMap$removeIf$1$iv":I
    iget-object v12, v2, Landroidx/collection/MutableObjectIntMap;->keys:[Ljava/lang/Object;

    aget-object v12, v12, v15

    move/from16 v20, v3

    .end local v3    # "$i$f$removeIf":I
    .local v12, "value":Ljava/lang/Object;
    .local v20, "$i$f$removeIf":I
    iget-object v3, v2, Landroidx/collection/MutableObjectIntMap;->values:[I

    aget v3, v3, v15

    .local v3, "token":I
    const/16 v21, 0x0

    .line 513
    .local v21, "$i$a$-removeIf-SnapshotStateObserver$ObservedScopeMap$clearObsoleteStateReads$1":I
    if-eq v3, v1, :cond_1

    move/from16 v18, v19

    :cond_1
    move/from16 v19, v18

    .local v19, "willRemove":Z
    const/16 v22, 0x0

    .line 514
    .local v22, "$i$a$-also-SnapshotStateObserver$ObservedScopeMap$clearObsoleteStateReads$1$1":I
    if-eqz v19, :cond_2

    .line 515
    move/from16 v23, v1

    move-object/from16 v1, p1

    .end local v1    # "currentToken":I
    .local v23, "currentToken":I
    invoke-direct {v0, v1, v12}, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->removeObservation(Ljava/lang/Object;Ljava/lang/Object;)V

    goto :goto_3

    .line 514
    .end local v23    # "currentToken":I
    .restart local v1    # "currentToken":I
    :cond_2
    move/from16 v23, v1

    move-object/from16 v1, p1

    .line 517
    .end local v1    # "currentToken":I
    .restart local v23    # "currentToken":I
    :goto_3
    nop

    .line 513
    .end local v19    # "willRemove":Z
    .end local v22    # "$i$a$-also-SnapshotStateObserver$ObservedScopeMap$clearObsoleteStateReads$1$1":I
    nop

    .line 706
    .end local v3    # "token":I
    .end local v12    # "value":Ljava/lang/Object;
    .end local v21    # "$i$a$-removeIf-SnapshotStateObserver$ObservedScopeMap$clearObsoleteStateReads$1":I
    if-eqz v18, :cond_3

    .line 707
    invoke-virtual {v2, v15}, Landroidx/collection/MutableObjectIntMap;->removeValueAt(I)V

    .line 709
    :cond_3
    nop

    .line 705
    .end local v15    # "index$iv":I
    .end local v16    # "$i$a$-forEachIndexed-MutableObjectIntMap$removeIf$1$iv":I
    goto :goto_4

    .line 702
    .end local v14    # "index$iv$iv":I
    .end local v20    # "$i$f$removeIf":I
    .end local v23    # "currentToken":I
    .restart local v1    # "currentToken":I
    .local v3, "$i$f$removeIf":I
    :cond_4
    move/from16 v23, v1

    move/from16 v20, v3

    move-object/from16 v1, p1

    .line 710
    .end local v1    # "currentToken":I
    .end local v3    # "$i$f$removeIf":I
    .restart local v20    # "$i$f$removeIf":I
    .restart local v23    # "currentToken":I
    :goto_4
    const/16 v3, 0x8

    shr-long/2addr v9, v3

    .line 701
    add-int/lit8 v13, v13, 0x1

    move v12, v3

    move/from16 v3, v20

    move/from16 v1, v23

    goto :goto_1

    .end local v20    # "$i$f$removeIf":I
    .end local v23    # "currentToken":I
    .restart local v1    # "currentToken":I
    .restart local v3    # "$i$f$removeIf":I
    :cond_5
    move/from16 v23, v1

    move/from16 v20, v3

    move v3, v12

    move-object/from16 v1, p1

    .line 712
    .end local v1    # "currentToken":I
    .end local v3    # "$i$f$removeIf":I
    .end local v13    # "j$iv$iv":I
    .restart local v20    # "$i$f$removeIf":I
    .restart local v23    # "currentToken":I
    if-ne v11, v3, :cond_9

    goto :goto_5

    .line 698
    .end local v11    # "bitCount$iv$iv":I
    .end local v20    # "$i$f$removeIf":I
    .end local v23    # "currentToken":I
    .restart local v1    # "currentToken":I
    .restart local v3    # "$i$f$removeIf":I
    :cond_6
    move/from16 v23, v1

    move/from16 v20, v3

    move-object/from16 v1, p1

    .line 696
    .end local v1    # "currentToken":I
    .end local v3    # "$i$f$removeIf":I
    .end local v9    # "slot$iv$iv":J
    .restart local v20    # "$i$f$removeIf":I
    .restart local v23    # "currentToken":I
    :goto_5
    if-eq v8, v7, :cond_8

    add-int/lit8 v8, v8, 0x1

    move/from16 v3, v20

    move/from16 v1, v23

    goto/16 :goto_0

    .end local v20    # "$i$f$removeIf":I
    .end local v23    # "currentToken":I
    .restart local v1    # "currentToken":I
    .restart local v3    # "$i$f$removeIf":I
    :cond_7
    move/from16 v23, v1

    move/from16 v20, v3

    move-object/from16 v1, p1

    .line 715
    .end local v1    # "currentToken":I
    .end local v3    # "$i$f$removeIf":I
    .end local v8    # "i$iv$iv":I
    .restart local v20    # "$i$f$removeIf":I
    .restart local v23    # "currentToken":I
    :cond_8
    nop

    .line 716
    .end local v4    # "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .end local v5    # "$i$f$forEachIndexed":I
    .end local v6    # "m$iv$iv":[J
    .end local v7    # "lastIndex$iv$iv":I
    :cond_9
    nop

    .end local v2    # "this_$iv":Landroidx/collection/MutableObjectIntMap;
    .end local v20    # "$i$f$removeIf":I
    goto :goto_6

    .line 512
    .end local v23    # "currentToken":I
    .restart local v1    # "currentToken":I
    :cond_a
    move/from16 v23, v1

    move-object/from16 v1, p1

    .line 519
    .end local v1    # "currentToken":I
    .restart local v23    # "currentToken":I
    :goto_6
    return-void
.end method

.method private final recordRead(Ljava/lang/Object;ILjava/lang/Object;Landroidx/collection/MutableObjectIntMap;)V
    .locals 29
    .param p1, "value"    # Ljava/lang/Object;
    .param p2, "currentToken"    # I
    .param p3, "currentScope"    # Ljava/lang/Object;
    .param p4, "recordedValues"    # Landroidx/collection/MutableObjectIntMap;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/lang/Object;",
            "I",
            "Ljava/lang/Object;",
            "Landroidx/collection/MutableObjectIntMap<",
            "Ljava/lang/Object;",
            ">;)V"
        }
    .end annotation

    .line 454
    move-object/from16 v0, p0

    move-object/from16 v1, p1

    move/from16 v2, p2

    iget v3, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->deriveStateScopeCount:I

    if-lez v3, :cond_0

    .line 456
    return-void

    .line 459
    :cond_0
    const/4 v3, -0x1

    move-object/from16 v4, p4

    invoke-virtual {v4, v1, v2, v3}, Landroidx/collection/MutableObjectIntMap;->put(Ljava/lang/Object;II)I

    move-result v5

    .line 460
    .local v5, "previousToken":I
    instance-of v6, v1, Landroidx/compose/runtime/DerivedState;

    const/4 v7, 0x2

    if-eqz v6, :cond_9

    if-eq v5, v2, :cond_9

    .line 461
    move-object v6, v1

    check-cast v6, Landroidx/compose/runtime/DerivedState;

    invoke-interface {v6}, Landroidx/compose/runtime/DerivedState;->getCurrentRecord()Landroidx/compose/runtime/DerivedState$Record;

    move-result-object v6

    .line 463
    .local v6, "record":Landroidx/compose/runtime/DerivedState$Record;
    iget-object v8, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->recordedDerivedStateValues:Ljava/util/HashMap;

    check-cast v8, Ljava/util/Map;

    invoke-interface {v6}, Landroidx/compose/runtime/DerivedState$Record;->getCurrentValue()Ljava/lang/Object;

    move-result-object v9

    invoke-interface {v8, v1, v9}, Ljava/util/Map;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    .line 465
    invoke-interface {v6}, Landroidx/compose/runtime/DerivedState$Record;->getDependencies()Landroidx/collection/ObjectIntMap;

    move-result-object v8

    .line 466
    .local v8, "dependencies":Landroidx/collection/ObjectIntMap;
    iget-object v9, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->dependencyToDerivedStates:Landroidx/compose/runtime/collection/ScopeMap;

    .line 468
    .local v9, "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    invoke-virtual {v9, v1}, Landroidx/compose/runtime/collection/ScopeMap;->removeScope(Ljava/lang/Object;)V

    .line 469
    move-object v10, v8

    .local v10, "this_$iv":Landroidx/collection/ObjectIntMap;
    const/4 v11, 0x0

    .line 656
    .local v11, "$i$f$forEachKey":I
    iget-object v12, v10, Landroidx/collection/ObjectIntMap;->keys:[Ljava/lang/Object;

    .line 658
    .local v12, "k$iv":[Ljava/lang/Object;
    move-object v13, v10

    .local v13, "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    const/4 v14, 0x0

    .line 659
    .local v14, "$i$f$forEachIndexed":I
    iget-object v15, v13, Landroidx/collection/ObjectIntMap;->metadata:[J

    .line 660
    .local v15, "m$iv$iv":[J
    array-length v3, v15

    sub-int/2addr v3, v7

    .line 662
    .local v3, "lastIndex$iv$iv":I
    const/4 v7, 0x0

    .local v7, "i$iv$iv":I
    if-gt v7, v3, :cond_6

    .line 663
    :goto_0
    aget-wide v17, v15, v7

    .line 664
    .local v17, "slot$iv$iv":J
    move-wide/from16 v19, v17

    .local v19, "$this$maskEmptyOrDeleted$iv$iv$iv":J
    const/16 v21, 0x0

    .line 665
    .local v21, "$i$f$maskEmptyOrDeleted":I
    move-object/from16 v22, v10

    move/from16 v23, v11

    move-wide/from16 v10, v19

    move-object/from16 v19, v13

    move/from16 v20, v14

    .end local v11    # "$i$f$forEachKey":I
    .end local v13    # "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .end local v14    # "$i$f$forEachIndexed":I
    .local v10, "$this$maskEmptyOrDeleted$iv$iv$iv":J
    .local v19, "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .local v20, "$i$f$forEachIndexed":I
    .local v22, "this_$iv":Landroidx/collection/ObjectIntMap;
    .local v23, "$i$f$forEachKey":I
    not-long v13, v10

    const/16 v24, 0x7

    shl-long v13, v13, v24

    and-long/2addr v13, v10

    const-wide v24, -0x7f7f7f7f7f7f7f80L    # -2.937446524422997E-306

    and-long v10, v13, v24

    .line 664
    .end local v10    # "$this$maskEmptyOrDeleted$iv$iv$iv":J
    .end local v21    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v10, v10, v24

    if-eqz v10, :cond_5

    .line 666
    sub-int v10, v7, v3

    not-int v10, v10

    ushr-int/lit8 v10, v10, 0x1f

    const/16 v11, 0x8

    rsub-int/lit8 v10, v10, 0x8

    .line 667
    .local v10, "bitCount$iv$iv":I
    const/4 v13, 0x0

    .local v13, "j$iv$iv":I
    :goto_1
    if-ge v13, v10, :cond_4

    .line 668
    const-wide/16 v24, 0xff

    and-long v24, v17, v24

    .local v24, "value$iv$iv$iv":J
    const/4 v14, 0x0

    .line 669
    .local v14, "$i$f$isFull":I
    const-wide/16 v26, 0x80

    cmp-long v21, v24, v26

    if-gez v21, :cond_1

    const/16 v21, 0x1

    goto :goto_2

    :cond_1
    const/16 v21, 0x0

    .line 668
    .end local v14    # "$i$f$isFull":I
    .end local v24    # "value$iv$iv$iv":J
    :goto_2
    if-eqz v21, :cond_3

    .line 670
    shl-int/lit8 v14, v7, 0x3

    add-int/2addr v14, v13

    .line 671
    .local v14, "index$iv$iv":I
    move/from16 v21, v14

    .local v21, "index$iv":I
    const/16 v24, 0x0

    .line 672
    .local v24, "$i$a$-forEachIndexed-ObjectIntMap$forEachKey$1$iv":I
    aget-object v25, v12, v21

    move-object/from16 v11, v25

    check-cast v11, Landroidx/compose/runtime/snapshots/StateObject;

    .local v11, "dependency":Landroidx/compose/runtime/snapshots/StateObject;
    const/16 v25, 0x0

    .line 470
    .local v25, "$i$a$-forEachKey-SnapshotStateObserver$ObservedScopeMap$recordRead$2":I
    instance-of v2, v11, Landroidx/compose/runtime/snapshots/StateObjectImpl;

    if-eqz v2, :cond_2

    .line 471
    move-object v2, v11

    check-cast v2, Landroidx/compose/runtime/snapshots/StateObjectImpl;

    sget-object v27, Landroidx/compose/runtime/snapshots/ReaderKind;->Companion:Landroidx/compose/runtime/snapshots/ReaderKind$Companion;

    .local v27, "this_$iv":Landroidx/compose/runtime/snapshots/ReaderKind$Companion;
    const/16 v28, 0x0

    .line 673
    .local v28, "$i$f$getSnapshotStateObserver-6f8NoZ8":I
    const/16 v16, 0x2

    invoke-static/range {v16 .. v16}, Landroidx/compose/runtime/snapshots/ReaderKind;->constructor-impl(I)I

    move-result v4

    .line 471
    .end local v27    # "this_$iv":Landroidx/compose/runtime/snapshots/ReaderKind$Companion;
    .end local v28    # "$i$f$getSnapshotStateObserver-6f8NoZ8":I
    invoke-virtual {v2, v4}, Landroidx/compose/runtime/snapshots/StateObjectImpl;->recordReadIn-h_f27i8$runtime_release(I)V

    .line 473
    :cond_2
    invoke-virtual {v9, v11, v1}, Landroidx/compose/runtime/collection/ScopeMap;->add(Ljava/lang/Object;Ljava/lang/Object;)V

    .line 474
    nop

    .line 672
    .end local v11    # "dependency":Landroidx/compose/runtime/snapshots/StateObject;
    .end local v25    # "$i$a$-forEachKey-SnapshotStateObserver$ObservedScopeMap$recordRead$2":I
    nop

    .line 674
    nop

    .line 671
    .end local v21    # "index$iv":I
    .end local v24    # "$i$a$-forEachIndexed-ObjectIntMap$forEachKey$1$iv":I
    nop

    .line 675
    .end local v14    # "index$iv$iv":I
    :cond_3
    const/16 v2, 0x8

    shr-long v17, v17, v2

    .line 667
    add-int/lit8 v13, v13, 0x1

    move-object/from16 v4, p4

    move v11, v2

    move/from16 v2, p2

    goto :goto_1

    :cond_4
    move v2, v11

    .line 677
    .end local v13    # "j$iv$iv":I
    if-ne v10, v2, :cond_8

    .line 662
    .end local v10    # "bitCount$iv$iv":I
    .end local v17    # "slot$iv$iv":J
    :cond_5
    if-eq v7, v3, :cond_7

    add-int/lit8 v7, v7, 0x1

    move/from16 v2, p2

    move-object/from16 v4, p4

    move-object/from16 v13, v19

    move/from16 v14, v20

    move-object/from16 v10, v22

    move/from16 v11, v23

    goto/16 :goto_0

    .end local v19    # "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .end local v20    # "$i$f$forEachIndexed":I
    .end local v22    # "this_$iv":Landroidx/collection/ObjectIntMap;
    .end local v23    # "$i$f$forEachKey":I
    .local v10, "this_$iv":Landroidx/collection/ObjectIntMap;
    .local v11, "$i$f$forEachKey":I
    .local v13, "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .local v14, "$i$f$forEachIndexed":I
    :cond_6
    move-object/from16 v22, v10

    move/from16 v23, v11

    move-object/from16 v19, v13

    move/from16 v20, v14

    .line 680
    .end local v7    # "i$iv$iv":I
    .end local v10    # "this_$iv":Landroidx/collection/ObjectIntMap;
    .end local v11    # "$i$f$forEachKey":I
    .end local v13    # "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .end local v14    # "$i$f$forEachIndexed":I
    .restart local v19    # "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .restart local v20    # "$i$f$forEachIndexed":I
    .restart local v22    # "this_$iv":Landroidx/collection/ObjectIntMap;
    .restart local v23    # "$i$f$forEachKey":I
    :cond_7
    nop

    .line 681
    .end local v3    # "lastIndex$iv$iv":I
    .end local v15    # "m$iv$iv":[J
    .end local v19    # "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .end local v20    # "$i$f$forEachIndexed":I
    :cond_8
    nop

    .line 477
    .end local v6    # "record":Landroidx/compose/runtime/DerivedState$Record;
    .end local v8    # "dependencies":Landroidx/collection/ObjectIntMap;
    .end local v9    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v12    # "k$iv":[Ljava/lang/Object;
    .end local v22    # "this_$iv":Landroidx/collection/ObjectIntMap;
    .end local v23    # "$i$f$forEachKey":I
    :cond_9
    const/4 v2, -0x1

    if-ne v5, v2, :cond_b

    .line 478
    instance-of v2, v1, Landroidx/compose/runtime/snapshots/StateObjectImpl;

    if-eqz v2, :cond_a

    .line 479
    move-object v2, v1

    check-cast v2, Landroidx/compose/runtime/snapshots/StateObjectImpl;

    sget-object v3, Landroidx/compose/runtime/snapshots/ReaderKind;->Companion:Landroidx/compose/runtime/snapshots/ReaderKind$Companion;

    .local v3, "this_$iv":Landroidx/compose/runtime/snapshots/ReaderKind$Companion;
    const/4 v4, 0x0

    .line 682
    .local v4, "$i$f$getSnapshotStateObserver-6f8NoZ8":I
    const/4 v6, 0x2

    invoke-static {v6}, Landroidx/compose/runtime/snapshots/ReaderKind;->constructor-impl(I)I

    move-result v3

    .line 479
    .end local v3    # "this_$iv":Landroidx/compose/runtime/snapshots/ReaderKind$Companion;
    .end local v4    # "$i$f$getSnapshotStateObserver-6f8NoZ8":I
    invoke-virtual {v2, v3}, Landroidx/compose/runtime/snapshots/StateObjectImpl;->recordReadIn-h_f27i8$runtime_release(I)V

    .line 481
    :cond_a
    iget-object v2, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->valueToScopes:Landroidx/compose/runtime/collection/ScopeMap;

    move-object/from16 v3, p3

    invoke-virtual {v2, v1, v3}, Landroidx/compose/runtime/collection/ScopeMap;->add(Ljava/lang/Object;Ljava/lang/Object;)V

    goto :goto_3

    .line 477
    :cond_b
    move-object/from16 v3, p3

    .line 483
    :goto_3
    return-void
.end method

.method private final removeObservation(Ljava/lang/Object;Ljava/lang/Object;)V
    .locals 1
    .param p1, "scope"    # Ljava/lang/Object;
    .param p2, "value"    # Ljava/lang/Object;

    .line 550
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->valueToScopes:Landroidx/compose/runtime/collection/ScopeMap;

    invoke-virtual {v0, p2, p1}, Landroidx/compose/runtime/collection/ScopeMap;->remove(Ljava/lang/Object;Ljava/lang/Object;)Z

    .line 551
    instance-of v0, p2, Landroidx/compose/runtime/DerivedState;

    if-eqz v0, :cond_0

    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->valueToScopes:Landroidx/compose/runtime/collection/ScopeMap;

    invoke-virtual {v0, p2}, Landroidx/compose/runtime/collection/ScopeMap;->contains(Ljava/lang/Object;)Z

    move-result v0

    if-nez v0, :cond_0

    .line 552
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->dependencyToDerivedStates:Landroidx/compose/runtime/collection/ScopeMap;

    invoke-virtual {v0, p2}, Landroidx/compose/runtime/collection/ScopeMap;->removeScope(Ljava/lang/Object;)V

    .line 553
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->recordedDerivedStateValues:Ljava/util/HashMap;

    invoke-virtual {v0, p2}, Ljava/util/HashMap;->remove(Ljava/lang/Object;)Ljava/lang/Object;

    .line 555
    :cond_0
    return-void
.end method


# virtual methods
.method public final clear()V
    .locals 1

    .line 561
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->valueToScopes:Landroidx/compose/runtime/collection/ScopeMap;

    invoke-virtual {v0}, Landroidx/compose/runtime/collection/ScopeMap;->clear()V

    .line 562
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->scopeToValues:Landroidx/collection/MutableScatterMap;

    invoke-virtual {v0}, Landroidx/collection/MutableScatterMap;->clear()V

    .line 563
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->dependencyToDerivedStates:Landroidx/compose/runtime/collection/ScopeMap;

    invoke-virtual {v0}, Landroidx/compose/runtime/collection/ScopeMap;->clear()V

    .line 564
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->recordedDerivedStateValues:Ljava/util/HashMap;

    invoke-virtual {v0}, Ljava/util/HashMap;->clear()V

    .line 565
    return-void
.end method

.method public final clearScopeObservations(Ljava/lang/Object;)V
    .locals 22
    .param p1, "scope"    # Ljava/lang/Object;

    .line 525
    move-object/from16 v0, p0

    move-object/from16 v1, p1

    iget-object v2, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->scopeToValues:Landroidx/collection/MutableScatterMap;

    invoke-virtual {v2, v1}, Landroidx/collection/MutableScatterMap;->remove(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Landroidx/collection/MutableObjectIntMap;

    if-nez v2, :cond_0

    return-void

    .line 526
    .local v2, "recordedValues":Landroidx/collection/MutableObjectIntMap;
    :cond_0
    move-object v3, v2

    check-cast v3, Landroidx/collection/ObjectIntMap;

    .local v3, "this_$iv":Landroidx/collection/ObjectIntMap;
    const/4 v4, 0x0

    .line 717
    .local v4, "$i$f$forEach":I
    iget-object v5, v3, Landroidx/collection/ObjectIntMap;->keys:[Ljava/lang/Object;

    .line 718
    .local v5, "k$iv":[Ljava/lang/Object;
    iget-object v6, v3, Landroidx/collection/ObjectIntMap;->values:[I

    .line 720
    .local v6, "v$iv":[I
    move-object v7, v3

    .local v7, "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    const/4 v8, 0x0

    .line 721
    .local v8, "$i$f$forEachIndexed":I
    iget-object v9, v7, Landroidx/collection/ObjectIntMap;->metadata:[J

    .line 722
    .local v9, "m$iv$iv":[J
    array-length v10, v9

    add-int/lit8 v10, v10, -0x2

    .line 724
    .local v10, "lastIndex$iv$iv":I
    const/4 v11, 0x0

    .local v11, "i$iv$iv":I
    if-gt v11, v10, :cond_5

    .line 725
    :goto_0
    aget-wide v12, v9, v11

    .line 726
    .local v12, "slot$iv$iv":J
    move-wide v14, v12

    .local v14, "$this$maskEmptyOrDeleted$iv$iv$iv":J
    const/16 v16, 0x0

    .line 727
    .local v16, "$i$f$maskEmptyOrDeleted":I
    move-object/from16 v17, v2

    move-object/from16 v18, v3

    .end local v2    # "recordedValues":Landroidx/collection/MutableObjectIntMap;
    .end local v3    # "this_$iv":Landroidx/collection/ObjectIntMap;
    .local v17, "recordedValues":Landroidx/collection/MutableObjectIntMap;
    .local v18, "this_$iv":Landroidx/collection/ObjectIntMap;
    not-long v2, v14

    const/16 v19, 0x7

    shl-long v2, v2, v19

    and-long/2addr v2, v14

    const-wide v19, -0x7f7f7f7f7f7f7f80L    # -2.937446524422997E-306

    and-long v2, v2, v19

    .line 726
    .end local v14    # "$this$maskEmptyOrDeleted$iv$iv$iv":J
    .end local v16    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v2, v2, v19

    if-eqz v2, :cond_4

    .line 728
    sub-int v2, v11, v10

    not-int v2, v2

    ushr-int/lit8 v2, v2, 0x1f

    const/16 v3, 0x8

    rsub-int/lit8 v2, v2, 0x8

    .line 729
    .local v2, "bitCount$iv$iv":I
    const/4 v14, 0x0

    .local v14, "j$iv$iv":I
    :goto_1
    if-ge v14, v2, :cond_3

    .line 730
    const-wide/16 v15, 0xff

    and-long/2addr v15, v12

    .local v15, "value$iv$iv$iv":J
    const/16 v19, 0x0

    .line 731
    .local v19, "$i$f$isFull":I
    const-wide/16 v20, 0x80

    cmp-long v20, v15, v20

    if-gez v20, :cond_1

    const/16 v20, 0x1

    goto :goto_2

    :cond_1
    const/16 v20, 0x0

    .line 730
    .end local v15    # "value$iv$iv$iv":J
    .end local v19    # "$i$f$isFull":I
    :goto_2
    if-eqz v20, :cond_2

    .line 732
    shl-int/lit8 v15, v11, 0x3

    add-int/2addr v15, v14

    .line 733
    .local v15, "index$iv$iv":I
    move/from16 v16, v15

    .local v16, "index$iv":I
    const/16 v19, 0x0

    .line 734
    .local v19, "$i$a$-forEachIndexed-ObjectIntMap$forEach$1$iv":I
    aget-object v3, v5, v16

    .local v3, "value":Ljava/lang/Object;
    aget v21, v6, v16

    const/16 v21, 0x0

    .line 527
    .local v21, "$i$a$-forEach-SnapshotStateObserver$ObservedScopeMap$clearScopeObservations$1":I
    invoke-direct {v0, v1, v3}, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->removeObservation(Ljava/lang/Object;Ljava/lang/Object;)V

    .line 528
    nop

    .line 734
    .end local v3    # "value":Ljava/lang/Object;
    .end local v21    # "$i$a$-forEach-SnapshotStateObserver$ObservedScopeMap$clearScopeObservations$1":I
    nop

    .line 735
    nop

    .line 733
    .end local v16    # "index$iv":I
    .end local v19    # "$i$a$-forEachIndexed-ObjectIntMap$forEach$1$iv":I
    nop

    .line 736
    .end local v15    # "index$iv$iv":I
    :cond_2
    const/16 v3, 0x8

    shr-long/2addr v12, v3

    .line 729
    add-int/lit8 v14, v14, 0x1

    goto :goto_1

    .line 738
    .end local v14    # "j$iv$iv":I
    :cond_3
    if-ne v2, v3, :cond_7

    .line 724
    .end local v2    # "bitCount$iv$iv":I
    .end local v12    # "slot$iv$iv":J
    :cond_4
    if-eq v11, v10, :cond_6

    add-int/lit8 v11, v11, 0x1

    move-object/from16 v2, v17

    move-object/from16 v3, v18

    goto :goto_0

    .end local v17    # "recordedValues":Landroidx/collection/MutableObjectIntMap;
    .end local v18    # "this_$iv":Landroidx/collection/ObjectIntMap;
    .local v2, "recordedValues":Landroidx/collection/MutableObjectIntMap;
    .local v3, "this_$iv":Landroidx/collection/ObjectIntMap;
    :cond_5
    move-object/from16 v17, v2

    move-object/from16 v18, v3

    .line 741
    .end local v2    # "recordedValues":Landroidx/collection/MutableObjectIntMap;
    .end local v3    # "this_$iv":Landroidx/collection/ObjectIntMap;
    .end local v11    # "i$iv$iv":I
    .restart local v17    # "recordedValues":Landroidx/collection/MutableObjectIntMap;
    .restart local v18    # "this_$iv":Landroidx/collection/ObjectIntMap;
    :cond_6
    nop

    .line 742
    .end local v7    # "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .end local v8    # "$i$f$forEachIndexed":I
    .end local v9    # "m$iv$iv":[J
    .end local v10    # "lastIndex$iv$iv":I
    :cond_7
    nop

    .line 529
    .end local v4    # "$i$f$forEach":I
    .end local v5    # "k$iv":[Ljava/lang/Object;
    .end local v6    # "v$iv":[I
    .end local v18    # "this_$iv":Landroidx/collection/ObjectIntMap;
    return-void
.end method

.method public final getDerivedStateObserver()Landroidx/compose/runtime/DerivedStateObserver;
    .locals 1

    .line 404
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->derivedStateObserver:Landroidx/compose/runtime/DerivedStateObserver;

    return-object v0
.end method

.method public final getOnChanged()Lkotlin/jvm/functions/Function1;
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation

    .line 360
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->onChanged:Lkotlin/jvm/functions/Function1;

    return-object v0
.end method

.method public final hasScopeObservations()Z
    .locals 1

    .line 547
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->scopeToValues:Landroidx/collection/MutableScatterMap;

    invoke-virtual {v0}, Landroidx/collection/MutableScatterMap;->isNotEmpty()Z

    move-result v0

    return v0
.end method

.method public final notifyInvalidatedScopes()V
    .locals 21

    .line 646
    move-object/from16 v0, p0

    iget-object v1, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->invalidated:Landroidx/collection/MutableScatterSet;

    .line 647
    .local v1, "invalidated":Landroidx/collection/MutableScatterSet;
    move-object v2, v1

    check-cast v2, Landroidx/collection/ScatterSet;

    .local v2, "this_$iv":Landroidx/collection/ScatterSet;
    iget-object v3, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->onChanged:Lkotlin/jvm/functions/Function1;

    .local v3, "block$iv":Lkotlin/jvm/functions/Function1;
    const/4 v4, 0x0

    .line 1132
    .local v4, "$i$f$forEach":I
    nop

    .line 1133
    iget-object v5, v2, Landroidx/collection/ScatterSet;->elements:[Ljava/lang/Object;

    .line 1135
    .local v5, "k$iv":[Ljava/lang/Object;
    move-object v6, v2

    .local v6, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v7, 0x0

    .line 1136
    .local v7, "$i$f$forEachIndex":I
    nop

    .line 1137
    iget-object v8, v6, Landroidx/collection/ScatterSet;->metadata:[J

    .line 1138
    .local v8, "m$iv$iv":[J
    array-length v9, v8

    add-int/lit8 v9, v9, -0x2

    .line 1140
    .local v9, "lastIndex$iv$iv":I
    const/4 v10, 0x0

    .local v10, "i$iv$iv":I
    if-gt v10, v9, :cond_4

    .line 1141
    :goto_0
    aget-wide v11, v8, v10

    .line 1142
    .local v11, "slot$iv$iv":J
    move-wide v13, v11

    .local v13, "$this$maskEmptyOrDeleted$iv$iv$iv":J
    const/4 v15, 0x0

    .line 1143
    .local v15, "$i$f$maskEmptyOrDeleted":I
    move-object/from16 v16, v6

    move/from16 v17, v7

    .end local v6    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v7    # "$i$f$forEachIndex":I
    .local v16, "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v17, "$i$f$forEachIndex":I
    not-long v6, v13

    const/16 v18, 0x7

    shl-long v6, v6, v18

    and-long/2addr v6, v13

    const-wide v18, -0x7f7f7f7f7f7f7f80L    # -2.937446524422997E-306

    and-long v6, v6, v18

    .line 1142
    .end local v13    # "$this$maskEmptyOrDeleted$iv$iv$iv":J
    .end local v15    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v6, v6, v18

    if-eqz v6, :cond_3

    .line 1144
    sub-int v6, v10, v9

    not-int v6, v6

    ushr-int/lit8 v6, v6, 0x1f

    const/16 v7, 0x8

    rsub-int/lit8 v6, v6, 0x8

    .line 1145
    .local v6, "bitCount$iv$iv":I
    const/4 v13, 0x0

    .local v13, "j$iv$iv":I
    :goto_1
    if-ge v13, v6, :cond_2

    .line 1146
    const-wide/16 v14, 0xff

    and-long/2addr v14, v11

    .local v14, "value$iv$iv$iv":J
    const/16 v18, 0x0

    .line 1147
    .local v18, "$i$f$isFull":I
    const-wide/16 v19, 0x80

    cmp-long v19, v14, v19

    if-gez v19, :cond_0

    const/16 v19, 0x1

    goto :goto_2

    :cond_0
    const/16 v19, 0x0

    .line 1146
    .end local v14    # "value$iv$iv$iv":J
    .end local v18    # "$i$f$isFull":I
    :goto_2
    if-eqz v19, :cond_1

    .line 1148
    shl-int/lit8 v14, v10, 0x3

    add-int/2addr v14, v13

    .line 1149
    .local v14, "index$iv$iv":I
    move v15, v14

    .local v15, "index$iv":I
    const/16 v18, 0x0

    .line 1150
    .local v18, "$i$a$-forEachIndex-ScatterSet$forEach$2$iv":I
    aget-object v7, v5, v15

    invoke-interface {v3, v7}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    .line 1151
    nop

    .line 1149
    .end local v15    # "index$iv":I
    .end local v18    # "$i$a$-forEachIndex-ScatterSet$forEach$2$iv":I
    nop

    .line 1152
    .end local v14    # "index$iv$iv":I
    :cond_1
    const/16 v7, 0x8

    shr-long/2addr v11, v7

    .line 1145
    add-int/lit8 v13, v13, 0x1

    goto :goto_1

    .line 1154
    .end local v13    # "j$iv$iv":I
    :cond_2
    if-ne v6, v7, :cond_6

    .line 1140
    .end local v6    # "bitCount$iv$iv":I
    .end local v11    # "slot$iv$iv":J
    :cond_3
    if-eq v10, v9, :cond_5

    add-int/lit8 v10, v10, 0x1

    move-object/from16 v6, v16

    move/from16 v7, v17

    goto :goto_0

    .end local v16    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v17    # "$i$f$forEachIndex":I
    .local v6, "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v7    # "$i$f$forEachIndex":I
    :cond_4
    move-object/from16 v16, v6

    move/from16 v17, v7

    .line 1157
    .end local v6    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v7    # "$i$f$forEachIndex":I
    .end local v10    # "i$iv$iv":I
    .restart local v16    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v17    # "$i$f$forEachIndex":I
    :cond_5
    nop

    .line 1158
    .end local v8    # "m$iv$iv":[J
    .end local v9    # "lastIndex$iv$iv":I
    .end local v16    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v17    # "$i$f$forEachIndex":I
    :cond_6
    nop

    .line 648
    .end local v2    # "this_$iv":Landroidx/collection/ScatterSet;
    .end local v3    # "block$iv":Lkotlin/jvm/functions/Function1;
    .end local v4    # "$i$f$forEach":I
    .end local v5    # "k$iv":[Ljava/lang/Object;
    invoke-virtual {v1}, Landroidx/collection/MutableScatterSet;->clear()V

    .line 649
    return-void
.end method

.method public final observe(Ljava/lang/Object;Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function0;)V
    .locals 10
    .param p1, "scope"    # Ljava/lang/Object;
    .param p2, "readObserver"    # Lkotlin/jvm/functions/Function1;
    .param p3, "block"    # Lkotlin/jvm/functions/Function0;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/lang/Object;",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;",
            "Lkotlin/jvm/functions/Function0<",
            "Lkotlin/Unit;",
            ">;)V"
        }
    .end annotation

    .line 489
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScope:Ljava/lang/Object;

    .line 490
    .local v0, "previousScope":Ljava/lang/Object;
    iget-object v1, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScopeReads:Landroidx/collection/MutableObjectIntMap;

    .line 491
    .local v1, "previousReads":Landroidx/collection/MutableObjectIntMap;
    iget v2, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentToken:I

    .line 493
    .local v2, "previousToken":I
    iput-object p1, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScope:Ljava/lang/Object;

    .line 494
    iget-object v3, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->scopeToValues:Landroidx/collection/MutableScatterMap;

    invoke-virtual {v3, p1}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Landroidx/collection/MutableObjectIntMap;

    iput-object v3, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScopeReads:Landroidx/collection/MutableObjectIntMap;

    .line 495
    iget v3, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentToken:I

    const/4 v4, -0x1

    if-ne v3, v4, :cond_0

    .line 496
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->currentSnapshot()Landroidx/compose/runtime/snapshots/Snapshot;

    move-result-object v3

    invoke-virtual {v3}, Landroidx/compose/runtime/snapshots/Snapshot;->getId()I

    move-result v3

    iput v3, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentToken:I

    .line 499
    :cond_0
    iget-object v3, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->derivedStateObserver:Landroidx/compose/runtime/DerivedStateObserver;

    .local v3, "observer$iv":Landroidx/compose/runtime/DerivedStateObserver;
    const/4 v4, 0x0

    .line 683
    .local v4, "$i$f$observeDerivedStateRecalculations":I
    invoke-static {}, Landroidx/compose/runtime/SnapshotStateKt;->derivedStateObservers()Landroidx/compose/runtime/collection/MutableVector;

    move-result-object v5

    .line 684
    .local v5, "observers$iv":Landroidx/compose/runtime/collection/MutableVector;
    nop

    .line 685
    :try_start_0
    invoke-virtual {v5, v3}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 686
    const/4 v6, 0x0

    .line 500
    .local v6, "$i$a$-observeDerivedStateRecalculations-SnapshotStateObserver$ObservedScopeMap$observe$1":I
    sget-object v7, Landroidx/compose/runtime/snapshots/Snapshot;->Companion:Landroidx/compose/runtime/snapshots/Snapshot$Companion;

    const/4 v8, 0x0

    invoke-virtual {v7, p2, v8, p3}, Landroidx/compose/runtime/snapshots/Snapshot$Companion;->observe(Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function0;)Ljava/lang/Object;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    .line 501
    nop

    .line 686
    .end local v6    # "$i$a$-observeDerivedStateRecalculations-SnapshotStateObserver$ObservedScopeMap$observe$1":I
    nop

    .line 688
    move-object v6, v5

    .local v6, "this_$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    const/4 v7, 0x0

    .line 689
    .local v7, "$i$f$getLastIndex":I
    invoke-virtual {v6}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v8

    add-int/lit8 v8, v8, -0x1

    .line 688
    .end local v6    # "this_$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    .end local v7    # "$i$f$getLastIndex":I
    invoke-virtual {v5, v8}, Landroidx/compose/runtime/collection/MutableVector;->removeAt(I)Ljava/lang/Object;

    .line 690
    nop

    .line 691
    nop

    .line 503
    .end local v3    # "observer$iv":Landroidx/compose/runtime/DerivedStateObserver;
    .end local v4    # "$i$f$observeDerivedStateRecalculations":I
    .end local v5    # "observers$iv":Landroidx/compose/runtime/collection/MutableVector;
    iget-object v3, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScope:Ljava/lang/Object;

    invoke-static {v3}, Lkotlin/jvm/internal/Intrinsics;->checkNotNull(Ljava/lang/Object;)V

    invoke-direct {p0, v3}, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->clearObsoleteStateReads(Ljava/lang/Object;)V

    .line 505
    iput-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScope:Ljava/lang/Object;

    .line 506
    iput-object v1, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScopeReads:Landroidx/collection/MutableObjectIntMap;

    .line 507
    iput v2, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentToken:I

    .line 508
    return-void

    .line 688
    .restart local v3    # "observer$iv":Landroidx/compose/runtime/DerivedStateObserver;
    .restart local v4    # "$i$f$observeDerivedStateRecalculations":I
    .restart local v5    # "observers$iv":Landroidx/compose/runtime/collection/MutableVector;
    :catchall_0
    move-exception v6

    move-object v7, v5

    .local v7, "this_$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    const/4 v8, 0x0

    .line 689
    .local v8, "$i$f$getLastIndex":I
    invoke-virtual {v7}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v9

    add-int/lit8 v9, v9, -0x1

    .line 688
    .end local v7    # "this_$iv$iv":Landroidx/compose/runtime/collection/MutableVector;
    .end local v8    # "$i$f$getLastIndex":I
    invoke-virtual {v5, v9}, Landroidx/compose/runtime/collection/MutableVector;->removeAt(I)Ljava/lang/Object;

    throw v6
.end method

.method public final recordInvalidation(Ljava/util/Set;)Z
    .locals 68
    .param p1, "changes"    # Ljava/util/Set;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/util/Set<",
            "+",
            "Ljava/lang/Object;",
            ">;)Z"
        }
    .end annotation

    .line 572
    move-object/from16 v0, p0

    const/4 v1, 0x0

    .line 574
    .local v1, "hasValues":Z
    iget-object v2, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->dependencyToDerivedStates:Landroidx/compose/runtime/collection/ScopeMap;

    .line 575
    .local v2, "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    iget-object v3, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->recordedDerivedStateValues:Ljava/util/HashMap;

    .line 576
    .local v3, "recordedDerivedStateValues":Ljava/util/HashMap;
    iget-object v4, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->valueToScopes:Landroidx/compose/runtime/collection/ScopeMap;

    .line 577
    .local v4, "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    iget-object v5, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->invalidated:Landroidx/collection/MutableScatterSet;

    .line 579
    .local v5, "invalidated":Landroidx/collection/MutableScatterSet;
    move-object/from16 v6, p1

    .local v6, "$this$fastForEach$iv":Ljava/util/Set;
    const/4 v7, 0x0

    .line 794
    .local v7, "$i$f$fastForEach":I
    instance-of v8, v6, Landroidx/compose/runtime/collection/IdentityArraySet;

    const-string/jumbo v9, "null cannot be cast to non-null type androidx.compose.runtime.DerivedState<kotlin.Any?>"

    const/16 v16, 0x2

    const-wide v17, -0x7f7f7f7f7f7f7f80L    # -2.937446524422997E-306

    const/16 v19, 0x1

    if-eqz v8, :cond_2b

    .line 795
    move-object v8, v6

    check-cast v8, Landroidx/compose/runtime/collection/IdentityArraySet;

    .local v8, "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    const/16 v20, 0x0

    .line 796
    .local v20, "$i$f$fastForEach":I
    nop

    .line 797
    invoke-virtual {v8}, Landroidx/compose/runtime/collection/IdentityArraySet;->getValues()[Ljava/lang/Object;

    move-result-object v21

    .line 798
    .local v21, "values$iv$iv":[Ljava/lang/Object;
    const/16 v22, 0x0

    .local v22, "i$iv$iv":I
    invoke-virtual {v8}, Landroidx/compose/runtime/collection/IdentityArraySet;->size()I

    move-result v11

    move/from16 v12, v22

    .end local v22    # "i$iv$iv":I
    .local v12, "i$iv$iv":I
    :goto_0
    if-ge v12, v11, :cond_2a

    .line 800
    aget-object v13, v21, v12

    const-string/jumbo v14, "null cannot be cast to non-null type T of androidx.compose.runtime.collection.IdentityArraySet"

    invoke-static {v13, v14}, Lkotlin/jvm/internal/Intrinsics;->checkNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .local v13, "value":Ljava/lang/Object;
    const/4 v14, 0x0

    .line 580
    .local v14, "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    instance-of v10, v13, Landroidx/compose/runtime/snapshots/StateObjectImpl;

    if-eqz v10, :cond_0

    .line 581
    move-object v10, v13

    check-cast v10, Landroidx/compose/runtime/snapshots/StateObjectImpl;

    sget-object v27, Landroidx/compose/runtime/snapshots/ReaderKind;->Companion:Landroidx/compose/runtime/snapshots/ReaderKind$Companion;

    .local v27, "this_$iv":Landroidx/compose/runtime/snapshots/ReaderKind$Companion;
    const/16 v28, 0x0

    .line 801
    .local v28, "$i$f$getSnapshotStateObserver-6f8NoZ8":I
    invoke-static/range {v16 .. v16}, Landroidx/compose/runtime/snapshots/ReaderKind;->constructor-impl(I)I

    move-result v15

    .line 581
    .end local v27    # "this_$iv":Landroidx/compose/runtime/snapshots/ReaderKind$Companion;
    .end local v28    # "$i$f$getSnapshotStateObserver-6f8NoZ8":I
    invoke-virtual {v10, v15}, Landroidx/compose/runtime/snapshots/StateObjectImpl;->isReadIn-h_f27i8$runtime_release(I)Z

    move-result v10

    if-nez v10, :cond_0

    .line 583
    move-object/from16 v58, v2

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v28, v7

    move-object/from16 v31, v8

    move-object/from16 v54, v9

    move/from16 v36, v11

    move/from16 v39, v12

    move-object v4, v3

    goto/16 :goto_1b

    .line 586
    :cond_0
    invoke-virtual {v2, v13}, Landroidx/compose/runtime/collection/ScopeMap;->contains(Ljava/lang/Object;)Z

    move-result v10

    if-eqz v10, :cond_20

    .line 588
    move-object v10, v2

    .local v10, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    const/4 v15, 0x0

    .line 802
    .local v15, "$i$f$forEachScopeOf":I
    move/from16 v27, v1

    .end local v1    # "hasValues":Z
    .local v27, "hasValues":Z
    invoke-virtual {v10}, Landroidx/compose/runtime/collection/ScopeMap;->getMap()Landroidx/collection/MutableScatterMap;

    move-result-object v1

    invoke-virtual {v1, v13}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v1

    .line 803
    .local v1, "value$iv":Ljava/lang/Object;
    if-eqz v1, :cond_1f

    .line 804
    move/from16 v28, v7

    .end local v7    # "$i$f$fastForEach":I
    .local v28, "$i$f$fastForEach":I
    instance-of v7, v1, Landroidx/collection/MutableScatterSet;

    if-eqz v7, :cond_13

    .line 806
    move-object v7, v1

    check-cast v7, Landroidx/collection/MutableScatterSet;

    check-cast v7, Landroidx/collection/ScatterSet;

    .local v7, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/16 v30, 0x0

    .line 807
    .local v30, "$i$f$forEach":I
    nop

    .line 808
    move-object/from16 v31, v8

    .end local v8    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .local v31, "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    iget-object v8, v7, Landroidx/collection/ScatterSet;->elements:[Ljava/lang/Object;

    .line 810
    .local v8, "k$iv$iv":[Ljava/lang/Object;
    move-object/from16 v32, v7

    .local v32, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    const/16 v33, 0x0

    .line 811
    .local v33, "$i$f$forEachIndex":I
    nop

    .line 812
    move-object/from16 v34, v7

    move-object/from16 v32, v10

    .end local v10    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v7, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v32, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v34, "this_$iv$iv":Landroidx/collection/ScatterSet;
    iget-object v10, v7, Landroidx/collection/ScatterSet;->metadata:[J

    .line 813
    .local v10, "m$iv$iv$iv":[J
    move-object/from16 v35, v7

    .end local v7    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v35, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    array-length v7, v10

    add-int/lit8 v7, v7, -0x2

    .line 815
    .local v7, "lastIndex$iv$iv$iv":I
    move/from16 v36, v11

    const/4 v11, 0x0

    .local v11, "i$iv$iv$iv":I
    if-gt v11, v7, :cond_10

    .line 816
    :goto_1
    aget-wide v37, v10, v11

    .line 817
    .local v37, "slot$iv$iv$iv":J
    move-wide/from16 v39, v37

    .local v39, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    const/16 v41, 0x0

    .line 818
    .local v41, "$i$f$maskEmptyOrDeleted":I
    move/from16 v42, v14

    move/from16 v43, v15

    move-wide/from16 v14, v39

    move/from16 v39, v12

    move-object/from16 v40, v13

    .end local v12    # "i$iv$iv":I
    .end local v13    # "value":Ljava/lang/Object;
    .end local v15    # "$i$f$forEachScopeOf":I
    .local v14, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .local v39, "i$iv$iv":I
    .local v40, "value":Ljava/lang/Object;
    .local v42, "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .local v43, "$i$f$forEachScopeOf":I
    not-long v12, v14

    const/16 v29, 0x7

    shl-long v12, v12, v29

    and-long/2addr v12, v14

    and-long v12, v12, v17

    .line 817
    .end local v14    # "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .end local v41    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v12, v12, v17

    if-eqz v12, :cond_f

    .line 819
    sub-int v12, v11, v7

    not-int v12, v12

    ushr-int/lit8 v12, v12, 0x1f

    const/16 v13, 0x8

    rsub-int/lit8 v12, v12, 0x8

    .line 820
    .local v12, "bitCount$iv$iv$iv":I
    const/4 v13, 0x0

    .local v13, "j$iv$iv$iv":I
    :goto_2
    if-ge v13, v12, :cond_e

    .line 821
    const-wide/16 v14, 0xff

    and-long v44, v37, v14

    .local v44, "value$iv$iv$iv$iv":J
    const/4 v14, 0x0

    .line 822
    .local v14, "$i$f$isFull":I
    const-wide/16 v23, 0x80

    cmp-long v15, v44, v23

    if-gez v15, :cond_1

    move/from16 v14, v19

    goto :goto_3

    :cond_1
    const/4 v14, 0x0

    .line 821
    .end local v14    # "$i$f$isFull":I
    .end local v44    # "value$iv$iv$iv$iv":J
    :goto_3
    if-eqz v14, :cond_d

    .line 823
    shl-int/lit8 v14, v11, 0x3

    add-int/2addr v14, v13

    .line 824
    .local v14, "index$iv$iv$iv":I
    move v15, v14

    .local v15, "index$iv$iv":I
    const/16 v41, 0x0

    .line 825
    .local v41, "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    aget-object v44, v8, v15

    move-object/from16 v45, v8

    .end local v8    # "k$iv$iv":[Ljava/lang/Object;
    .local v45, "k$iv$iv":[Ljava/lang/Object;
    move-object/from16 v8, v44

    check-cast v8, Landroidx/compose/runtime/DerivedState;

    .local v8, "derivedState":Landroidx/compose/runtime/DerivedState;
    const/16 v44, 0x0

    .line 589
    .local v44, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    invoke-static {v8, v9}, Lkotlin/jvm/internal/Intrinsics;->checkNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 590
    move-object/from16 v46, v10

    .end local v10    # "m$iv$iv$iv":[J
    .local v46, "m$iv$iv$iv":[J
    invoke-virtual {v3, v8}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v10

    .line 591
    .local v10, "previousValue":Ljava/lang/Object;
    invoke-interface {v8}, Landroidx/compose/runtime/DerivedState;->getPolicy()Landroidx/compose/runtime/SnapshotMutationPolicy;

    move-result-object v47

    if-nez v47, :cond_2

    invoke-static {}, Landroidx/compose/runtime/SnapshotStateKt;->structuralEqualityPolicy()Landroidx/compose/runtime/SnapshotMutationPolicy;

    move-result-object v47

    :cond_2
    move-object/from16 v48, v47

    .line 594
    .local v48, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    nop

    .line 595
    invoke-interface {v8}, Landroidx/compose/runtime/DerivedState;->getCurrentRecord()Landroidx/compose/runtime/DerivedState$Record;

    move-result-object v47

    move/from16 v49, v14

    .end local v14    # "index$iv$iv$iv":I
    .local v49, "index$iv$iv$iv":I
    invoke-interface/range {v47 .. v47}, Landroidx/compose/runtime/DerivedState$Record;->getCurrentValue()Ljava/lang/Object;

    move-result-object v14

    .line 596
    nop

    .line 594
    move/from16 v47, v15

    move-object/from16 v15, v48

    .end local v48    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v15, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v47, "index$iv$iv":I
    invoke-interface {v15, v14, v10}, Landroidx/compose/runtime/SnapshotMutationPolicy;->equivalent(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v14

    if-nez v14, :cond_c

    .line 599
    move-object v14, v4

    .local v14, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    const/16 v48, 0x0

    .line 826
    .local v48, "$i$f$forEachScopeOf":I
    move-object/from16 v50, v10

    .end local v10    # "previousValue":Ljava/lang/Object;
    .local v50, "previousValue":Ljava/lang/Object;
    invoke-virtual {v14}, Landroidx/compose/runtime/collection/ScopeMap;->getMap()Landroidx/collection/MutableScatterMap;

    move-result-object v10

    invoke-virtual {v10, v8}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v10

    .line 827
    .local v10, "value$iv":Ljava/lang/Object;
    if-eqz v10, :cond_b

    .line 828
    move-object/from16 v51, v14

    .end local v14    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v51, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    instance-of v14, v10, Landroidx/collection/MutableScatterSet;

    if-eqz v14, :cond_a

    .line 830
    move-object v14, v10

    check-cast v14, Landroidx/collection/MutableScatterSet;

    check-cast v14, Landroidx/collection/ScatterSet;

    .local v14, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/16 v52, 0x0

    .line 831
    .local v52, "$i$f$forEach":I
    nop

    .line 832
    move-object/from16 v53, v15

    .end local v15    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v53, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    iget-object v15, v14, Landroidx/collection/ScatterSet;->elements:[Ljava/lang/Object;

    .line 834
    .local v15, "k$iv$iv":[Ljava/lang/Object;
    move-object/from16 v54, v14

    .local v54, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    const/16 v55, 0x0

    .line 835
    .local v55, "$i$f$forEachIndex":I
    nop

    .line 836
    move-object/from16 v56, v14

    move-object/from16 v54, v9

    .end local v54    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v14, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v56, "this_$iv$iv":Landroidx/collection/ScatterSet;
    iget-object v9, v14, Landroidx/collection/ScatterSet;->metadata:[J

    .line 837
    .local v9, "m$iv$iv$iv":[J
    move-object/from16 v57, v14

    .end local v14    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v57, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    array-length v14, v9

    add-int/lit8 v14, v14, -0x2

    .line 839
    .local v14, "lastIndex$iv$iv$iv":I
    move-object/from16 v58, v2

    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .local v58, "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    const/4 v2, 0x0

    .local v2, "i$iv$iv$iv":I
    if-gt v2, v14, :cond_7

    .line 840
    :goto_4
    aget-wide v59, v9, v2

    .line 841
    .local v59, "slot$iv$iv$iv":J
    move-wide/from16 v61, v59

    .local v61, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    const/16 v63, 0x0

    .line 842
    .local v63, "$i$f$maskEmptyOrDeleted":I
    move-object/from16 v64, v3

    move-object/from16 v65, v4

    move-wide/from16 v3, v61

    move-object/from16 v61, v6

    move/from16 v62, v7

    .end local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "lastIndex$iv$iv$iv":I
    .local v3, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .local v61, "$this$fastForEach$iv":Ljava/util/Set;
    .local v62, "lastIndex$iv$iv$iv":I
    .local v64, "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v65, "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    not-long v6, v3

    const/16 v29, 0x7

    shl-long v6, v6, v29

    and-long/2addr v6, v3

    and-long v3, v6, v17

    .line 841
    .end local v3    # "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .end local v63    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v3, v3, v17

    if-eqz v3, :cond_6

    .line 843
    sub-int v3, v2, v14

    not-int v3, v3

    ushr-int/lit8 v3, v3, 0x1f

    const/16 v4, 0x8

    rsub-int/lit8 v3, v3, 0x8

    .line 844
    .local v3, "bitCount$iv$iv$iv":I
    const/4 v4, 0x0

    .local v4, "j$iv$iv$iv":I
    :goto_5
    if-ge v4, v3, :cond_5

    .line 845
    const-wide/16 v6, 0xff

    and-long v66, v59, v6

    .local v66, "value$iv$iv$iv$iv":J
    const/4 v6, 0x0

    .line 846
    .local v6, "$i$f$isFull":I
    const-wide/16 v23, 0x80

    cmp-long v7, v66, v23

    if-gez v7, :cond_3

    move/from16 v6, v19

    goto :goto_6

    :cond_3
    const/4 v6, 0x0

    .line 845
    .end local v6    # "$i$f$isFull":I
    .end local v66    # "value$iv$iv$iv$iv":J
    :goto_6
    if-eqz v6, :cond_4

    .line 847
    shl-int/lit8 v6, v2, 0x3

    add-int/2addr v6, v4

    .line 848
    .local v6, "index$iv$iv$iv":I
    move v7, v6

    .local v7, "index$iv$iv":I
    const/16 v63, 0x0

    .line 849
    .local v63, "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    move/from16 v66, v6

    .end local v6    # "index$iv$iv$iv":I
    .local v66, "index$iv$iv$iv":I
    aget-object v6, v15, v7

    .local v6, "scope":Ljava/lang/Object;
    const/16 v67, 0x0

    .line 600
    .local v67, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    invoke-virtual {v5, v6}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 601
    const/16 v27, 0x1

    .line 602
    nop

    .line 849
    .end local v6    # "scope":Ljava/lang/Object;
    .end local v67    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    nop

    .line 850
    nop

    .line 848
    .end local v7    # "index$iv$iv":I
    .end local v63    # "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    nop

    .line 851
    .end local v66    # "index$iv$iv$iv":I
    :cond_4
    const/16 v6, 0x8

    shr-long v59, v59, v6

    .line 844
    add-int/lit8 v4, v4, 0x1

    goto :goto_5

    :cond_5
    const/16 v6, 0x8

    .line 853
    .end local v4    # "j$iv$iv$iv":I
    if-ne v3, v6, :cond_9

    .line 839
    .end local v3    # "bitCount$iv$iv$iv":I
    .end local v59    # "slot$iv$iv$iv":J
    :cond_6
    if-eq v2, v14, :cond_8

    add-int/lit8 v2, v2, 0x1

    move-object/from16 v6, v61

    move/from16 v7, v62

    move-object/from16 v3, v64

    move-object/from16 v4, v65

    goto :goto_4

    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v62    # "lastIndex$iv$iv$iv":I
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .local v3, "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v4, "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .local v6, "$this$fastForEach$iv":Ljava/util/Set;
    .local v7, "lastIndex$iv$iv$iv":I
    :cond_7
    move-object/from16 v64, v3

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v62, v7

    .line 856
    .end local v2    # "i$iv$iv$iv":I
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "lastIndex$iv$iv$iv":I
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v62    # "lastIndex$iv$iv$iv":I
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    :cond_8
    nop

    .line 857
    .end local v9    # "m$iv$iv$iv":[J
    .end local v14    # "lastIndex$iv$iv$iv":I
    .end local v55    # "$i$f$forEachIndex":I
    .end local v57    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    :cond_9
    nop

    .end local v15    # "k$iv$iv":[Ljava/lang/Object;
    .end local v52    # "$i$f$forEach":I
    .end local v56    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    goto :goto_7

    .line 858
    .end local v53    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v62    # "lastIndex$iv$iv$iv":I
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .local v2, "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v7    # "lastIndex$iv$iv$iv":I
    .local v15, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    :cond_a
    move-object/from16 v58, v2

    move-object/from16 v64, v3

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v62, v7

    move-object/from16 v54, v9

    move-object/from16 v53, v15

    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "lastIndex$iv$iv$iv":I
    .end local v15    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v53    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v62    # "lastIndex$iv$iv$iv":I
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    move-object v2, v10

    .local v2, "scope":Ljava/lang/Object;
    const/4 v3, 0x0

    .line 600
    .local v3, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    invoke-virtual {v5, v2}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 601
    const/4 v4, 0x1

    .line 602
    .end local v27    # "hasValues":Z
    .local v4, "hasValues":Z
    nop

    .line 858
    .end local v2    # "scope":Ljava/lang/Object;
    .end local v3    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    move/from16 v27, v4

    goto :goto_7

    .line 827
    .end local v51    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v53    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v62    # "lastIndex$iv$iv$iv":I
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .local v2, "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .local v3, "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v4, "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v7    # "lastIndex$iv$iv$iv":I
    .local v14, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v15    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v27    # "hasValues":Z
    :cond_b
    move-object/from16 v58, v2

    move-object/from16 v64, v3

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v62, v7

    move-object/from16 v54, v9

    move-object/from16 v51, v14

    move-object/from16 v53, v15

    .line 861
    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "lastIndex$iv$iv$iv":I
    .end local v10    # "value$iv":Ljava/lang/Object;
    .end local v14    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v15    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v51    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v53    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v62    # "lastIndex$iv$iv$iv":I
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    :goto_7
    nop

    .end local v48    # "$i$f$forEachScopeOf":I
    .end local v51    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    goto :goto_8

    .line 605
    .end local v50    # "previousValue":Ljava/lang/Object;
    .end local v53    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v62    # "lastIndex$iv$iv$iv":I
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v7    # "lastIndex$iv$iv$iv":I
    .local v10, "previousValue":Ljava/lang/Object;
    .restart local v15    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    :cond_c
    move-object/from16 v58, v2

    move-object/from16 v64, v3

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v62, v7

    move-object/from16 v54, v9

    move-object/from16 v50, v10

    move-object/from16 v53, v15

    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "lastIndex$iv$iv$iv":I
    .end local v10    # "previousValue":Ljava/lang/Object;
    .end local v15    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v50    # "previousValue":Ljava/lang/Object;
    .restart local v53    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v62    # "lastIndex$iv$iv$iv":I
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    iget-object v2, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->statesToReread:Landroidx/compose/runtime/collection/MutableVector;

    invoke-virtual {v2, v8}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 607
    :goto_8
    nop

    .line 825
    .end local v8    # "derivedState":Landroidx/compose/runtime/DerivedState;
    .end local v44    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v50    # "previousValue":Ljava/lang/Object;
    .end local v53    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    nop

    .line 862
    nop

    .line 824
    .end local v41    # "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    .end local v47    # "index$iv$iv":I
    goto :goto_9

    .line 821
    .end local v45    # "k$iv$iv":[Ljava/lang/Object;
    .end local v46    # "m$iv$iv$iv":[J
    .end local v49    # "index$iv$iv$iv":I
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v62    # "lastIndex$iv$iv$iv":I
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v7    # "lastIndex$iv$iv$iv":I
    .local v8, "k$iv$iv":[Ljava/lang/Object;
    .local v10, "m$iv$iv$iv":[J
    :cond_d
    move-object/from16 v58, v2

    move-object/from16 v64, v3

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v62, v7

    move-object/from16 v45, v8

    move-object/from16 v54, v9

    move-object/from16 v46, v10

    .line 863
    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "lastIndex$iv$iv$iv":I
    .end local v8    # "k$iv$iv":[Ljava/lang/Object;
    .end local v10    # "m$iv$iv$iv":[J
    .restart local v45    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v46    # "m$iv$iv$iv":[J
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v62    # "lastIndex$iv$iv$iv":I
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    :goto_9
    const/16 v2, 0x8

    shr-long v37, v37, v2

    .line 820
    add-int/lit8 v13, v13, 0x1

    move-object/from16 v8, v45

    move-object/from16 v10, v46

    move-object/from16 v9, v54

    move-object/from16 v2, v58

    move-object/from16 v6, v61

    move/from16 v7, v62

    move-object/from16 v3, v64

    move-object/from16 v4, v65

    goto/16 :goto_2

    .end local v45    # "k$iv$iv":[Ljava/lang/Object;
    .end local v46    # "m$iv$iv$iv":[J
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v62    # "lastIndex$iv$iv$iv":I
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v7    # "lastIndex$iv$iv$iv":I
    .restart local v8    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v10    # "m$iv$iv$iv":[J
    :cond_e
    move-object/from16 v58, v2

    move-object/from16 v64, v3

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v62, v7

    move-object/from16 v45, v8

    move-object/from16 v54, v9

    move-object/from16 v46, v10

    const/16 v2, 0x8

    .line 865
    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "lastIndex$iv$iv$iv":I
    .end local v8    # "k$iv$iv":[Ljava/lang/Object;
    .end local v10    # "m$iv$iv$iv":[J
    .end local v13    # "j$iv$iv$iv":I
    .restart local v45    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v46    # "m$iv$iv$iv":[J
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v62    # "lastIndex$iv$iv$iv":I
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    if-ne v12, v2, :cond_12

    goto :goto_a

    .line 817
    .end local v12    # "bitCount$iv$iv$iv":I
    .end local v45    # "k$iv$iv":[Ljava/lang/Object;
    .end local v46    # "m$iv$iv$iv":[J
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v62    # "lastIndex$iv$iv$iv":I
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v7    # "lastIndex$iv$iv$iv":I
    .restart local v8    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v10    # "m$iv$iv$iv":[J
    :cond_f
    move-object/from16 v58, v2

    move-object/from16 v64, v3

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v62, v7

    move-object/from16 v45, v8

    move-object/from16 v54, v9

    move-object/from16 v46, v10

    .line 815
    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "lastIndex$iv$iv$iv":I
    .end local v8    # "k$iv$iv":[Ljava/lang/Object;
    .end local v10    # "m$iv$iv$iv":[J
    .end local v37    # "slot$iv$iv$iv":J
    .restart local v45    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v46    # "m$iv$iv$iv":[J
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v62    # "lastIndex$iv$iv$iv":I
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    :goto_a
    move/from16 v7, v62

    .end local v62    # "lastIndex$iv$iv$iv":I
    .restart local v7    # "lastIndex$iv$iv$iv":I
    if-eq v11, v7, :cond_11

    add-int/lit8 v11, v11, 0x1

    move/from16 v12, v39

    move-object/from16 v13, v40

    move/from16 v14, v42

    move/from16 v15, v43

    move-object/from16 v8, v45

    move-object/from16 v10, v46

    move-object/from16 v9, v54

    move-object/from16 v2, v58

    move-object/from16 v6, v61

    move-object/from16 v3, v64

    move-object/from16 v4, v65

    goto/16 :goto_1

    .end local v39    # "i$iv$iv":I
    .end local v40    # "value":Ljava/lang/Object;
    .end local v42    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v43    # "$i$f$forEachScopeOf":I
    .end local v45    # "k$iv$iv":[Ljava/lang/Object;
    .end local v46    # "m$iv$iv$iv":[J
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v8    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v10    # "m$iv$iv$iv":[J
    .local v12, "i$iv$iv":I
    .local v13, "value":Ljava/lang/Object;
    .local v14, "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .local v15, "$i$f$forEachScopeOf":I
    :cond_10
    move-object/from16 v58, v2

    move-object/from16 v64, v3

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move-object/from16 v45, v8

    move-object/from16 v54, v9

    move-object/from16 v46, v10

    move/from16 v39, v12

    move-object/from16 v40, v13

    move/from16 v42, v14

    move/from16 v43, v15

    .line 868
    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v8    # "k$iv$iv":[Ljava/lang/Object;
    .end local v10    # "m$iv$iv$iv":[J
    .end local v11    # "i$iv$iv$iv":I
    .end local v12    # "i$iv$iv":I
    .end local v13    # "value":Ljava/lang/Object;
    .end local v14    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v15    # "$i$f$forEachScopeOf":I
    .restart local v39    # "i$iv$iv":I
    .restart local v40    # "value":Ljava/lang/Object;
    .restart local v42    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v43    # "$i$f$forEachScopeOf":I
    .restart local v45    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v46    # "m$iv$iv$iv":[J
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    :cond_11
    nop

    .line 869
    .end local v7    # "lastIndex$iv$iv$iv":I
    .end local v33    # "$i$f$forEachIndex":I
    .end local v35    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .end local v46    # "m$iv$iv$iv":[J
    :cond_12
    move/from16 v1, v27

    move-object/from16 v4, v64

    .end local v30    # "$i$f$forEach":I
    .end local v34    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v45    # "k$iv$iv":[Ljava/lang/Object;
    goto/16 :goto_12

    .line 870
    .end local v31    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .end local v32    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v39    # "i$iv$iv":I
    .end local v40    # "value":Ljava/lang/Object;
    .end local v42    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v43    # "$i$f$forEachScopeOf":I
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .local v8, "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .local v10, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v12    # "i$iv$iv":I
    .restart local v13    # "value":Ljava/lang/Object;
    .restart local v14    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v15    # "$i$f$forEachScopeOf":I
    :cond_13
    move-object/from16 v58, v2

    move-object/from16 v64, v3

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move-object/from16 v31, v8

    move-object/from16 v54, v9

    move-object/from16 v32, v10

    move/from16 v36, v11

    move/from16 v39, v12

    move-object/from16 v40, v13

    move/from16 v42, v14

    move/from16 v43, v15

    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v4    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v8    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .end local v10    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v12    # "i$iv$iv":I
    .end local v13    # "value":Ljava/lang/Object;
    .end local v14    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v15    # "$i$f$forEachScopeOf":I
    .restart local v31    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .restart local v32    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v39    # "i$iv$iv":I
    .restart local v40    # "value":Ljava/lang/Object;
    .restart local v42    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v43    # "$i$f$forEachScopeOf":I
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    move-object v2, v1

    check-cast v2, Landroidx/compose/runtime/DerivedState;

    .local v2, "derivedState":Landroidx/compose/runtime/DerivedState;
    const/4 v3, 0x0

    .line 589
    .local v3, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    nop

    .line 590
    move-object/from16 v4, v64

    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v4, "recordedDerivedStateValues":Ljava/util/HashMap;
    invoke-virtual {v4, v2}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v6

    .line 591
    .local v6, "previousValue":Ljava/lang/Object;
    invoke-interface {v2}, Landroidx/compose/runtime/DerivedState;->getPolicy()Landroidx/compose/runtime/SnapshotMutationPolicy;

    move-result-object v7

    if-nez v7, :cond_14

    invoke-static {}, Landroidx/compose/runtime/SnapshotStateKt;->structuralEqualityPolicy()Landroidx/compose/runtime/SnapshotMutationPolicy;

    move-result-object v7

    .line 594
    .local v7, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    :cond_14
    nop

    .line 595
    invoke-interface {v2}, Landroidx/compose/runtime/DerivedState;->getCurrentRecord()Landroidx/compose/runtime/DerivedState$Record;

    move-result-object v8

    invoke-interface {v8}, Landroidx/compose/runtime/DerivedState$Record;->getCurrentValue()Ljava/lang/Object;

    move-result-object v8

    .line 596
    nop

    .line 594
    invoke-interface {v7, v8, v6}, Landroidx/compose/runtime/SnapshotMutationPolicy;->equivalent(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v8

    if-nez v8, :cond_1e

    .line 599
    move-object/from16 v8, v65

    .local v8, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    const/4 v9, 0x0

    .line 871
    .local v9, "$i$f$forEachScopeOf":I
    invoke-virtual {v8}, Landroidx/compose/runtime/collection/ScopeMap;->getMap()Landroidx/collection/MutableScatterMap;

    move-result-object v10

    invoke-virtual {v10, v2}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v10

    .line 872
    .local v10, "value$iv":Ljava/lang/Object;
    if-eqz v10, :cond_1d

    .line 873
    instance-of v11, v10, Landroidx/collection/MutableScatterSet;

    if-eqz v11, :cond_1c

    .line 875
    move-object v11, v10

    check-cast v11, Landroidx/collection/MutableScatterSet;

    check-cast v11, Landroidx/collection/ScatterSet;

    .local v11, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v12, 0x0

    .line 876
    .local v12, "$i$f$forEach":I
    nop

    .line 877
    iget-object v13, v11, Landroidx/collection/ScatterSet;->elements:[Ljava/lang/Object;

    .line 879
    .local v13, "k$iv$iv":[Ljava/lang/Object;
    move-object v14, v11

    .local v14, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v15, 0x0

    .line 880
    .local v15, "$i$f$forEachIndex":I
    nop

    .line 881
    move-object/from16 v30, v1

    .end local v1    # "value$iv":Ljava/lang/Object;
    .local v30, "value$iv":Ljava/lang/Object;
    iget-object v1, v14, Landroidx/collection/ScatterSet;->metadata:[J

    .line 882
    .local v1, "m$iv$iv$iv":[J
    move/from16 v33, v3

    .end local v3    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .local v33, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    array-length v3, v1

    add-int/lit8 v3, v3, -0x2

    .line 884
    .local v3, "lastIndex$iv$iv$iv":I
    move-object/from16 v34, v6

    .end local v6    # "previousValue":Ljava/lang/Object;
    .local v34, "previousValue":Ljava/lang/Object;
    const/4 v6, 0x0

    .local v6, "i$iv$iv$iv":I
    if-gt v6, v3, :cond_19

    .line 885
    :goto_b
    aget-wide v37, v1, v6

    .line 886
    .restart local v37    # "slot$iv$iv$iv":J
    move-wide/from16 v44, v37

    .local v44, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    const/16 v35, 0x0

    .line 887
    .local v35, "$i$f$maskEmptyOrDeleted":I
    move-object/from16 v41, v7

    move-object/from16 v46, v8

    move-wide/from16 v7, v44

    move-object/from16 v44, v11

    move/from16 v45, v12

    .end local v8    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v11    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v12    # "$i$f$forEach":I
    .local v7, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .local v41, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v44, "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v45, "$i$f$forEach":I
    .local v46, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    not-long v11, v7

    const/16 v29, 0x7

    shl-long v11, v11, v29

    and-long/2addr v11, v7

    and-long v7, v11, v17

    .line 886
    .end local v7    # "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .end local v35    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v7, v7, v17

    if-eqz v7, :cond_18

    .line 888
    sub-int v7, v6, v3

    not-int v7, v7

    ushr-int/lit8 v7, v7, 0x1f

    const/16 v8, 0x8

    rsub-int/lit8 v7, v7, 0x8

    .line 889
    .local v7, "bitCount$iv$iv$iv":I
    const/4 v8, 0x0

    .local v8, "j$iv$iv$iv":I
    :goto_c
    if-ge v8, v7, :cond_17

    .line 890
    const-wide/16 v11, 0xff

    and-long v47, v37, v11

    .local v47, "value$iv$iv$iv$iv":J
    const/4 v11, 0x0

    .line 891
    .local v11, "$i$f$isFull":I
    const-wide/16 v23, 0x80

    cmp-long v12, v47, v23

    if-gez v12, :cond_15

    move/from16 v11, v19

    goto :goto_d

    :cond_15
    const/4 v11, 0x0

    .line 890
    .end local v11    # "$i$f$isFull":I
    .end local v47    # "value$iv$iv$iv$iv":J
    :goto_d
    if-eqz v11, :cond_16

    .line 892
    shl-int/lit8 v11, v6, 0x3

    add-int/2addr v11, v8

    .line 893
    .local v11, "index$iv$iv$iv":I
    move v12, v11

    .local v12, "index$iv$iv":I
    const/16 v35, 0x0

    .line 894
    .local v35, "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    move-object/from16 v47, v1

    .end local v1    # "m$iv$iv$iv":[J
    .local v47, "m$iv$iv$iv":[J
    aget-object v1, v13, v12

    .local v1, "scope":Ljava/lang/Object;
    const/16 v48, 0x0

    .line 600
    .local v48, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    invoke-virtual {v5, v1}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 601
    const/16 v27, 0x1

    .line 602
    nop

    .line 894
    .end local v1    # "scope":Ljava/lang/Object;
    .end local v48    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    nop

    .line 895
    nop

    .line 893
    .end local v12    # "index$iv$iv":I
    .end local v35    # "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    goto :goto_e

    .line 890
    .end local v11    # "index$iv$iv$iv":I
    .end local v47    # "m$iv$iv$iv":[J
    .local v1, "m$iv$iv$iv":[J
    :cond_16
    move-object/from16 v47, v1

    .line 896
    .end local v1    # "m$iv$iv$iv":[J
    .restart local v47    # "m$iv$iv$iv":[J
    :goto_e
    const/16 v1, 0x8

    shr-long v37, v37, v1

    .line 889
    add-int/lit8 v8, v8, 0x1

    move-object/from16 v1, v47

    goto :goto_c

    .end local v47    # "m$iv$iv$iv":[J
    .restart local v1    # "m$iv$iv$iv":[J
    :cond_17
    move-object/from16 v47, v1

    const/16 v1, 0x8

    .line 898
    .end local v1    # "m$iv$iv$iv":[J
    .end local v8    # "j$iv$iv$iv":I
    .restart local v47    # "m$iv$iv$iv":[J
    if-ne v7, v1, :cond_1b

    goto :goto_f

    .line 886
    .end local v7    # "bitCount$iv$iv$iv":I
    .end local v47    # "m$iv$iv$iv":[J
    .restart local v1    # "m$iv$iv$iv":[J
    :cond_18
    move-object/from16 v47, v1

    .line 884
    .end local v1    # "m$iv$iv$iv":[J
    .end local v37    # "slot$iv$iv$iv":J
    .restart local v47    # "m$iv$iv$iv":[J
    :goto_f
    if-eq v6, v3, :cond_1a

    add-int/lit8 v6, v6, 0x1

    move-object/from16 v7, v41

    move-object/from16 v11, v44

    move/from16 v12, v45

    move-object/from16 v8, v46

    move-object/from16 v1, v47

    goto :goto_b

    .end local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v44    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v45    # "$i$f$forEach":I
    .end local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v47    # "m$iv$iv$iv":[J
    .restart local v1    # "m$iv$iv$iv":[J
    .local v7, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v8, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v11, "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v12, "$i$f$forEach":I
    :cond_19
    move-object/from16 v47, v1

    move-object/from16 v41, v7

    move-object/from16 v46, v8

    move-object/from16 v44, v11

    move/from16 v45, v12

    .end local v1    # "m$iv$iv$iv":[J
    .end local v7    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v8    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v11    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v12    # "$i$f$forEach":I
    .restart local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v44    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v45    # "$i$f$forEach":I
    .restart local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v47    # "m$iv$iv$iv":[J
    :cond_1a
    move/from16 v1, v27

    .line 901
    .end local v6    # "i$iv$iv$iv":I
    .end local v27    # "hasValues":Z
    .local v1, "hasValues":Z
    move/from16 v27, v1

    .line 902
    .end local v1    # "hasValues":Z
    .end local v3    # "lastIndex$iv$iv$iv":I
    .end local v14    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .end local v15    # "$i$f$forEachIndex":I
    .end local v47    # "m$iv$iv$iv":[J
    .restart local v27    # "hasValues":Z
    :cond_1b
    move/from16 v1, v27

    .end local v13    # "k$iv$iv":[Ljava/lang/Object;
    .end local v44    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v45    # "$i$f$forEach":I
    goto :goto_10

    .line 903
    .end local v30    # "value$iv":Ljava/lang/Object;
    .end local v33    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v34    # "previousValue":Ljava/lang/Object;
    .end local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v1, "value$iv":Ljava/lang/Object;
    .local v3, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .local v6, "previousValue":Ljava/lang/Object;
    .restart local v7    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v8    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    :cond_1c
    move-object/from16 v30, v1

    move/from16 v33, v3

    move-object/from16 v34, v6

    move-object/from16 v41, v7

    move-object/from16 v46, v8

    .end local v1    # "value$iv":Ljava/lang/Object;
    .end local v3    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v6    # "previousValue":Ljava/lang/Object;
    .end local v7    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v8    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v30    # "value$iv":Ljava/lang/Object;
    .restart local v33    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .restart local v34    # "previousValue":Ljava/lang/Object;
    .restart local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    move-object v1, v10

    .local v1, "scope":Ljava/lang/Object;
    const/4 v3, 0x0

    .line 600
    .local v3, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    invoke-virtual {v5, v1}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 601
    const/4 v6, 0x1

    .line 602
    .end local v27    # "hasValues":Z
    .local v6, "hasValues":Z
    nop

    .line 903
    .end local v1    # "scope":Ljava/lang/Object;
    .end local v3    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    move v1, v6

    goto :goto_10

    .line 872
    .end local v30    # "value$iv":Ljava/lang/Object;
    .end local v33    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v34    # "previousValue":Ljava/lang/Object;
    .end local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v1, "value$iv":Ljava/lang/Object;
    .local v3, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .local v6, "previousValue":Ljava/lang/Object;
    .restart local v7    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v8    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v27    # "hasValues":Z
    :cond_1d
    move-object/from16 v30, v1

    move/from16 v33, v3

    move-object/from16 v34, v6

    move-object/from16 v41, v7

    move-object/from16 v46, v8

    .end local v1    # "value$iv":Ljava/lang/Object;
    .end local v3    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v6    # "previousValue":Ljava/lang/Object;
    .end local v7    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v8    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v30    # "value$iv":Ljava/lang/Object;
    .restart local v33    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .restart local v34    # "previousValue":Ljava/lang/Object;
    .restart local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    move/from16 v1, v27

    .line 906
    .end local v10    # "value$iv":Ljava/lang/Object;
    .end local v27    # "hasValues":Z
    .local v1, "hasValues":Z
    :goto_10
    nop

    .end local v9    # "$i$f$forEachScopeOf":I
    .end local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    goto :goto_11

    .line 605
    .end local v30    # "value$iv":Ljava/lang/Object;
    .end local v33    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v34    # "previousValue":Ljava/lang/Object;
    .end local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v1, "value$iv":Ljava/lang/Object;
    .restart local v3    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .restart local v6    # "previousValue":Ljava/lang/Object;
    .restart local v7    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v27    # "hasValues":Z
    :cond_1e
    move-object/from16 v30, v1

    move/from16 v33, v3

    move-object/from16 v34, v6

    move-object/from16 v41, v7

    .end local v1    # "value$iv":Ljava/lang/Object;
    .end local v3    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v6    # "previousValue":Ljava/lang/Object;
    .end local v7    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v30    # "value$iv":Ljava/lang/Object;
    .restart local v33    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .restart local v34    # "previousValue":Ljava/lang/Object;
    .restart local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    iget-object v1, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->statesToReread:Landroidx/compose/runtime/collection/MutableVector;

    invoke-virtual {v1, v2}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    move/from16 v1, v27

    .line 607
    .end local v27    # "hasValues":Z
    .local v1, "hasValues":Z
    :goto_11
    nop

    .line 870
    .end local v2    # "derivedState":Landroidx/compose/runtime/DerivedState;
    .end local v33    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v34    # "previousValue":Ljava/lang/Object;
    .end local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    goto :goto_12

    .line 803
    .end local v28    # "$i$f$fastForEach":I
    .end local v30    # "value$iv":Ljava/lang/Object;
    .end local v31    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .end local v32    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v39    # "i$iv$iv":I
    .end local v40    # "value":Ljava/lang/Object;
    .end local v42    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v43    # "$i$f$forEachScopeOf":I
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .local v1, "value$iv":Ljava/lang/Object;
    .local v2, "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .local v3, "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v4, "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .local v6, "$this$fastForEach$iv":Ljava/util/Set;
    .local v7, "$i$f$fastForEach":I
    .local v8, "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .local v10, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v12, "i$iv$iv":I
    .local v13, "value":Ljava/lang/Object;
    .local v14, "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .local v15, "$i$f$forEachScopeOf":I
    .restart local v27    # "hasValues":Z
    :cond_1f
    move-object/from16 v30, v1

    move-object/from16 v58, v2

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v28, v7

    move-object/from16 v31, v8

    move-object/from16 v54, v9

    move-object/from16 v32, v10

    move/from16 v36, v11

    move/from16 v39, v12

    move-object/from16 v40, v13

    move/from16 v42, v14

    move/from16 v43, v15

    move-object v4, v3

    .end local v1    # "value$iv":Ljava/lang/Object;
    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "$i$f$fastForEach":I
    .end local v8    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .end local v10    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v12    # "i$iv$iv":I
    .end local v13    # "value":Ljava/lang/Object;
    .end local v14    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v15    # "$i$f$forEachScopeOf":I
    .local v4, "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v28    # "$i$f$fastForEach":I
    .restart local v30    # "value$iv":Ljava/lang/Object;
    .restart local v31    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .restart local v32    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v39    # "i$iv$iv":I
    .restart local v40    # "value":Ljava/lang/Object;
    .restart local v42    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v43    # "$i$f$forEachScopeOf":I
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    move/from16 v1, v27

    .line 907
    .end local v27    # "hasValues":Z
    .end local v30    # "value$iv":Ljava/lang/Object;
    .local v1, "hasValues":Z
    :goto_12
    goto :goto_13

    .line 586
    .end local v28    # "$i$f$fastForEach":I
    .end local v31    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .end local v32    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v39    # "i$iv$iv":I
    .end local v40    # "value":Ljava/lang/Object;
    .end local v42    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v43    # "$i$f$forEachScopeOf":I
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v4, "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v7    # "$i$f$fastForEach":I
    .restart local v8    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .restart local v12    # "i$iv$iv":I
    .restart local v13    # "value":Ljava/lang/Object;
    .restart local v14    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    :cond_20
    move/from16 v27, v1

    move-object/from16 v58, v2

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v28, v7

    move-object/from16 v31, v8

    move-object/from16 v54, v9

    move/from16 v36, v11

    move/from16 v39, v12

    move-object/from16 v40, v13

    move/from16 v42, v14

    move-object v4, v3

    .line 610
    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "$i$f$fastForEach":I
    .end local v8    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .end local v12    # "i$iv$iv":I
    .end local v13    # "value":Ljava/lang/Object;
    .end local v14    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .local v4, "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v28    # "$i$f$fastForEach":I
    .restart local v31    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .restart local v39    # "i$iv$iv":I
    .restart local v40    # "value":Ljava/lang/Object;
    .restart local v42    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    :goto_13
    move-object/from16 v2, v65

    .local v2, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    const/4 v3, 0x0

    .line 908
    .local v3, "$i$f$forEachScopeOf":I
    invoke-virtual {v2}, Landroidx/compose/runtime/collection/ScopeMap;->getMap()Landroidx/collection/MutableScatterMap;

    move-result-object v6

    move-object/from16 v7, v40

    .end local v40    # "value":Ljava/lang/Object;
    .local v7, "value":Ljava/lang/Object;
    invoke-virtual {v6, v7}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v6

    .line 909
    .local v6, "value$iv":Ljava/lang/Object;
    if-eqz v6, :cond_29

    .line 910
    instance-of v8, v6, Landroidx/collection/MutableScatterSet;

    if-eqz v8, :cond_28

    .line 912
    move-object v8, v6

    check-cast v8, Landroidx/collection/MutableScatterSet;

    check-cast v8, Landroidx/collection/ScatterSet;

    .local v8, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v9, 0x0

    .line 913
    .local v9, "$i$f$forEach":I
    nop

    .line 914
    iget-object v10, v8, Landroidx/collection/ScatterSet;->elements:[Ljava/lang/Object;

    .line 916
    .local v10, "k$iv$iv":[Ljava/lang/Object;
    move-object v11, v8

    .local v11, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v12, 0x0

    .line 917
    .local v12, "$i$f$forEachIndex":I
    nop

    .line 918
    iget-object v13, v11, Landroidx/collection/ScatterSet;->metadata:[J

    .line 919
    .local v13, "m$iv$iv$iv":[J
    array-length v14, v13

    add-int/lit8 v14, v14, -0x2

    .line 921
    .local v14, "lastIndex$iv$iv$iv":I
    const/4 v15, 0x0

    .local v15, "i$iv$iv$iv":I
    if-gt v15, v14, :cond_26

    .line 922
    :goto_14
    aget-wide v32, v13, v15

    .line 923
    .local v32, "slot$iv$iv$iv":J
    move-wide/from16 v34, v32

    .local v34, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    const/16 v27, 0x0

    .line 924
    .local v27, "$i$f$maskEmptyOrDeleted":I
    move/from16 v37, v1

    move-object/from16 v30, v2

    move-object/from16 v40, v7

    move-wide/from16 v1, v34

    move-object/from16 v34, v8

    .end local v2    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v7    # "value":Ljava/lang/Object;
    .end local v8    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v1, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .local v30, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v34, "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v37, "hasValues":Z
    .restart local v40    # "value":Ljava/lang/Object;
    not-long v7, v1

    const/16 v29, 0x7

    shl-long v7, v7, v29

    and-long/2addr v7, v1

    and-long v1, v7, v17

    .line 923
    .end local v1    # "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .end local v27    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v1, v1, v17

    if-eqz v1, :cond_25

    .line 925
    sub-int v1, v15, v14

    not-int v1, v1

    ushr-int/lit8 v1, v1, 0x1f

    const/16 v2, 0x8

    rsub-int/lit8 v1, v1, 0x8

    .line 926
    .local v1, "bitCount$iv$iv$iv":I
    const/4 v2, 0x0

    .local v2, "j$iv$iv$iv":I
    :goto_15
    if-ge v2, v1, :cond_23

    .line 927
    const-wide/16 v7, 0xff

    and-long v43, v32, v7

    .local v43, "value$iv$iv$iv$iv":J
    const/4 v7, 0x0

    .line 928
    .local v7, "$i$f$isFull":I
    const-wide/16 v23, 0x80

    cmp-long v8, v43, v23

    if-gez v8, :cond_21

    move/from16 v7, v19

    goto :goto_16

    :cond_21
    const/4 v7, 0x0

    .line 927
    .end local v7    # "$i$f$isFull":I
    .end local v43    # "value$iv$iv$iv$iv":J
    :goto_16
    if-eqz v7, :cond_22

    .line 929
    shl-int/lit8 v7, v15, 0x3

    add-int/2addr v7, v2

    .line 930
    .local v7, "index$iv$iv$iv":I
    move v8, v7

    .local v8, "index$iv$iv":I
    const/16 v27, 0x0

    .line 931
    .local v27, "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    move/from16 v35, v3

    .end local v3    # "$i$f$forEachScopeOf":I
    .local v35, "$i$f$forEachScopeOf":I
    aget-object v3, v10, v8

    .local v3, "scope":Ljava/lang/Object;
    const/16 v38, 0x0

    .line 611
    .local v38, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$2":I
    invoke-virtual {v5, v3}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 612
    const/16 v37, 0x1

    .line 613
    nop

    .line 931
    .end local v3    # "scope":Ljava/lang/Object;
    .end local v38    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$2":I
    nop

    .line 932
    nop

    .line 930
    .end local v8    # "index$iv$iv":I
    .end local v27    # "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    goto :goto_17

    .line 927
    .end local v7    # "index$iv$iv$iv":I
    .end local v35    # "$i$f$forEachScopeOf":I
    .local v3, "$i$f$forEachScopeOf":I
    :cond_22
    move/from16 v35, v3

    .line 933
    .end local v3    # "$i$f$forEachScopeOf":I
    .restart local v35    # "$i$f$forEachScopeOf":I
    :goto_17
    const/16 v3, 0x8

    shr-long v32, v32, v3

    .line 926
    add-int/lit8 v2, v2, 0x1

    move/from16 v3, v35

    goto :goto_15

    .end local v35    # "$i$f$forEachScopeOf":I
    .restart local v3    # "$i$f$forEachScopeOf":I
    :cond_23
    move/from16 v35, v3

    const/16 v3, 0x8

    .line 935
    .end local v2    # "j$iv$iv$iv":I
    .end local v3    # "$i$f$forEachScopeOf":I
    .restart local v35    # "$i$f$forEachScopeOf":I
    if-ne v1, v3, :cond_24

    move/from16 v1, v37

    goto :goto_18

    :cond_24
    move/from16 v1, v37

    goto :goto_19

    .line 923
    .end local v1    # "bitCount$iv$iv$iv":I
    .end local v35    # "$i$f$forEachScopeOf":I
    .restart local v3    # "$i$f$forEachScopeOf":I
    :cond_25
    move/from16 v35, v3

    .end local v3    # "$i$f$forEachScopeOf":I
    .restart local v35    # "$i$f$forEachScopeOf":I
    move/from16 v1, v37

    .line 921
    .end local v32    # "slot$iv$iv$iv":J
    .end local v37    # "hasValues":Z
    .local v1, "hasValues":Z
    :goto_18
    if-eq v15, v14, :cond_27

    add-int/lit8 v15, v15, 0x1

    move-object/from16 v2, v30

    move-object/from16 v8, v34

    move/from16 v3, v35

    move-object/from16 v7, v40

    goto :goto_14

    .end local v30    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v34    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v35    # "$i$f$forEachScopeOf":I
    .end local v40    # "value":Ljava/lang/Object;
    .local v2, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "$i$f$forEachScopeOf":I
    .local v7, "value":Ljava/lang/Object;
    .local v8, "this_$iv$iv":Landroidx/collection/ScatterSet;
    :cond_26
    move-object/from16 v30, v2

    move/from16 v35, v3

    move-object/from16 v40, v7

    move-object/from16 v34, v8

    .line 938
    .end local v2    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "$i$f$forEachScopeOf":I
    .end local v7    # "value":Ljava/lang/Object;
    .end local v8    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v15    # "i$iv$iv$iv":I
    .restart local v30    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v34    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v35    # "$i$f$forEachScopeOf":I
    .restart local v40    # "value":Ljava/lang/Object;
    :cond_27
    nop

    .line 939
    .end local v11    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .end local v12    # "$i$f$forEachIndex":I
    .end local v13    # "m$iv$iv$iv":[J
    .end local v14    # "lastIndex$iv$iv$iv":I
    :goto_19
    nop

    .end local v9    # "$i$f$forEach":I
    .end local v10    # "k$iv$iv":[Ljava/lang/Object;
    .end local v34    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    goto :goto_1a

    .line 940
    .end local v30    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v35    # "$i$f$forEachScopeOf":I
    .end local v40    # "value":Ljava/lang/Object;
    .restart local v2    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "$i$f$forEachScopeOf":I
    .restart local v7    # "value":Ljava/lang/Object;
    :cond_28
    move-object/from16 v30, v2

    move/from16 v35, v3

    move-object/from16 v40, v7

    .end local v2    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "$i$f$forEachScopeOf":I
    .end local v7    # "value":Ljava/lang/Object;
    .restart local v30    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v35    # "$i$f$forEachScopeOf":I
    .restart local v40    # "value":Ljava/lang/Object;
    move-object v2, v6

    .local v2, "scope":Ljava/lang/Object;
    const/4 v3, 0x0

    .line 611
    .local v3, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$2":I
    invoke-virtual {v5, v2}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 612
    const/4 v1, 0x1

    .line 613
    nop

    .line 940
    .end local v2    # "scope":Ljava/lang/Object;
    .end local v3    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$2":I
    goto :goto_1a

    .line 909
    .end local v30    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v35    # "$i$f$forEachScopeOf":I
    .end local v40    # "value":Ljava/lang/Object;
    .local v2, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v3, "$i$f$forEachScopeOf":I
    .restart local v7    # "value":Ljava/lang/Object;
    :cond_29
    move-object/from16 v30, v2

    move/from16 v35, v3

    move-object/from16 v40, v7

    .line 943
    .end local v2    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "$i$f$forEachScopeOf":I
    .end local v6    # "value$iv":Ljava/lang/Object;
    .end local v7    # "value":Ljava/lang/Object;
    .restart local v30    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v35    # "$i$f$forEachScopeOf":I
    .restart local v40    # "value":Ljava/lang/Object;
    :goto_1a
    nop

    .line 614
    .end local v30    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v35    # "$i$f$forEachScopeOf":I
    nop

    .line 800
    .end local v40    # "value":Ljava/lang/Object;
    .end local v42    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    :goto_1b
    nop

    .line 798
    add-int/lit8 v12, v39, 0x1

    move-object v3, v4

    move/from16 v7, v28

    move-object/from16 v8, v31

    move/from16 v11, v36

    move-object/from16 v9, v54

    move-object/from16 v2, v58

    move-object/from16 v6, v61

    move-object/from16 v4, v65

    .end local v39    # "i$iv$iv":I
    .local v12, "i$iv$iv":I
    goto/16 :goto_0

    .end local v28    # "$i$f$fastForEach":I
    .end local v31    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .local v2, "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .local v3, "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v4, "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .local v6, "$this$fastForEach$iv":Ljava/util/Set;
    .local v7, "$i$f$fastForEach":I
    .local v8, "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    :cond_2a
    move/from16 v27, v1

    move-object/from16 v58, v2

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v28, v7

    move-object/from16 v31, v8

    move/from16 v39, v12

    move-object v4, v3

    .line 944
    .end local v1    # "hasValues":Z
    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "$i$f$fastForEach":I
    .end local v8    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .end local v12    # "i$iv$iv":I
    .local v4, "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v27, "hasValues":Z
    .restart local v28    # "$i$f$fastForEach":I
    .restart local v31    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    move-object/from16 v64, v4

    .end local v20    # "$i$f$fastForEach":I
    .end local v21    # "values$iv$iv":[Ljava/lang/Object;
    .end local v31    # "this_$iv$iv":Landroidx/compose/runtime/collection/IdentityArraySet;
    goto/16 :goto_37

    .line 945
    .end local v27    # "hasValues":Z
    .end local v28    # "$i$f$fastForEach":I
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v1    # "hasValues":Z
    .restart local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v4, "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v7    # "$i$f$fastForEach":I
    :cond_2b
    move-object/from16 v58, v2

    move-object/from16 v65, v4

    move-object/from16 v61, v6

    move/from16 v28, v7

    move-object/from16 v54, v9

    move-object v4, v3

    .end local v2    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v3    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v6    # "$this$fastForEach$iv":Ljava/util/Set;
    .end local v7    # "$i$f$fastForEach":I
    .local v4, "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v28    # "$i$f$fastForEach":I
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    .restart local v65    # "valueToScopes":Landroidx/compose/runtime/collection/ScopeMap;
    move-object/from16 v2, v61

    check-cast v2, Ljava/lang/Iterable;

    .local v2, "$this$forEach$iv$iv":Ljava/lang/Iterable;
    const/4 v3, 0x0

    .line 946
    .local v3, "$i$f$forEach":I
    invoke-interface {v2}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v6

    :goto_1c
    invoke-interface {v6}, Ljava/util/Iterator;->hasNext()Z

    move-result v7

    if-eqz v7, :cond_56

    invoke-interface {v6}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v7

    .local v7, "element$iv$iv":Ljava/lang/Object;
    move-object v8, v7

    .local v8, "value":Ljava/lang/Object;
    const/4 v9, 0x0

    .line 580
    .local v9, "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    instance-of v10, v8, Landroidx/compose/runtime/snapshots/StateObjectImpl;

    if-eqz v10, :cond_2c

    .line 581
    move-object v10, v8

    check-cast v10, Landroidx/compose/runtime/snapshots/StateObjectImpl;

    sget-object v11, Landroidx/compose/runtime/snapshots/ReaderKind;->Companion:Landroidx/compose/runtime/snapshots/ReaderKind$Companion;

    .local v11, "this_$iv":Landroidx/compose/runtime/snapshots/ReaderKind$Companion;
    const/4 v12, 0x0

    .line 801
    .local v12, "$i$f$getSnapshotStateObserver-6f8NoZ8":I
    invoke-static/range {v16 .. v16}, Landroidx/compose/runtime/snapshots/ReaderKind;->constructor-impl(I)I

    move-result v11

    .line 581
    .end local v11    # "this_$iv":Landroidx/compose/runtime/snapshots/ReaderKind$Companion;
    .end local v12    # "$i$f$getSnapshotStateObserver-6f8NoZ8":I
    invoke-virtual {v10, v11}, Landroidx/compose/runtime/snapshots/StateObjectImpl;->isReadIn-h_f27i8$runtime_release(I)Z

    move-result v10

    if-nez v10, :cond_2c

    .line 583
    move-object/from16 v30, v2

    move/from16 v21, v3

    move-object/from16 v64, v4

    move-object/from16 v32, v6

    move-object/from16 v44, v7

    const/16 v6, 0x8

    const-wide/16 v23, 0x80

    const/16 v29, 0x7

    goto/16 :goto_36

    .line 586
    :cond_2c
    move-object/from16 v10, v58

    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .local v10, "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    invoke-virtual {v10, v8}, Landroidx/compose/runtime/collection/ScopeMap;->contains(Ljava/lang/Object;)Z

    move-result v11

    if-eqz v11, :cond_4c

    .line 588
    move-object v11, v10

    .local v11, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    const/4 v12, 0x0

    .line 802
    .local v12, "$i$f$forEachScopeOf":I
    invoke-virtual {v11}, Landroidx/compose/runtime/collection/ScopeMap;->getMap()Landroidx/collection/MutableScatterMap;

    move-result-object v13

    invoke-virtual {v13, v8}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v13

    .line 803
    .local v13, "value$iv":Ljava/lang/Object;
    if-eqz v13, :cond_4b

    .line 804
    instance-of v14, v13, Landroidx/collection/MutableScatterSet;

    if-eqz v14, :cond_3f

    .line 806
    move-object v14, v13

    check-cast v14, Landroidx/collection/MutableScatterSet;

    check-cast v14, Landroidx/collection/ScatterSet;

    .local v14, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v15, 0x0

    .line 807
    .local v15, "$i$f$forEach":I
    nop

    .line 808
    move/from16 v20, v1

    .end local v1    # "hasValues":Z
    .local v20, "hasValues":Z
    iget-object v1, v14, Landroidx/collection/ScatterSet;->elements:[Ljava/lang/Object;

    .line 810
    .local v1, "k$iv$iv":[Ljava/lang/Object;
    move-object/from16 v21, v14

    .local v21, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    const/16 v27, 0x0

    .line 947
    .local v27, "$i$f$forEachIndex":I
    nop

    .line 948
    move-object/from16 v30, v2

    move-object/from16 v2, v21

    move/from16 v21, v3

    .end local v3    # "$i$f$forEach":I
    .local v2, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v21, "$i$f$forEach":I
    .local v30, "$this$forEach$iv$iv":Ljava/lang/Iterable;
    iget-object v3, v2, Landroidx/collection/ScatterSet;->metadata:[J

    .line 949
    .local v3, "m$iv$iv$iv":[J
    move-object/from16 v31, v2

    .end local v2    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v31, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    array-length v2, v3

    add-int/lit8 v2, v2, -0x2

    .line 951
    .local v2, "lastIndex$iv$iv$iv":I
    move-object/from16 v32, v6

    const/4 v6, 0x0

    .local v6, "i$iv$iv$iv":I
    if-gt v6, v2, :cond_3c

    .line 952
    :goto_1d
    aget-wide v33, v3, v6

    .line 953
    .local v33, "slot$iv$iv$iv":J
    move-wide/from16 v35, v33

    .local v35, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    const/16 v37, 0x0

    .line 818
    .local v37, "$i$f$maskEmptyOrDeleted":I
    move/from16 v38, v9

    move-object/from16 v58, v10

    move-wide/from16 v9, v35

    move-object/from16 v35, v11

    move/from16 v36, v12

    .end local v10    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v11    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v12    # "$i$f$forEachScopeOf":I
    .local v9, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .local v35, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v36, "$i$f$forEachScopeOf":I
    .local v38, "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    not-long v11, v9

    const/16 v29, 0x7

    shl-long v11, v11, v29

    and-long/2addr v11, v9

    and-long v9, v11, v17

    .line 953
    .end local v9    # "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .end local v37    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v9, v9, v17

    if-eqz v9, :cond_3b

    .line 958
    sub-int v9, v6, v2

    not-int v9, v9

    ushr-int/lit8 v9, v9, 0x1f

    const/16 v10, 0x8

    rsub-int/lit8 v9, v9, 0x8

    .line 959
    .local v9, "bitCount$iv$iv$iv":I
    const/4 v10, 0x0

    .local v10, "j$iv$iv$iv":I
    :goto_1e
    if-ge v10, v9, :cond_3a

    .line 960
    const-wide/16 v11, 0xff

    and-long v39, v33, v11

    .local v39, "value$iv$iv$iv$iv":J
    const/4 v11, 0x0

    .line 961
    .local v11, "$i$f$isFull":I
    const-wide/16 v23, 0x80

    cmp-long v12, v39, v23

    if-gez v12, :cond_2d

    move/from16 v11, v19

    goto :goto_1f

    :cond_2d
    const/4 v11, 0x0

    .line 960
    .end local v11    # "$i$f$isFull":I
    .end local v39    # "value$iv$iv$iv$iv":J
    :goto_1f
    if-eqz v11, :cond_39

    .line 962
    shl-int/lit8 v11, v6, 0x3

    add-int/2addr v11, v10

    .line 963
    .local v11, "index$iv$iv$iv":I
    move v12, v11

    .local v12, "index$iv$iv":I
    const/16 v37, 0x0

    .line 964
    .local v37, "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    aget-object v39, v1, v12

    move-object/from16 v40, v1

    .end local v1    # "k$iv$iv":[Ljava/lang/Object;
    .local v40, "k$iv$iv":[Ljava/lang/Object;
    move-object/from16 v1, v39

    check-cast v1, Landroidx/compose/runtime/DerivedState;

    .local v1, "derivedState":Landroidx/compose/runtime/DerivedState;
    const/16 v39, 0x0

    .line 589
    .local v39, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    move-object/from16 v41, v3

    move-object/from16 v3, v54

    .end local v3    # "m$iv$iv$iv":[J
    .local v41, "m$iv$iv$iv":[J
    invoke-static {v1, v3}, Lkotlin/jvm/internal/Intrinsics;->checkNotNull(Ljava/lang/Object;Ljava/lang/String;)V

    .line 590
    invoke-virtual {v4, v1}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    .line 591
    .local v3, "previousValue":Ljava/lang/Object;
    invoke-interface {v1}, Landroidx/compose/runtime/DerivedState;->getPolicy()Landroidx/compose/runtime/SnapshotMutationPolicy;

    move-result-object v42

    if-nez v42, :cond_2e

    invoke-static {}, Landroidx/compose/runtime/SnapshotStateKt;->structuralEqualityPolicy()Landroidx/compose/runtime/SnapshotMutationPolicy;

    move-result-object v42

    :cond_2e
    move-object/from16 v43, v42

    .line 594
    .local v43, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    nop

    .line 595
    invoke-interface {v1}, Landroidx/compose/runtime/DerivedState;->getCurrentRecord()Landroidx/compose/runtime/DerivedState$Record;

    move-result-object v42

    move-object/from16 v44, v7

    .end local v7    # "element$iv$iv":Ljava/lang/Object;
    .local v44, "element$iv$iv":Ljava/lang/Object;
    invoke-interface/range {v42 .. v42}, Landroidx/compose/runtime/DerivedState$Record;->getCurrentValue()Ljava/lang/Object;

    move-result-object v7

    .line 596
    nop

    .line 594
    move/from16 v42, v11

    move-object/from16 v11, v43

    .end local v43    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v11, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v42, "index$iv$iv$iv":I
    invoke-interface {v11, v7, v3}, Landroidx/compose/runtime/SnapshotMutationPolicy;->equivalent(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v7

    if-nez v7, :cond_38

    .line 599
    move-object/from16 v7, v65

    .local v7, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    const/16 v43, 0x0

    .line 965
    .local v43, "$i$f$forEachScopeOf":I
    move-object/from16 v45, v3

    .end local v3    # "previousValue":Ljava/lang/Object;
    .local v45, "previousValue":Ljava/lang/Object;
    invoke-virtual {v7}, Landroidx/compose/runtime/collection/ScopeMap;->getMap()Landroidx/collection/MutableScatterMap;

    move-result-object v3

    invoke-virtual {v3, v1}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    .line 966
    .local v3, "value$iv":Ljava/lang/Object;
    if-eqz v3, :cond_37

    .line 967
    move-object/from16 v46, v7

    .end local v7    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    instance-of v7, v3, Landroidx/collection/MutableScatterSet;

    if-eqz v7, :cond_36

    .line 969
    move-object v7, v3

    check-cast v7, Landroidx/collection/MutableScatterSet;

    check-cast v7, Landroidx/collection/ScatterSet;

    .local v7, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/16 v47, 0x0

    .line 970
    .local v47, "$i$f$forEach":I
    nop

    .line 971
    move-object/from16 v48, v11

    .end local v11    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v48, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    iget-object v11, v7, Landroidx/collection/ScatterSet;->elements:[Ljava/lang/Object;

    .line 973
    .local v11, "k$iv$iv":[Ljava/lang/Object;
    move-object/from16 v49, v7

    .local v49, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    const/16 v50, 0x0

    .line 974
    .local v50, "$i$f$forEachIndex":I
    nop

    .line 975
    move-object/from16 v51, v7

    move/from16 v49, v12

    .end local v12    # "index$iv$iv":I
    .local v7, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v49, "index$iv$iv":I
    .local v51, "this_$iv$iv":Landroidx/collection/ScatterSet;
    iget-object v12, v7, Landroidx/collection/ScatterSet;->metadata:[J

    .line 976
    .local v12, "m$iv$iv$iv":[J
    move-object/from16 v52, v7

    .end local v7    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v52, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    array-length v7, v12

    add-int/lit8 v7, v7, -0x2

    .line 978
    .local v7, "lastIndex$iv$iv$iv":I
    move-object/from16 v53, v14

    .end local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v53, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v14, 0x0

    .local v14, "i$iv$iv$iv":I
    if-gt v14, v7, :cond_33

    .line 979
    :goto_20
    aget-wide v55, v12, v14

    .line 980
    .local v55, "slot$iv$iv$iv":J
    move-wide/from16 v59, v55

    .local v59, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    const/16 v57, 0x0

    .line 981
    .local v57, "$i$f$maskEmptyOrDeleted":I
    move-object/from16 v63, v12

    move-object/from16 v62, v13

    move-wide/from16 v12, v59

    move-object/from16 v59, v8

    move/from16 v60, v9

    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "bitCount$iv$iv$iv":I
    .end local v13    # "value$iv":Ljava/lang/Object;
    .local v12, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .local v59, "value":Ljava/lang/Object;
    .local v60, "bitCount$iv$iv$iv":I
    .local v62, "value$iv":Ljava/lang/Object;
    .local v63, "m$iv$iv$iv":[J
    not-long v8, v12

    const/16 v29, 0x7

    shl-long v8, v8, v29

    and-long/2addr v8, v12

    and-long v8, v8, v17

    .line 980
    .end local v12    # "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .end local v57    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v8, v8, v17

    if-eqz v8, :cond_32

    .line 982
    sub-int v8, v14, v7

    not-int v8, v8

    ushr-int/lit8 v8, v8, 0x1f

    const/16 v9, 0x8

    rsub-int/lit8 v8, v8, 0x8

    .line 983
    .local v8, "bitCount$iv$iv$iv":I
    const/4 v9, 0x0

    .local v9, "j$iv$iv$iv":I
    :goto_21
    if-ge v9, v8, :cond_31

    .line 984
    const-wide/16 v12, 0xff

    and-long v66, v55, v12

    .local v66, "value$iv$iv$iv$iv":J
    const/4 v12, 0x0

    .line 985
    .local v12, "$i$f$isFull":I
    const-wide/16 v23, 0x80

    cmp-long v13, v66, v23

    if-gez v13, :cond_2f

    move/from16 v12, v19

    goto :goto_22

    :cond_2f
    const/4 v12, 0x0

    .line 984
    .end local v12    # "$i$f$isFull":I
    .end local v66    # "value$iv$iv$iv$iv":J
    :goto_22
    if-eqz v12, :cond_30

    .line 986
    shl-int/lit8 v12, v14, 0x3

    add-int/2addr v12, v9

    .line 987
    .local v12, "index$iv$iv$iv":I
    move v13, v12

    .local v13, "index$iv$iv":I
    const/16 v57, 0x0

    .line 988
    .local v57, "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    move/from16 v64, v12

    .end local v12    # "index$iv$iv$iv":I
    .local v64, "index$iv$iv$iv":I
    aget-object v12, v11, v13

    .local v12, "scope":Ljava/lang/Object;
    const/16 v66, 0x0

    .line 600
    .local v66, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    invoke-virtual {v5, v12}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 601
    const/16 v20, 0x1

    .line 602
    nop

    .line 988
    .end local v12    # "scope":Ljava/lang/Object;
    .end local v66    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    nop

    .line 989
    nop

    .line 987
    .end local v13    # "index$iv$iv":I
    .end local v57    # "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    nop

    .line 990
    .end local v64    # "index$iv$iv$iv":I
    :cond_30
    const/16 v12, 0x8

    shr-long v55, v55, v12

    .line 983
    add-int/lit8 v9, v9, 0x1

    goto :goto_21

    :cond_31
    const/16 v12, 0x8

    .line 992
    .end local v9    # "j$iv$iv$iv":I
    if-ne v8, v12, :cond_35

    .line 978
    .end local v8    # "bitCount$iv$iv$iv":I
    .end local v55    # "slot$iv$iv$iv":J
    :cond_32
    if-eq v14, v7, :cond_34

    add-int/lit8 v14, v14, 0x1

    move-object/from16 v8, v59

    move/from16 v9, v60

    move-object/from16 v13, v62

    move-object/from16 v12, v63

    goto :goto_20

    .end local v59    # "value":Ljava/lang/Object;
    .end local v60    # "bitCount$iv$iv$iv":I
    .end local v62    # "value$iv":Ljava/lang/Object;
    .end local v63    # "m$iv$iv$iv":[J
    .local v8, "value":Ljava/lang/Object;
    .local v9, "bitCount$iv$iv$iv":I
    .local v12, "m$iv$iv$iv":[J
    .local v13, "value$iv":Ljava/lang/Object;
    :cond_33
    move-object/from16 v59, v8

    move/from16 v60, v9

    move-object/from16 v63, v12

    move-object/from16 v62, v13

    .line 995
    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "bitCount$iv$iv$iv":I
    .end local v12    # "m$iv$iv$iv":[J
    .end local v13    # "value$iv":Ljava/lang/Object;
    .end local v14    # "i$iv$iv$iv":I
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v60    # "bitCount$iv$iv$iv":I
    .restart local v62    # "value$iv":Ljava/lang/Object;
    .restart local v63    # "m$iv$iv$iv":[J
    :cond_34
    nop

    .line 996
    .end local v7    # "lastIndex$iv$iv$iv":I
    .end local v50    # "$i$f$forEachIndex":I
    .end local v52    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .end local v63    # "m$iv$iv$iv":[J
    :cond_35
    nop

    .end local v11    # "k$iv$iv":[Ljava/lang/Object;
    .end local v47    # "$i$f$forEach":I
    .end local v51    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    goto :goto_23

    .line 997
    .end local v48    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v49    # "index$iv$iv":I
    .end local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v59    # "value":Ljava/lang/Object;
    .end local v60    # "bitCount$iv$iv$iv":I
    .end local v62    # "value$iv":Ljava/lang/Object;
    .restart local v8    # "value":Ljava/lang/Object;
    .restart local v9    # "bitCount$iv$iv$iv":I
    .local v11, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v12, "index$iv$iv":I
    .restart local v13    # "value$iv":Ljava/lang/Object;
    .local v14, "this_$iv$iv":Landroidx/collection/ScatterSet;
    :cond_36
    move-object/from16 v59, v8

    move/from16 v60, v9

    move-object/from16 v48, v11

    move/from16 v49, v12

    move-object/from16 v62, v13

    move-object/from16 v53, v14

    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "bitCount$iv$iv$iv":I
    .end local v11    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v12    # "index$iv$iv":I
    .end local v13    # "value$iv":Ljava/lang/Object;
    .end local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v48    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v49    # "index$iv$iv":I
    .restart local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v60    # "bitCount$iv$iv$iv":I
    .restart local v62    # "value$iv":Ljava/lang/Object;
    move-object v7, v3

    .local v7, "scope":Ljava/lang/Object;
    const/4 v8, 0x0

    .line 600
    .local v8, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    invoke-virtual {v5, v7}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 601
    const/4 v9, 0x1

    .line 602
    .end local v20    # "hasValues":Z
    .local v9, "hasValues":Z
    nop

    .line 997
    .end local v7    # "scope":Ljava/lang/Object;
    .end local v8    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    move/from16 v20, v9

    goto :goto_23

    .line 966
    .end local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v48    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v49    # "index$iv$iv":I
    .end local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v59    # "value":Ljava/lang/Object;
    .end local v60    # "bitCount$iv$iv$iv":I
    .end local v62    # "value$iv":Ljava/lang/Object;
    .local v7, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v8, "value":Ljava/lang/Object;
    .local v9, "bitCount$iv$iv$iv":I
    .restart local v11    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v12    # "index$iv$iv":I
    .restart local v13    # "value$iv":Ljava/lang/Object;
    .restart local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v20    # "hasValues":Z
    :cond_37
    move-object/from16 v46, v7

    move-object/from16 v59, v8

    move/from16 v60, v9

    move-object/from16 v48, v11

    move/from16 v49, v12

    move-object/from16 v62, v13

    move-object/from16 v53, v14

    .line 1000
    .end local v3    # "value$iv":Ljava/lang/Object;
    .end local v7    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "bitCount$iv$iv$iv":I
    .end local v11    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v12    # "index$iv$iv":I
    .end local v13    # "value$iv":Ljava/lang/Object;
    .end local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v48    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v49    # "index$iv$iv":I
    .restart local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v60    # "bitCount$iv$iv$iv":I
    .restart local v62    # "value$iv":Ljava/lang/Object;
    :goto_23
    nop

    .end local v43    # "$i$f$forEachScopeOf":I
    .end local v46    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    goto :goto_24

    .line 605
    .end local v45    # "previousValue":Ljava/lang/Object;
    .end local v48    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v49    # "index$iv$iv":I
    .end local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v59    # "value":Ljava/lang/Object;
    .end local v60    # "bitCount$iv$iv$iv":I
    .end local v62    # "value$iv":Ljava/lang/Object;
    .local v3, "previousValue":Ljava/lang/Object;
    .restart local v8    # "value":Ljava/lang/Object;
    .restart local v9    # "bitCount$iv$iv$iv":I
    .restart local v11    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v12    # "index$iv$iv":I
    .restart local v13    # "value$iv":Ljava/lang/Object;
    .restart local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    :cond_38
    move-object/from16 v45, v3

    move-object/from16 v59, v8

    move/from16 v60, v9

    move-object/from16 v48, v11

    move/from16 v49, v12

    move-object/from16 v62, v13

    move-object/from16 v53, v14

    .end local v3    # "previousValue":Ljava/lang/Object;
    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "bitCount$iv$iv$iv":I
    .end local v11    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v12    # "index$iv$iv":I
    .end local v13    # "value$iv":Ljava/lang/Object;
    .end local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v45    # "previousValue":Ljava/lang/Object;
    .restart local v48    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v49    # "index$iv$iv":I
    .restart local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v60    # "bitCount$iv$iv$iv":I
    .restart local v62    # "value$iv":Ljava/lang/Object;
    iget-object v3, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->statesToReread:Landroidx/compose/runtime/collection/MutableVector;

    invoke-virtual {v3, v1}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 607
    :goto_24
    nop

    .line 964
    .end local v1    # "derivedState":Landroidx/compose/runtime/DerivedState;
    .end local v39    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v45    # "previousValue":Ljava/lang/Object;
    .end local v48    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    nop

    .line 1001
    nop

    .line 963
    .end local v37    # "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    .end local v49    # "index$iv$iv":I
    goto :goto_25

    .line 960
    .end local v40    # "k$iv$iv":[Ljava/lang/Object;
    .end local v41    # "m$iv$iv$iv":[J
    .end local v42    # "index$iv$iv$iv":I
    .end local v44    # "element$iv$iv":Ljava/lang/Object;
    .end local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v59    # "value":Ljava/lang/Object;
    .end local v60    # "bitCount$iv$iv$iv":I
    .end local v62    # "value$iv":Ljava/lang/Object;
    .local v1, "k$iv$iv":[Ljava/lang/Object;
    .local v3, "m$iv$iv$iv":[J
    .local v7, "element$iv$iv":Ljava/lang/Object;
    .restart local v8    # "value":Ljava/lang/Object;
    .restart local v9    # "bitCount$iv$iv$iv":I
    .restart local v13    # "value$iv":Ljava/lang/Object;
    .restart local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    :cond_39
    move-object/from16 v40, v1

    move-object/from16 v41, v3

    move-object/from16 v44, v7

    move-object/from16 v59, v8

    move/from16 v60, v9

    move-object/from16 v62, v13

    move-object/from16 v53, v14

    .line 1002
    .end local v1    # "k$iv$iv":[Ljava/lang/Object;
    .end local v3    # "m$iv$iv$iv":[J
    .end local v7    # "element$iv$iv":Ljava/lang/Object;
    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "bitCount$iv$iv$iv":I
    .end local v13    # "value$iv":Ljava/lang/Object;
    .end local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v40    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v41    # "m$iv$iv$iv":[J
    .restart local v44    # "element$iv$iv":Ljava/lang/Object;
    .restart local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v60    # "bitCount$iv$iv$iv":I
    .restart local v62    # "value$iv":Ljava/lang/Object;
    :goto_25
    const/16 v1, 0x8

    shr-long v33, v33, v1

    .line 959
    add-int/lit8 v10, v10, 0x1

    move-object/from16 v1, v40

    move-object/from16 v3, v41

    move-object/from16 v7, v44

    move-object/from16 v14, v53

    move-object/from16 v8, v59

    move/from16 v9, v60

    move-object/from16 v13, v62

    goto/16 :goto_1e

    .end local v40    # "k$iv$iv":[Ljava/lang/Object;
    .end local v41    # "m$iv$iv$iv":[J
    .end local v44    # "element$iv$iv":Ljava/lang/Object;
    .end local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v59    # "value":Ljava/lang/Object;
    .end local v60    # "bitCount$iv$iv$iv":I
    .end local v62    # "value$iv":Ljava/lang/Object;
    .restart local v1    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v3    # "m$iv$iv$iv":[J
    .restart local v7    # "element$iv$iv":Ljava/lang/Object;
    .restart local v8    # "value":Ljava/lang/Object;
    .restart local v9    # "bitCount$iv$iv$iv":I
    .restart local v13    # "value$iv":Ljava/lang/Object;
    .restart local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    :cond_3a
    move-object/from16 v40, v1

    move-object/from16 v41, v3

    move-object/from16 v44, v7

    move-object/from16 v59, v8

    move/from16 v60, v9

    move-object/from16 v62, v13

    move-object/from16 v53, v14

    const/16 v1, 0x8

    .line 1004
    .end local v1    # "k$iv$iv":[Ljava/lang/Object;
    .end local v3    # "m$iv$iv$iv":[J
    .end local v7    # "element$iv$iv":Ljava/lang/Object;
    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "bitCount$iv$iv$iv":I
    .end local v10    # "j$iv$iv$iv":I
    .end local v13    # "value$iv":Ljava/lang/Object;
    .end local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v40    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v41    # "m$iv$iv$iv":[J
    .restart local v44    # "element$iv$iv":Ljava/lang/Object;
    .restart local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v60    # "bitCount$iv$iv$iv":I
    .restart local v62    # "value$iv":Ljava/lang/Object;
    move/from16 v10, v60

    .end local v60    # "bitCount$iv$iv$iv":I
    .local v10, "bitCount$iv$iv$iv":I
    if-ne v10, v1, :cond_3e

    goto :goto_26

    .line 953
    .end local v10    # "bitCount$iv$iv$iv":I
    .end local v40    # "k$iv$iv":[Ljava/lang/Object;
    .end local v41    # "m$iv$iv$iv":[J
    .end local v44    # "element$iv$iv":Ljava/lang/Object;
    .end local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v59    # "value":Ljava/lang/Object;
    .end local v62    # "value$iv":Ljava/lang/Object;
    .restart local v1    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v3    # "m$iv$iv$iv":[J
    .restart local v7    # "element$iv$iv":Ljava/lang/Object;
    .restart local v8    # "value":Ljava/lang/Object;
    .restart local v13    # "value$iv":Ljava/lang/Object;
    .restart local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    :cond_3b
    move-object/from16 v40, v1

    move-object/from16 v41, v3

    move-object/from16 v44, v7

    move-object/from16 v59, v8

    move-object/from16 v62, v13

    move-object/from16 v53, v14

    .line 951
    .end local v1    # "k$iv$iv":[Ljava/lang/Object;
    .end local v3    # "m$iv$iv$iv":[J
    .end local v7    # "element$iv$iv":Ljava/lang/Object;
    .end local v8    # "value":Ljava/lang/Object;
    .end local v13    # "value$iv":Ljava/lang/Object;
    .end local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v33    # "slot$iv$iv$iv":J
    .restart local v40    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v41    # "m$iv$iv$iv":[J
    .restart local v44    # "element$iv$iv":Ljava/lang/Object;
    .restart local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v62    # "value$iv":Ljava/lang/Object;
    :goto_26
    if-eq v6, v2, :cond_3d

    add-int/lit8 v6, v6, 0x1

    move-object/from16 v11, v35

    move/from16 v12, v36

    move/from16 v9, v38

    move-object/from16 v1, v40

    move-object/from16 v3, v41

    move-object/from16 v7, v44

    move-object/from16 v14, v53

    move-object/from16 v10, v58

    move-object/from16 v8, v59

    move-object/from16 v13, v62

    goto/16 :goto_1d

    .end local v35    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v36    # "$i$f$forEachScopeOf":I
    .end local v38    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v40    # "k$iv$iv":[Ljava/lang/Object;
    .end local v41    # "m$iv$iv$iv":[J
    .end local v44    # "element$iv$iv":Ljava/lang/Object;
    .end local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v59    # "value":Ljava/lang/Object;
    .end local v62    # "value$iv":Ljava/lang/Object;
    .restart local v1    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v3    # "m$iv$iv$iv":[J
    .restart local v7    # "element$iv$iv":Ljava/lang/Object;
    .restart local v8    # "value":Ljava/lang/Object;
    .local v9, "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .local v10, "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .local v11, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v12, "$i$f$forEachScopeOf":I
    .restart local v13    # "value$iv":Ljava/lang/Object;
    .restart local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    :cond_3c
    move-object/from16 v40, v1

    move-object/from16 v41, v3

    move-object/from16 v44, v7

    move-object/from16 v59, v8

    move/from16 v38, v9

    move-object/from16 v58, v10

    move-object/from16 v35, v11

    move/from16 v36, v12

    move-object/from16 v62, v13

    move-object/from16 v53, v14

    .end local v1    # "k$iv$iv":[Ljava/lang/Object;
    .end local v3    # "m$iv$iv$iv":[J
    .end local v7    # "element$iv$iv":Ljava/lang/Object;
    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v10    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v11    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v12    # "$i$f$forEachScopeOf":I
    .end local v13    # "value$iv":Ljava/lang/Object;
    .end local v14    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v35    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v36    # "$i$f$forEachScopeOf":I
    .restart local v38    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v40    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v41    # "m$iv$iv$iv":[J
    .restart local v44    # "element$iv$iv":Ljava/lang/Object;
    .restart local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v62    # "value$iv":Ljava/lang/Object;
    :cond_3d
    move/from16 v1, v20

    .line 1007
    .end local v6    # "i$iv$iv$iv":I
    .end local v20    # "hasValues":Z
    .local v1, "hasValues":Z
    move/from16 v20, v1

    .line 1008
    .end local v1    # "hasValues":Z
    .end local v2    # "lastIndex$iv$iv$iv":I
    .end local v27    # "$i$f$forEachIndex":I
    .end local v31    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .end local v41    # "m$iv$iv$iv":[J
    .restart local v20    # "hasValues":Z
    :cond_3e
    move-object/from16 v64, v4

    move/from16 v1, v20

    .end local v15    # "$i$f$forEach":I
    .end local v40    # "k$iv$iv":[Ljava/lang/Object;
    .end local v53    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    goto/16 :goto_2e

    .line 1009
    .end local v20    # "hasValues":Z
    .end local v21    # "$i$f$forEach":I
    .end local v30    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .end local v35    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v36    # "$i$f$forEachScopeOf":I
    .end local v38    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v44    # "element$iv$iv":Ljava/lang/Object;
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v59    # "value":Ljava/lang/Object;
    .end local v62    # "value$iv":Ljava/lang/Object;
    .restart local v1    # "hasValues":Z
    .local v2, "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .local v3, "$i$f$forEach":I
    .restart local v7    # "element$iv$iv":Ljava/lang/Object;
    .restart local v8    # "value":Ljava/lang/Object;
    .restart local v9    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v10    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v11    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v12    # "$i$f$forEachScopeOf":I
    .restart local v13    # "value$iv":Ljava/lang/Object;
    :cond_3f
    move/from16 v20, v1

    move-object/from16 v30, v2

    move/from16 v21, v3

    move-object/from16 v32, v6

    move-object/from16 v44, v7

    move-object/from16 v59, v8

    move/from16 v38, v9

    move-object/from16 v58, v10

    move-object/from16 v35, v11

    move/from16 v36, v12

    move-object/from16 v62, v13

    .end local v1    # "hasValues":Z
    .end local v2    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .end local v3    # "$i$f$forEach":I
    .end local v7    # "element$iv$iv":Ljava/lang/Object;
    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v10    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v11    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v12    # "$i$f$forEachScopeOf":I
    .end local v13    # "value$iv":Ljava/lang/Object;
    .restart local v20    # "hasValues":Z
    .restart local v21    # "$i$f$forEach":I
    .restart local v30    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .restart local v35    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v36    # "$i$f$forEachScopeOf":I
    .restart local v38    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v44    # "element$iv$iv":Ljava/lang/Object;
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v62    # "value$iv":Ljava/lang/Object;
    move-object/from16 v1, v62

    check-cast v1, Landroidx/compose/runtime/DerivedState;

    .local v1, "derivedState":Landroidx/compose/runtime/DerivedState;
    const/4 v2, 0x0

    .line 589
    .local v2, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    nop

    .line 590
    invoke-virtual {v4, v1}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    .line 591
    .local v3, "previousValue":Ljava/lang/Object;
    invoke-interface {v1}, Landroidx/compose/runtime/DerivedState;->getPolicy()Landroidx/compose/runtime/SnapshotMutationPolicy;

    move-result-object v6

    if-nez v6, :cond_40

    invoke-static {}, Landroidx/compose/runtime/SnapshotStateKt;->structuralEqualityPolicy()Landroidx/compose/runtime/SnapshotMutationPolicy;

    move-result-object v6

    .line 594
    .local v6, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    :cond_40
    nop

    .line 595
    invoke-interface {v1}, Landroidx/compose/runtime/DerivedState;->getCurrentRecord()Landroidx/compose/runtime/DerivedState$Record;

    move-result-object v7

    invoke-interface {v7}, Landroidx/compose/runtime/DerivedState$Record;->getCurrentValue()Ljava/lang/Object;

    move-result-object v7

    .line 596
    nop

    .line 594
    invoke-interface {v6, v7, v3}, Landroidx/compose/runtime/SnapshotMutationPolicy;->equivalent(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v7

    if-nez v7, :cond_4a

    .line 599
    move-object/from16 v7, v65

    .local v7, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    const/4 v8, 0x0

    .line 1010
    .local v8, "$i$f$forEachScopeOf":I
    invoke-virtual {v7}, Landroidx/compose/runtime/collection/ScopeMap;->getMap()Landroidx/collection/MutableScatterMap;

    move-result-object v9

    invoke-virtual {v9, v1}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v9

    .line 1011
    .local v9, "value$iv":Ljava/lang/Object;
    if-eqz v9, :cond_49

    .line 1012
    instance-of v10, v9, Landroidx/collection/MutableScatterSet;

    if-eqz v10, :cond_48

    .line 1014
    move-object v10, v9

    check-cast v10, Landroidx/collection/MutableScatterSet;

    check-cast v10, Landroidx/collection/ScatterSet;

    .local v10, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v11, 0x0

    .line 1015
    .local v11, "$i$f$forEach":I
    nop

    .line 1016
    iget-object v12, v10, Landroidx/collection/ScatterSet;->elements:[Ljava/lang/Object;

    .line 1018
    .local v12, "k$iv$iv":[Ljava/lang/Object;
    move-object v13, v10

    .local v13, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v14, 0x0

    .line 1019
    .local v14, "$i$f$forEachIndex":I
    nop

    .line 1020
    iget-object v15, v13, Landroidx/collection/ScatterSet;->metadata:[J

    .line 1021
    .local v15, "m$iv$iv$iv":[J
    move/from16 v27, v2

    .end local v2    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .local v27, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    array-length v2, v15

    add-int/lit8 v2, v2, -0x2

    .line 1023
    .local v2, "lastIndex$iv$iv$iv":I
    move-object/from16 v31, v3

    .end local v3    # "previousValue":Ljava/lang/Object;
    .local v31, "previousValue":Ljava/lang/Object;
    const/4 v3, 0x0

    .local v3, "i$iv$iv$iv":I
    if-gt v3, v2, :cond_45

    .line 1024
    :goto_27
    aget-wide v33, v15, v3

    .line 1025
    .restart local v33    # "slot$iv$iv$iv":J
    move-wide/from16 v39, v33

    .local v39, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    const/16 v37, 0x0

    .line 1026
    .local v37, "$i$f$maskEmptyOrDeleted":I
    move-object/from16 v41, v6

    move-object/from16 v42, v7

    move-wide/from16 v6, v39

    move-object/from16 v39, v10

    move/from16 v40, v11

    .end local v7    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v10    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v11    # "$i$f$forEach":I
    .local v6, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .local v39, "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v40, "$i$f$forEach":I
    .local v41, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v42, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    not-long v10, v6

    const/16 v29, 0x7

    shl-long v10, v10, v29

    and-long/2addr v10, v6

    and-long v6, v10, v17

    .line 1025
    .end local v6    # "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .end local v37    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v6, v6, v17

    if-eqz v6, :cond_44

    .line 1027
    sub-int v6, v3, v2

    not-int v6, v6

    ushr-int/lit8 v6, v6, 0x1f

    const/16 v7, 0x8

    rsub-int/lit8 v10, v6, 0x8

    .line 1028
    .local v10, "bitCount$iv$iv$iv":I
    const/4 v6, 0x0

    .local v6, "j$iv$iv$iv":I
    :goto_28
    if-ge v6, v10, :cond_43

    .line 1029
    const-wide/16 v25, 0xff

    and-long v45, v33, v25

    .local v45, "value$iv$iv$iv$iv":J
    const/4 v7, 0x0

    .line 1030
    .local v7, "$i$f$isFull":I
    const-wide/16 v23, 0x80

    cmp-long v11, v45, v23

    if-gez v11, :cond_41

    move/from16 v7, v19

    goto :goto_29

    :cond_41
    const/4 v7, 0x0

    .line 1029
    .end local v7    # "$i$f$isFull":I
    .end local v45    # "value$iv$iv$iv$iv":J
    :goto_29
    if-eqz v7, :cond_42

    .line 1031
    shl-int/lit8 v7, v3, 0x3

    add-int/2addr v7, v6

    .line 1032
    .local v7, "index$iv$iv$iv":I
    move v11, v7

    .local v11, "index$iv$iv":I
    const/16 v37, 0x0

    .line 1033
    .local v37, "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    move-object/from16 v64, v4

    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v64, "recordedDerivedStateValues":Ljava/util/HashMap;
    aget-object v4, v12, v11

    .local v4, "scope":Ljava/lang/Object;
    const/16 v43, 0x0

    .line 600
    .local v43, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    invoke-virtual {v5, v4}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 601
    const/16 v20, 0x1

    .line 602
    nop

    .line 1033
    .end local v4    # "scope":Ljava/lang/Object;
    .end local v43    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    nop

    .line 1034
    nop

    .line 1032
    .end local v11    # "index$iv$iv":I
    .end local v37    # "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    goto :goto_2a

    .line 1029
    .end local v7    # "index$iv$iv$iv":I
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v4, "recordedDerivedStateValues":Ljava/util/HashMap;
    :cond_42
    move-object/from16 v64, v4

    .line 1035
    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    :goto_2a
    const/16 v4, 0x8

    shr-long v33, v33, v4

    .line 1028
    add-int/lit8 v6, v6, 0x1

    move-object/from16 v4, v64

    goto :goto_28

    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    :cond_43
    move-object/from16 v64, v4

    const/16 v4, 0x8

    .line 1037
    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v6    # "j$iv$iv$iv":I
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    if-ne v10, v4, :cond_47

    goto :goto_2b

    .line 1025
    .end local v10    # "bitCount$iv$iv$iv":I
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    :cond_44
    move-object/from16 v64, v4

    .line 1023
    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v33    # "slot$iv$iv$iv":J
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    :goto_2b
    if-eq v3, v2, :cond_46

    add-int/lit8 v3, v3, 0x1

    move-object/from16 v10, v39

    move/from16 v11, v40

    move-object/from16 v6, v41

    move-object/from16 v7, v42

    move-object/from16 v4, v64

    goto :goto_27

    .end local v39    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v40    # "$i$f$forEach":I
    .end local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v42    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v6, "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .local v7, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v10, "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v11, "$i$f$forEach":I
    :cond_45
    move-object/from16 v64, v4

    move-object/from16 v41, v6

    move-object/from16 v42, v7

    move-object/from16 v39, v10

    move/from16 v40, v11

    .line 1040
    .end local v3    # "i$iv$iv$iv":I
    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v6    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v7    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v10    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v11    # "$i$f$forEach":I
    .restart local v39    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v40    # "$i$f$forEach":I
    .restart local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v42    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    :cond_46
    nop

    .line 1041
    .end local v2    # "lastIndex$iv$iv$iv":I
    .end local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .end local v14    # "$i$f$forEachIndex":I
    .end local v15    # "m$iv$iv$iv":[J
    :cond_47
    nop

    .end local v12    # "k$iv$iv":[Ljava/lang/Object;
    .end local v39    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v40    # "$i$f$forEach":I
    goto :goto_2c

    .line 1042
    .end local v27    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v31    # "previousValue":Ljava/lang/Object;
    .end local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v42    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v2, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .local v3, "previousValue":Ljava/lang/Object;
    .restart local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v6    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v7    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    :cond_48
    move/from16 v27, v2

    move-object/from16 v31, v3

    move-object/from16 v64, v4

    move-object/from16 v41, v6

    move-object/from16 v42, v7

    .end local v2    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v3    # "previousValue":Ljava/lang/Object;
    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v6    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v7    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v27    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .restart local v31    # "previousValue":Ljava/lang/Object;
    .restart local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v42    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    move-object v2, v9

    .local v2, "scope":Ljava/lang/Object;
    const/4 v3, 0x0

    .line 600
    .local v3, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    invoke-virtual {v5, v2}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 601
    const/4 v4, 0x1

    .line 602
    .end local v20    # "hasValues":Z
    .local v4, "hasValues":Z
    nop

    .line 1042
    .end local v2    # "scope":Ljava/lang/Object;
    .end local v3    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1$1":I
    move/from16 v20, v4

    goto :goto_2c

    .line 1011
    .end local v27    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v31    # "previousValue":Ljava/lang/Object;
    .end local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v42    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v2, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .local v3, "previousValue":Ljava/lang/Object;
    .local v4, "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v6    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v7    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v20    # "hasValues":Z
    :cond_49
    move/from16 v27, v2

    move-object/from16 v31, v3

    move-object/from16 v64, v4

    move-object/from16 v41, v6

    move-object/from16 v42, v7

    .line 1045
    .end local v2    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v3    # "previousValue":Ljava/lang/Object;
    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v6    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v7    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v9    # "value$iv":Ljava/lang/Object;
    .restart local v27    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .restart local v31    # "previousValue":Ljava/lang/Object;
    .restart local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v42    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    :goto_2c
    nop

    .end local v8    # "$i$f$forEachScopeOf":I
    .end local v42    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    goto :goto_2d

    .line 605
    .end local v27    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v31    # "previousValue":Ljava/lang/Object;
    .end local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v2    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .restart local v3    # "previousValue":Ljava/lang/Object;
    .restart local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v6    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    :cond_4a
    move/from16 v27, v2

    move-object/from16 v31, v3

    move-object/from16 v64, v4

    move-object/from16 v41, v6

    .end local v2    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v3    # "previousValue":Ljava/lang/Object;
    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v6    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v27    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .restart local v31    # "previousValue":Ljava/lang/Object;
    .restart local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    iget-object v2, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->statesToReread:Landroidx/compose/runtime/collection/MutableVector;

    invoke-virtual {v2, v1}, Landroidx/compose/runtime/collection/MutableVector;->add(Ljava/lang/Object;)Z

    .line 607
    :goto_2d
    nop

    .line 1009
    .end local v1    # "derivedState":Landroidx/compose/runtime/DerivedState;
    .end local v27    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$1":I
    .end local v31    # "previousValue":Ljava/lang/Object;
    .end local v41    # "policy":Landroidx/compose/runtime/SnapshotMutationPolicy;
    move/from16 v1, v20

    goto :goto_2e

    .line 803
    .end local v20    # "hasValues":Z
    .end local v21    # "$i$f$forEach":I
    .end local v30    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .end local v35    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v36    # "$i$f$forEachScopeOf":I
    .end local v38    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v44    # "element$iv$iv":Ljava/lang/Object;
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v59    # "value":Ljava/lang/Object;
    .end local v62    # "value$iv":Ljava/lang/Object;
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v1, "hasValues":Z
    .local v2, "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .local v3, "$i$f$forEach":I
    .restart local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v7, "element$iv$iv":Ljava/lang/Object;
    .local v8, "value":Ljava/lang/Object;
    .local v9, "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .local v10, "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .local v11, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v12, "$i$f$forEachScopeOf":I
    .local v13, "value$iv":Ljava/lang/Object;
    :cond_4b
    move/from16 v20, v1

    move-object/from16 v30, v2

    move/from16 v21, v3

    move-object/from16 v64, v4

    move-object/from16 v32, v6

    move-object/from16 v44, v7

    move-object/from16 v59, v8

    move/from16 v38, v9

    move-object/from16 v58, v10

    move-object/from16 v35, v11

    move/from16 v36, v12

    move-object/from16 v62, v13

    .line 1046
    .end local v2    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .end local v3    # "$i$f$forEach":I
    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v7    # "element$iv$iv":Ljava/lang/Object;
    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v10    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v11    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v12    # "$i$f$forEachScopeOf":I
    .end local v13    # "value$iv":Ljava/lang/Object;
    .restart local v21    # "$i$f$forEach":I
    .restart local v30    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .restart local v35    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v36    # "$i$f$forEachScopeOf":I
    .restart local v38    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v44    # "element$iv$iv":Ljava/lang/Object;
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    :goto_2e
    goto :goto_2f

    .line 586
    .end local v21    # "$i$f$forEach":I
    .end local v30    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .end local v35    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v36    # "$i$f$forEachScopeOf":I
    .end local v38    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v44    # "element$iv$iv":Ljava/lang/Object;
    .end local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v59    # "value":Ljava/lang/Object;
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v2    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .restart local v3    # "$i$f$forEach":I
    .restart local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v7    # "element$iv$iv":Ljava/lang/Object;
    .restart local v8    # "value":Ljava/lang/Object;
    .restart local v9    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v10    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    :cond_4c
    move/from16 v20, v1

    move-object/from16 v30, v2

    move/from16 v21, v3

    move-object/from16 v64, v4

    move-object/from16 v32, v6

    move-object/from16 v44, v7

    move-object/from16 v59, v8

    move/from16 v38, v9

    move-object/from16 v58, v10

    .line 610
    .end local v2    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .end local v3    # "$i$f$forEach":I
    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .end local v7    # "element$iv$iv":Ljava/lang/Object;
    .end local v8    # "value":Ljava/lang/Object;
    .end local v9    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v10    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v21    # "$i$f$forEach":I
    .restart local v30    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .restart local v38    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .restart local v44    # "element$iv$iv":Ljava/lang/Object;
    .restart local v58    # "dependencyToDerivedStates":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v59    # "value":Ljava/lang/Object;
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    :goto_2f
    move-object/from16 v2, v65

    .local v2, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    const/4 v3, 0x0

    .line 1047
    .local v3, "$i$f$forEachScopeOf":I
    invoke-virtual {v2}, Landroidx/compose/runtime/collection/ScopeMap;->getMap()Landroidx/collection/MutableScatterMap;

    move-result-object v4

    move-object/from16 v6, v59

    .end local v59    # "value":Ljava/lang/Object;
    .local v6, "value":Ljava/lang/Object;
    invoke-virtual {v4, v6}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v4

    .line 1048
    .local v4, "value$iv":Ljava/lang/Object;
    if-eqz v4, :cond_55

    .line 1049
    instance-of v7, v4, Landroidx/collection/MutableScatterSet;

    if-eqz v7, :cond_54

    .line 1051
    move-object v7, v4

    check-cast v7, Landroidx/collection/MutableScatterSet;

    check-cast v7, Landroidx/collection/ScatterSet;

    .local v7, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v8, 0x0

    .line 1052
    .local v8, "$i$f$forEach":I
    nop

    .line 1053
    iget-object v9, v7, Landroidx/collection/ScatterSet;->elements:[Ljava/lang/Object;

    .line 1055
    .local v9, "k$iv$iv":[Ljava/lang/Object;
    move-object v10, v7

    .local v10, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v11, 0x0

    .line 1056
    .local v11, "$i$f$forEachIndex":I
    nop

    .line 1057
    iget-object v12, v10, Landroidx/collection/ScatterSet;->metadata:[J

    .line 1058
    .local v12, "m$iv$iv$iv":[J
    array-length v13, v12

    add-int/lit8 v13, v13, -0x2

    .line 1060
    .local v13, "lastIndex$iv$iv$iv":I
    const/4 v14, 0x0

    .local v14, "i$iv$iv$iv":I
    if-gt v14, v13, :cond_52

    .line 1061
    :goto_30
    aget-wide v33, v12, v14

    .line 1062
    .restart local v33    # "slot$iv$iv$iv":J
    move-wide/from16 v35, v33

    .local v35, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    const/4 v15, 0x0

    .line 1063
    .local v15, "$i$f$maskEmptyOrDeleted":I
    move/from16 v27, v1

    move-object/from16 v20, v2

    move-object/from16 v59, v6

    move-object/from16 v31, v7

    move-wide/from16 v1, v35

    .end local v2    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "value":Ljava/lang/Object;
    .end local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v35    # "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .local v1, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .local v20, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v27, "hasValues":Z
    .local v31, "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v59    # "value":Ljava/lang/Object;
    not-long v6, v1

    const/16 v29, 0x7

    shl-long v6, v6, v29

    and-long/2addr v6, v1

    and-long v1, v6, v17

    .line 1062
    .end local v1    # "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .end local v15    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v1, v1, v17

    if-eqz v1, :cond_51

    .line 1064
    sub-int v1, v14, v13

    not-int v1, v1

    ushr-int/lit8 v1, v1, 0x1f

    const/16 v2, 0x8

    rsub-int/lit8 v1, v1, 0x8

    .line 1065
    .local v1, "bitCount$iv$iv$iv":I
    const/4 v2, 0x0

    .local v2, "j$iv$iv$iv":I
    :goto_31
    if-ge v2, v1, :cond_4f

    .line 1066
    const-wide/16 v6, 0xff

    and-long v25, v33, v6

    .local v25, "value$iv$iv$iv$iv":J
    const/4 v15, 0x0

    .line 1067
    .local v15, "$i$f$isFull":I
    const-wide/16 v23, 0x80

    cmp-long v35, v25, v23

    if-gez v35, :cond_4d

    move/from16 v15, v19

    goto :goto_32

    :cond_4d
    const/4 v15, 0x0

    .line 1066
    .end local v15    # "$i$f$isFull":I
    .end local v25    # "value$iv$iv$iv$iv":J
    :goto_32
    if-eqz v15, :cond_4e

    .line 1068
    shl-int/lit8 v15, v14, 0x3

    add-int/2addr v15, v2

    .line 1069
    .local v15, "index$iv$iv$iv":I
    move/from16 v25, v15

    .local v25, "index$iv$iv":I
    const/16 v26, 0x0

    .line 1070
    .local v26, "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    aget-object v6, v9, v25

    .local v6, "scope":Ljava/lang/Object;
    const/4 v7, 0x0

    .line 611
    .local v7, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$2":I
    invoke-virtual {v5, v6}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 612
    const/16 v27, 0x1

    .line 613
    nop

    .line 1070
    .end local v6    # "scope":Ljava/lang/Object;
    .end local v7    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$2":I
    nop

    .line 1071
    nop

    .line 1069
    .end local v25    # "index$iv$iv":I
    .end local v26    # "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    nop

    .line 1072
    .end local v15    # "index$iv$iv$iv":I
    :cond_4e
    const/16 v6, 0x8

    shr-long v33, v33, v6

    .line 1065
    add-int/lit8 v2, v2, 0x1

    goto :goto_31

    :cond_4f
    const/16 v6, 0x8

    const-wide/16 v23, 0x80

    .line 1074
    .end local v2    # "j$iv$iv$iv":I
    if-ne v1, v6, :cond_50

    move/from16 v1, v27

    goto :goto_33

    :cond_50
    move/from16 v1, v27

    goto :goto_34

    .line 1062
    .end local v1    # "bitCount$iv$iv$iv":I
    :cond_51
    const/16 v6, 0x8

    const-wide/16 v23, 0x80

    move/from16 v1, v27

    .line 1060
    .end local v27    # "hasValues":Z
    .end local v33    # "slot$iv$iv$iv":J
    .local v1, "hasValues":Z
    :goto_33
    if-eq v14, v13, :cond_53

    add-int/lit8 v14, v14, 0x1

    move-object/from16 v2, v20

    move-object/from16 v7, v31

    move-object/from16 v6, v59

    goto :goto_30

    .end local v20    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v31    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v59    # "value":Ljava/lang/Object;
    .local v2, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v6, "value":Ljava/lang/Object;
    .local v7, "this_$iv$iv":Landroidx/collection/ScatterSet;
    :cond_52
    move-object/from16 v20, v2

    move-object/from16 v59, v6

    move-object/from16 v31, v7

    const/16 v6, 0x8

    const-wide/16 v23, 0x80

    const/16 v29, 0x7

    .line 1077
    .end local v2    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "value":Ljava/lang/Object;
    .end local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v14    # "i$iv$iv$iv":I
    .restart local v20    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v31    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v59    # "value":Ljava/lang/Object;
    :cond_53
    nop

    .line 1078
    .end local v10    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .end local v11    # "$i$f$forEachIndex":I
    .end local v12    # "m$iv$iv$iv":[J
    .end local v13    # "lastIndex$iv$iv$iv":I
    :goto_34
    nop

    .end local v8    # "$i$f$forEach":I
    .end local v9    # "k$iv$iv":[Ljava/lang/Object;
    .end local v31    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    goto :goto_35

    .line 1079
    .end local v20    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v59    # "value":Ljava/lang/Object;
    .restart local v2    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "value":Ljava/lang/Object;
    :cond_54
    move-object/from16 v20, v2

    move-object/from16 v59, v6

    const/16 v6, 0x8

    const-wide/16 v23, 0x80

    const/16 v29, 0x7

    .end local v2    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v6    # "value":Ljava/lang/Object;
    .restart local v20    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v59    # "value":Ljava/lang/Object;
    move-object v2, v4

    .local v2, "scope":Ljava/lang/Object;
    const/4 v7, 0x0

    .line 611
    .local v7, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$2":I
    invoke-virtual {v5, v2}, Landroidx/collection/MutableScatterSet;->add(Ljava/lang/Object;)Z

    .line 612
    const/4 v1, 0x1

    .line 613
    nop

    .line 1079
    .end local v2    # "scope":Ljava/lang/Object;
    .end local v7    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1$2":I
    goto :goto_35

    .line 1048
    .end local v20    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v59    # "value":Ljava/lang/Object;
    .local v2, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v6    # "value":Ljava/lang/Object;
    :cond_55
    move-object/from16 v20, v2

    move-object/from16 v59, v6

    const/16 v6, 0x8

    const-wide/16 v23, 0x80

    const/16 v29, 0x7

    .line 1082
    .end local v2    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v4    # "value$iv":Ljava/lang/Object;
    .end local v6    # "value":Ljava/lang/Object;
    .restart local v20    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v59    # "value":Ljava/lang/Object;
    :goto_35
    nop

    .line 614
    .end local v3    # "$i$f$forEachScopeOf":I
    .end local v20    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    nop

    .line 946
    .end local v38    # "$i$a$-fastForEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$1":I
    .end local v59    # "value":Ljava/lang/Object;
    :goto_36
    move/from16 v3, v21

    move-object/from16 v2, v30

    move-object/from16 v6, v32

    move-object/from16 v4, v64

    .end local v44    # "element$iv$iv":Ljava/lang/Object;
    goto/16 :goto_1c

    .line 1083
    .end local v21    # "$i$f$forEach":I
    .end local v30    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .end local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .local v2, "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .local v3, "$i$f$forEach":I
    .local v4, "recordedDerivedStateValues":Ljava/util/HashMap;
    :cond_56
    move/from16 v20, v1

    move-object/from16 v30, v2

    move/from16 v21, v3

    move-object/from16 v64, v4

    .line 1084
    .end local v2    # "$this$forEach$iv$iv":Ljava/lang/Iterable;
    .end local v3    # "$i$f$forEach":I
    .end local v4    # "recordedDerivedStateValues":Ljava/util/HashMap;
    .restart local v64    # "recordedDerivedStateValues":Ljava/util/HashMap;
    :goto_37
    nop

    .line 616
    .end local v28    # "$i$f$fastForEach":I
    .end local v61    # "$this$fastForEach$iv":Ljava/util/Set;
    iget-object v2, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->statesToReread:Landroidx/compose/runtime/collection/MutableVector;

    invoke-virtual {v2}, Landroidx/compose/runtime/collection/MutableVector;->isNotEmpty()Z

    move-result v2

    if-eqz v2, :cond_59

    .line 617
    iget-object v2, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->statesToReread:Landroidx/compose/runtime/collection/MutableVector;

    .local v2, "this_$iv":Landroidx/compose/runtime/collection/MutableVector;
    const/4 v3, 0x0

    .line 1085
    .restart local v3    # "$i$f$forEach":I
    nop

    .line 1086
    invoke-virtual {v2}, Landroidx/compose/runtime/collection/MutableVector;->getSize()I

    move-result v4

    .line 1087
    .local v4, "size$iv":I
    if-lez v4, :cond_58

    .line 1088
    const/4 v6, 0x0

    .line 1089
    .local v6, "i$iv":I
    invoke-virtual {v2}, Landroidx/compose/runtime/collection/MutableVector;->getContent()[Ljava/lang/Object;

    move-result-object v7

    .line 1091
    .local v7, "content$iv":[Ljava/lang/Object;
    :cond_57
    aget-object v8, v7, v6

    check-cast v8, Landroidx/compose/runtime/DerivedState;

    .local v8, "it":Landroidx/compose/runtime/DerivedState;
    const/4 v9, 0x0

    .line 618
    .local v9, "$i$a$-forEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$2":I
    invoke-virtual {v0, v8}, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->rereadDerivedState(Landroidx/compose/runtime/DerivedState;)V

    .line 619
    nop

    .line 1091
    .end local v8    # "it":Landroidx/compose/runtime/DerivedState;
    .end local v9    # "$i$a$-forEach-SnapshotStateObserver$ObservedScopeMap$recordInvalidation$2":I
    nop

    .line 1092
    add-int/lit8 v6, v6, 0x1

    .line 1093
    if-lt v6, v4, :cond_57

    .line 1095
    .end local v6    # "i$iv":I
    .end local v7    # "content$iv":[Ljava/lang/Object;
    :cond_58
    nop

    .line 620
    .end local v2    # "this_$iv":Landroidx/compose/runtime/collection/MutableVector;
    .end local v3    # "$i$f$forEach":I
    .end local v4    # "size$iv":I
    iget-object v2, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->statesToReread:Landroidx/compose/runtime/collection/MutableVector;

    invoke-virtual {v2}, Landroidx/compose/runtime/collection/MutableVector;->clear()V

    .line 623
    :cond_59
    return v1
.end method

.method public final recordRead(Ljava/lang/Object;)V
    .locals 6
    .param p1, "value"    # Ljava/lang/Object;

    .line 433
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScope:Ljava/lang/Object;

    invoke-static {v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNull(Ljava/lang/Object;)V

    .line 434
    .local v0, "scope":Ljava/lang/Object;
    nop

    .line 435
    nop

    .line 436
    iget v1, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentToken:I

    .line 437
    nop

    .line 438
    iget-object v2, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScopeReads:Landroidx/collection/MutableObjectIntMap;

    if-nez v2, :cond_0

    new-instance v2, Landroidx/collection/MutableObjectIntMap;

    const/4 v3, 0x1

    const/4 v4, 0x0

    const/4 v5, 0x0

    invoke-direct {v2, v5, v3, v4}, Landroidx/collection/MutableObjectIntMap;-><init>(IILkotlin/jvm/internal/DefaultConstructorMarker;)V

    move-object v3, v2

    .local v3, "it":Landroidx/collection/MutableObjectIntMap;
    const/4 v4, 0x0

    .line 439
    .local v4, "$i$a$-also-SnapshotStateObserver$ObservedScopeMap$recordRead$1":I
    iput-object v3, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->currentScopeReads:Landroidx/collection/MutableObjectIntMap;

    .line 440
    iget-object v5, p0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->scopeToValues:Landroidx/collection/MutableScatterMap;

    invoke-virtual {v5, v0, v3}, Landroidx/collection/MutableScatterMap;->set(Ljava/lang/Object;Ljava/lang/Object;)V

    .line 441
    nop

    .end local v3    # "it":Landroidx/collection/MutableObjectIntMap;
    .end local v4    # "$i$a$-also-SnapshotStateObserver$ObservedScopeMap$recordRead$1":I
    sget-object v3, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    .line 438
    nop

    .line 434
    :cond_0
    invoke-direct {p0, p1, v1, v0, v2}, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->recordRead(Ljava/lang/Object;ILjava/lang/Object;Landroidx/collection/MutableObjectIntMap;)V

    .line 443
    return-void
.end method

.method public final removeScopeIf(Lkotlin/jvm/functions/Function1;)V
    .locals 50
    .param p1, "predicate"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Ljava/lang/Boolean;",
            ">;)V"
        }
    .end annotation

    .line 535
    move-object/from16 v0, p0

    iget-object v1, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->scopeToValues:Landroidx/collection/MutableScatterMap;

    .local v1, "this_$iv":Landroidx/collection/MutableScatterMap;
    const/4 v2, 0x0

    .line 743
    .local v2, "$i$f$removeIf":I
    move-object v3, v1

    check-cast v3, Landroidx/collection/ScatterMap;

    .local v3, "this_$iv$iv":Landroidx/collection/ScatterMap;
    const/4 v4, 0x0

    .line 744
    .local v4, "$i$f$forEachIndexed":I
    iget-object v5, v3, Landroidx/collection/ScatterMap;->metadata:[J

    .line 745
    .local v5, "m$iv$iv":[J
    array-length v6, v5

    add-int/lit8 v6, v6, -0x2

    .line 747
    .local v6, "lastIndex$iv$iv":I
    const/4 v7, 0x0

    .local v7, "i$iv$iv":I
    if-gt v7, v6, :cond_d

    .line 748
    :goto_0
    aget-wide v8, v5, v7

    .line 749
    .local v8, "slot$iv$iv":J
    move-wide v10, v8

    .local v10, "$this$maskEmptyOrDeleted$iv$iv$iv":J
    const/4 v12, 0x0

    .line 750
    .local v12, "$i$f$maskEmptyOrDeleted":I
    not-long v13, v10

    const/4 v15, 0x7

    shl-long/2addr v13, v15

    and-long/2addr v13, v10

    const-wide v16, -0x7f7f7f7f7f7f7f80L    # -2.937446524422997E-306

    and-long v10, v13, v16

    .line 749
    .end local v10    # "$this$maskEmptyOrDeleted$iv$iv$iv":J
    .end local v12    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v10, v10, v16

    if-eqz v10, :cond_c

    .line 751
    sub-int v10, v7, v6

    not-int v10, v10

    ushr-int/lit8 v10, v10, 0x1f

    const/16 v11, 0x8

    rsub-int/lit8 v10, v10, 0x8

    .line 752
    .local v10, "bitCount$iv$iv":I
    const/4 v12, 0x0

    .local v12, "j$iv$iv":I
    :goto_1
    if-ge v12, v10, :cond_b

    .line 753
    const-wide/16 v13, 0xff

    and-long v18, v8, v13

    .local v18, "value$iv$iv$iv":J
    const/16 v20, 0x0

    .line 754
    .local v20, "$i$f$isFull":I
    const-wide/16 v21, 0x80

    cmp-long v23, v18, v21

    const/16 v24, 0x0

    const/16 v25, 0x1

    if-gez v23, :cond_0

    move/from16 v18, v25

    goto :goto_2

    :cond_0
    move/from16 v18, v24

    .line 753
    .end local v18    # "value$iv$iv$iv":J
    .end local v20    # "$i$f$isFull":I
    :goto_2
    if-eqz v18, :cond_a

    .line 755
    shl-int/lit8 v18, v7, 0x3

    add-int v18, v18, v12

    .line 756
    .local v18, "index$iv$iv":I
    move/from16 v19, v18

    .local v19, "index$iv":I
    const/16 v20, 0x0

    .line 757
    .local v20, "$i$a$-forEachIndexed-MutableScatterMap$removeIf$1$iv":I
    iget-object v13, v1, Landroidx/collection/MutableScatterMap;->keys:[Ljava/lang/Object;

    move/from16 v14, v19

    .end local v19    # "index$iv":I
    .local v14, "index$iv":I
    aget-object v13, v13, v14

    .local v13, "scope":Ljava/lang/Object;
    iget-object v11, v1, Landroidx/collection/MutableScatterMap;->values:[Ljava/lang/Object;

    aget-object v11, v11, v14

    check-cast v11, Landroidx/collection/MutableObjectIntMap;

    .local v11, "valueSet":Landroidx/collection/MutableObjectIntMap;
    const/16 v23, 0x0

    .line 536
    .local v23, "$i$a$-removeIf-SnapshotStateObserver$ObservedScopeMap$removeScopeIf$1":I
    move-object/from16 v15, p1

    invoke-interface {v15, v13}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v29

    check-cast v29, Ljava/lang/Boolean;

    invoke-virtual/range {v29 .. v29}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v30

    .local v30, "willRemove":Z
    const/16 v31, 0x0

    .line 537
    .local v31, "$i$a$-also-SnapshotStateObserver$ObservedScopeMap$removeScopeIf$1$1":I
    if-eqz v30, :cond_8

    .line 538
    move/from16 v32, v2

    .end local v2    # "$i$f$removeIf":I
    .local v32, "$i$f$removeIf":I
    move-object v2, v11

    check-cast v2, Landroidx/collection/ObjectIntMap;

    .local v2, "this_$iv":Landroidx/collection/ObjectIntMap;
    const/16 v33, 0x0

    .line 758
    .local v33, "$i$f$forEach":I
    move-object/from16 v34, v3

    .end local v3    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .local v34, "this_$iv$iv":Landroidx/collection/ScatterMap;
    iget-object v3, v2, Landroidx/collection/ObjectIntMap;->keys:[Ljava/lang/Object;

    .line 759
    .local v3, "k$iv":[Ljava/lang/Object;
    move/from16 v35, v4

    .end local v4    # "$i$f$forEachIndexed":I
    .local v35, "$i$f$forEachIndexed":I
    iget-object v4, v2, Landroidx/collection/ObjectIntMap;->values:[I

    .line 761
    .local v4, "v$iv":[I
    move-object/from16 v36, v2

    .local v36, "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    const/16 v37, 0x0

    .line 762
    .local v37, "$i$f$forEachIndexed":I
    move-object/from16 v38, v2

    move-object/from16 v36, v5

    .end local v5    # "m$iv$iv":[J
    .local v2, "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .local v36, "m$iv$iv":[J
    .local v38, "this_$iv":Landroidx/collection/ObjectIntMap;
    iget-object v5, v2, Landroidx/collection/ObjectIntMap;->metadata:[J

    .line 763
    .restart local v5    # "m$iv$iv":[J
    move-object/from16 v39, v2

    .end local v2    # "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .local v39, "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    array-length v2, v5

    add-int/lit8 v2, v2, -0x2

    .line 765
    .local v2, "lastIndex$iv$iv":I
    move-object/from16 v40, v11

    .end local v11    # "valueSet":Landroidx/collection/MutableObjectIntMap;
    .local v40, "valueSet":Landroidx/collection/MutableObjectIntMap;
    const/4 v11, 0x0

    .local v11, "i$iv$iv":I
    if-gt v11, v2, :cond_5

    .line 766
    :goto_3
    aget-wide v41, v5, v11

    .line 767
    .local v41, "slot$iv$iv":J
    move-wide/from16 v43, v41

    .local v43, "$this$maskEmptyOrDeleted$iv$iv$iv":J
    const/16 v45, 0x0

    .line 768
    .local v45, "$i$f$maskEmptyOrDeleted":I
    move-object/from16 v47, v5

    move/from16 v46, v6

    move-wide/from16 v48, v8

    move-wide/from16 v5, v43

    move/from16 v43, v7

    .end local v6    # "lastIndex$iv$iv":I
    .end local v7    # "i$iv$iv":I
    .end local v8    # "slot$iv$iv":J
    .local v5, "$this$maskEmptyOrDeleted$iv$iv$iv":J
    .local v43, "i$iv$iv":I
    .local v46, "lastIndex$iv$iv":I
    .local v47, "m$iv$iv":[J
    .local v48, "slot$iv$iv":J
    not-long v7, v5

    const/4 v9, 0x7

    shl-long/2addr v7, v9

    and-long/2addr v7, v5

    and-long v5, v7, v16

    .line 767
    .end local v5    # "$this$maskEmptyOrDeleted$iv$iv$iv":J
    .end local v45    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v5, v5, v16

    if-eqz v5, :cond_4

    .line 769
    sub-int v5, v11, v2

    not-int v5, v5

    ushr-int/lit8 v5, v5, 0x1f

    const/16 v6, 0x8

    rsub-int/lit8 v5, v5, 0x8

    .line 770
    .local v5, "bitCount$iv$iv":I
    const/4 v6, 0x0

    .local v6, "j$iv$iv":I
    :goto_4
    if-ge v6, v5, :cond_3

    .line 771
    const-wide/16 v7, 0xff

    and-long v26, v41, v7

    .local v26, "value$iv$iv$iv":J
    const/16 v28, 0x0

    .line 772
    .local v28, "$i$f$isFull":I
    cmp-long v44, v26, v21

    if-gez v44, :cond_1

    move/from16 v26, v25

    goto :goto_5

    :cond_1
    move/from16 v26, v24

    .line 771
    .end local v26    # "value$iv$iv$iv":J
    .end local v28    # "$i$f$isFull":I
    :goto_5
    if-eqz v26, :cond_2

    .line 773
    shl-int/lit8 v26, v11, 0x3

    add-int v26, v26, v6

    .line 774
    .local v26, "index$iv$iv":I
    move/from16 v27, v26

    .local v27, "index$iv":I
    const/16 v28, 0x0

    .line 775
    .local v28, "$i$a$-forEachIndexed-ObjectIntMap$forEach$1$iv":I
    aget-object v7, v3, v27

    .local v7, "value":Ljava/lang/Object;
    aget v8, v4, v27

    const/4 v8, 0x0

    .line 539
    .local v8, "$i$a$-forEach-SnapshotStateObserver$ObservedScopeMap$removeScopeIf$1$1$1":I
    invoke-direct {v0, v13, v7}, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->removeObservation(Ljava/lang/Object;Ljava/lang/Object;)V

    .line 540
    nop

    .line 775
    .end local v7    # "value":Ljava/lang/Object;
    .end local v8    # "$i$a$-forEach-SnapshotStateObserver$ObservedScopeMap$removeScopeIf$1$1$1":I
    nop

    .line 776
    nop

    .line 774
    .end local v27    # "index$iv":I
    .end local v28    # "$i$a$-forEachIndexed-ObjectIntMap$forEach$1$iv":I
    nop

    .line 777
    .end local v26    # "index$iv$iv":I
    :cond_2
    const/16 v7, 0x8

    shr-long v41, v41, v7

    .line 770
    add-int/lit8 v6, v6, 0x1

    goto :goto_4

    :cond_3
    const/16 v7, 0x8

    .line 779
    .end local v6    # "j$iv$iv":I
    if-ne v5, v7, :cond_7

    .line 765
    .end local v5    # "bitCount$iv$iv":I
    .end local v41    # "slot$iv$iv":J
    :cond_4
    if-eq v11, v2, :cond_6

    add-int/lit8 v11, v11, 0x1

    move/from16 v7, v43

    move/from16 v6, v46

    move-object/from16 v5, v47

    move-wide/from16 v8, v48

    goto :goto_3

    .end local v43    # "i$iv$iv":I
    .end local v46    # "lastIndex$iv$iv":I
    .end local v47    # "m$iv$iv":[J
    .end local v48    # "slot$iv$iv":J
    .local v5, "m$iv$iv":[J
    .local v6, "lastIndex$iv$iv":I
    .local v7, "i$iv$iv":I
    .local v8, "slot$iv$iv":J
    :cond_5
    move-object/from16 v47, v5

    move/from16 v46, v6

    move/from16 v43, v7

    move-wide/from16 v48, v8

    const/4 v9, 0x7

    .line 782
    .end local v5    # "m$iv$iv":[J
    .end local v6    # "lastIndex$iv$iv":I
    .end local v7    # "i$iv$iv":I
    .end local v8    # "slot$iv$iv":J
    .end local v11    # "i$iv$iv":I
    .restart local v43    # "i$iv$iv":I
    .restart local v46    # "lastIndex$iv$iv":I
    .restart local v47    # "m$iv$iv":[J
    .restart local v48    # "slot$iv$iv":J
    :cond_6
    nop

    .line 783
    .end local v2    # "lastIndex$iv$iv":I
    .end local v37    # "$i$f$forEachIndexed":I
    .end local v39    # "this_$iv$iv":Landroidx/collection/ObjectIntMap;
    .end local v47    # "m$iv$iv":[J
    :cond_7
    goto :goto_6

    .line 537
    .end local v32    # "$i$f$removeIf":I
    .end local v33    # "$i$f$forEach":I
    .end local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v35    # "$i$f$forEachIndexed":I
    .end local v36    # "m$iv$iv":[J
    .end local v38    # "this_$iv":Landroidx/collection/ObjectIntMap;
    .end local v40    # "valueSet":Landroidx/collection/MutableObjectIntMap;
    .end local v43    # "i$iv$iv":I
    .end local v46    # "lastIndex$iv$iv":I
    .end local v48    # "slot$iv$iv":J
    .local v2, "$i$f$removeIf":I
    .local v3, "this_$iv$iv":Landroidx/collection/ScatterMap;
    .local v4, "$i$f$forEachIndexed":I
    .restart local v5    # "m$iv$iv":[J
    .restart local v6    # "lastIndex$iv$iv":I
    .restart local v7    # "i$iv$iv":I
    .restart local v8    # "slot$iv$iv":J
    .local v11, "valueSet":Landroidx/collection/MutableObjectIntMap;
    :cond_8
    move/from16 v32, v2

    move-object/from16 v34, v3

    move/from16 v35, v4

    move-object/from16 v36, v5

    move/from16 v46, v6

    move/from16 v43, v7

    move-wide/from16 v48, v8

    move-object/from16 v40, v11

    const/4 v9, 0x7

    .line 542
    .end local v2    # "$i$f$removeIf":I
    .end local v3    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v4    # "$i$f$forEachIndexed":I
    .end local v5    # "m$iv$iv":[J
    .end local v6    # "lastIndex$iv$iv":I
    .end local v7    # "i$iv$iv":I
    .end local v8    # "slot$iv$iv":J
    .end local v11    # "valueSet":Landroidx/collection/MutableObjectIntMap;
    .restart local v32    # "$i$f$removeIf":I
    .restart local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .restart local v35    # "$i$f$forEachIndexed":I
    .restart local v36    # "m$iv$iv":[J
    .restart local v40    # "valueSet":Landroidx/collection/MutableObjectIntMap;
    .restart local v43    # "i$iv$iv":I
    .restart local v46    # "lastIndex$iv$iv":I
    .restart local v48    # "slot$iv$iv":J
    :goto_6
    nop

    .line 536
    .end local v30    # "willRemove":Z
    .end local v31    # "$i$a$-also-SnapshotStateObserver$ObservedScopeMap$removeScopeIf$1$1":I
    invoke-virtual/range {v29 .. v29}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    .line 757
    .end local v13    # "scope":Ljava/lang/Object;
    .end local v23    # "$i$a$-removeIf-SnapshotStateObserver$ObservedScopeMap$removeScopeIf$1":I
    .end local v40    # "valueSet":Landroidx/collection/MutableObjectIntMap;
    if-eqz v2, :cond_9

    .line 784
    invoke-virtual {v1, v14}, Landroidx/collection/MutableScatterMap;->removeValueAt(I)Ljava/lang/Object;

    .line 786
    :cond_9
    nop

    .line 756
    .end local v14    # "index$iv":I
    .end local v20    # "$i$a$-forEachIndexed-MutableScatterMap$removeIf$1$iv":I
    goto :goto_7

    .line 753
    .end local v18    # "index$iv$iv":I
    .end local v32    # "$i$f$removeIf":I
    .end local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v35    # "$i$f$forEachIndexed":I
    .end local v36    # "m$iv$iv":[J
    .end local v43    # "i$iv$iv":I
    .end local v46    # "lastIndex$iv$iv":I
    .end local v48    # "slot$iv$iv":J
    .restart local v2    # "$i$f$removeIf":I
    .restart local v3    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .restart local v4    # "$i$f$forEachIndexed":I
    .restart local v5    # "m$iv$iv":[J
    .restart local v6    # "lastIndex$iv$iv":I
    .restart local v7    # "i$iv$iv":I
    .restart local v8    # "slot$iv$iv":J
    :cond_a
    move/from16 v32, v2

    move-object/from16 v34, v3

    move/from16 v35, v4

    move-object/from16 v36, v5

    move/from16 v46, v6

    move/from16 v43, v7

    move-wide/from16 v48, v8

    move v9, v15

    move-object/from16 v15, p1

    .line 787
    .end local v2    # "$i$f$removeIf":I
    .end local v3    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v4    # "$i$f$forEachIndexed":I
    .end local v5    # "m$iv$iv":[J
    .end local v6    # "lastIndex$iv$iv":I
    .end local v7    # "i$iv$iv":I
    .end local v8    # "slot$iv$iv":J
    .restart local v32    # "$i$f$removeIf":I
    .restart local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .restart local v35    # "$i$f$forEachIndexed":I
    .restart local v36    # "m$iv$iv":[J
    .restart local v43    # "i$iv$iv":I
    .restart local v46    # "lastIndex$iv$iv":I
    .restart local v48    # "slot$iv$iv":J
    :goto_7
    const/16 v2, 0x8

    shr-long v3, v48, v2

    .line 752
    .end local v48    # "slot$iv$iv":J
    .local v3, "slot$iv$iv":J
    add-int/lit8 v12, v12, 0x1

    move v11, v2

    move v15, v9

    move/from16 v2, v32

    move-object/from16 v5, v36

    move/from16 v7, v43

    move/from16 v6, v46

    move-wide v8, v3

    move-object/from16 v3, v34

    move/from16 v4, v35

    goto/16 :goto_1

    .end local v32    # "$i$f$removeIf":I
    .end local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v35    # "$i$f$forEachIndexed":I
    .end local v36    # "m$iv$iv":[J
    .end local v43    # "i$iv$iv":I
    .end local v46    # "lastIndex$iv$iv":I
    .restart local v2    # "$i$f$removeIf":I
    .local v3, "this_$iv$iv":Landroidx/collection/ScatterMap;
    .restart local v4    # "$i$f$forEachIndexed":I
    .restart local v5    # "m$iv$iv":[J
    .restart local v6    # "lastIndex$iv$iv":I
    .restart local v7    # "i$iv$iv":I
    .restart local v8    # "slot$iv$iv":J
    :cond_b
    move-object/from16 v15, p1

    move/from16 v32, v2

    move-object/from16 v34, v3

    move/from16 v35, v4

    move-object/from16 v36, v5

    move/from16 v46, v6

    move/from16 v43, v7

    move-wide/from16 v48, v8

    move v2, v11

    .line 789
    .end local v2    # "$i$f$removeIf":I
    .end local v3    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v4    # "$i$f$forEachIndexed":I
    .end local v5    # "m$iv$iv":[J
    .end local v6    # "lastIndex$iv$iv":I
    .end local v7    # "i$iv$iv":I
    .end local v8    # "slot$iv$iv":J
    .end local v12    # "j$iv$iv":I
    .restart local v32    # "$i$f$removeIf":I
    .restart local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .restart local v35    # "$i$f$forEachIndexed":I
    .restart local v36    # "m$iv$iv":[J
    .restart local v43    # "i$iv$iv":I
    .restart local v46    # "lastIndex$iv$iv":I
    .restart local v48    # "slot$iv$iv":J
    if-ne v10, v2, :cond_f

    goto :goto_8

    .line 749
    .end local v10    # "bitCount$iv$iv":I
    .end local v32    # "$i$f$removeIf":I
    .end local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v35    # "$i$f$forEachIndexed":I
    .end local v36    # "m$iv$iv":[J
    .end local v43    # "i$iv$iv":I
    .end local v46    # "lastIndex$iv$iv":I
    .end local v48    # "slot$iv$iv":J
    .restart local v2    # "$i$f$removeIf":I
    .restart local v3    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .restart local v4    # "$i$f$forEachIndexed":I
    .restart local v5    # "m$iv$iv":[J
    .restart local v6    # "lastIndex$iv$iv":I
    .restart local v7    # "i$iv$iv":I
    .restart local v8    # "slot$iv$iv":J
    :cond_c
    move-object/from16 v15, p1

    move/from16 v32, v2

    move-object/from16 v34, v3

    move/from16 v35, v4

    move-object/from16 v36, v5

    move/from16 v46, v6

    move/from16 v43, v7

    .line 747
    .end local v2    # "$i$f$removeIf":I
    .end local v3    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v4    # "$i$f$forEachIndexed":I
    .end local v5    # "m$iv$iv":[J
    .end local v6    # "lastIndex$iv$iv":I
    .end local v7    # "i$iv$iv":I
    .end local v8    # "slot$iv$iv":J
    .restart local v32    # "$i$f$removeIf":I
    .restart local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .restart local v35    # "$i$f$forEachIndexed":I
    .restart local v36    # "m$iv$iv":[J
    .restart local v43    # "i$iv$iv":I
    .restart local v46    # "lastIndex$iv$iv":I
    :goto_8
    move/from16 v7, v43

    move/from16 v6, v46

    .end local v43    # "i$iv$iv":I
    .end local v46    # "lastIndex$iv$iv":I
    .restart local v6    # "lastIndex$iv$iv":I
    .restart local v7    # "i$iv$iv":I
    if-eq v7, v6, :cond_e

    add-int/lit8 v7, v7, 0x1

    move/from16 v2, v32

    move-object/from16 v3, v34

    move/from16 v4, v35

    move-object/from16 v5, v36

    goto/16 :goto_0

    .end local v32    # "$i$f$removeIf":I
    .end local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v35    # "$i$f$forEachIndexed":I
    .end local v36    # "m$iv$iv":[J
    .restart local v2    # "$i$f$removeIf":I
    .restart local v3    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .restart local v4    # "$i$f$forEachIndexed":I
    .restart local v5    # "m$iv$iv":[J
    :cond_d
    move-object/from16 v15, p1

    move/from16 v32, v2

    move-object/from16 v34, v3

    move/from16 v35, v4

    move-object/from16 v36, v5

    .line 792
    .end local v2    # "$i$f$removeIf":I
    .end local v3    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v4    # "$i$f$forEachIndexed":I
    .end local v5    # "m$iv$iv":[J
    .end local v7    # "i$iv$iv":I
    .restart local v32    # "$i$f$removeIf":I
    .restart local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .restart local v35    # "$i$f$forEachIndexed":I
    .restart local v36    # "m$iv$iv":[J
    :cond_e
    nop

    .line 793
    .end local v6    # "lastIndex$iv$iv":I
    .end local v34    # "this_$iv$iv":Landroidx/collection/ScatterMap;
    .end local v35    # "$i$f$forEachIndexed":I
    .end local v36    # "m$iv$iv":[J
    :cond_f
    nop

    .line 544
    .end local v1    # "this_$iv":Landroidx/collection/MutableScatterMap;
    .end local v32    # "$i$f$removeIf":I
    return-void
.end method

.method public final rereadDerivedState(Landroidx/compose/runtime/DerivedState;)V
    .locals 30
    .param p1, "derivedState"    # Landroidx/compose/runtime/DerivedState;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroidx/compose/runtime/DerivedState<",
            "*>;)V"
        }
    .end annotation

    .line 627
    move-object/from16 v0, p0

    move-object/from16 v1, p1

    iget-object v2, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->scopeToValues:Landroidx/collection/MutableScatterMap;

    .line 628
    .local v2, "scopeToValues":Landroidx/collection/MutableScatterMap;
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->currentSnapshot()Landroidx/compose/runtime/snapshots/Snapshot;

    move-result-object v3

    invoke-virtual {v3}, Landroidx/compose/runtime/snapshots/Snapshot;->getId()I

    move-result v3

    .line 630
    .local v3, "token":I
    iget-object v4, v0, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->valueToScopes:Landroidx/compose/runtime/collection/ScopeMap;

    .local v4, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    const/4 v5, 0x0

    .line 1096
    .local v5, "$i$f$forEachScopeOf":I
    invoke-virtual {v4}, Landroidx/compose/runtime/collection/ScopeMap;->getMap()Landroidx/collection/MutableScatterMap;

    move-result-object v6

    invoke-virtual {v6, v1}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v6

    .line 1097
    .local v6, "value$iv":Ljava/lang/Object;
    if-eqz v6, :cond_a

    .line 1098
    instance-of v7, v6, Landroidx/collection/MutableScatterSet;

    if-eqz v7, :cond_8

    .line 1100
    move-object v7, v6

    check-cast v7, Landroidx/collection/MutableScatterSet;

    check-cast v7, Landroidx/collection/ScatterSet;

    .local v7, "this_$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v11, 0x0

    .line 1101
    .local v11, "$i$f$forEach":I
    nop

    .line 1102
    iget-object v12, v7, Landroidx/collection/ScatterSet;->elements:[Ljava/lang/Object;

    .line 1104
    .local v12, "k$iv$iv":[Ljava/lang/Object;
    move-object v13, v7

    .local v13, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    const/4 v14, 0x0

    .line 1105
    .local v14, "$i$f$forEachIndex":I
    nop

    .line 1106
    iget-object v15, v13, Landroidx/collection/ScatterSet;->metadata:[J

    .line 1107
    .local v15, "m$iv$iv$iv":[J
    array-length v8, v15

    add-int/lit8 v8, v8, -0x2

    .line 1109
    .local v8, "lastIndex$iv$iv$iv":I
    const/4 v9, 0x0

    .local v9, "i$iv$iv$iv":I
    if-gt v9, v8, :cond_5

    .line 1110
    :goto_0
    aget-wide v16, v15, v9

    .line 1111
    .local v16, "slot$iv$iv$iv":J
    move-wide/from16 v18, v16

    .local v18, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    const/16 v20, 0x0

    .line 1112
    .local v20, "$i$f$maskEmptyOrDeleted":I
    move/from16 v21, v11

    move-wide/from16 v10, v18

    move-object/from16 v18, v4

    move/from16 v19, v5

    .end local v4    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v5    # "$i$f$forEachScopeOf":I
    .end local v11    # "$i$f$forEach":I
    .local v10, "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .local v18, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v19, "$i$f$forEachScopeOf":I
    .local v21, "$i$f$forEach":I
    not-long v4, v10

    const/16 v22, 0x7

    shl-long v4, v4, v22

    and-long/2addr v4, v10

    const-wide v22, -0x7f7f7f7f7f7f7f80L    # -2.937446524422997E-306

    and-long v4, v4, v22

    .line 1111
    .end local v10    # "$this$maskEmptyOrDeleted$iv$iv$iv$iv":J
    .end local v20    # "$i$f$maskEmptyOrDeleted":I
    cmp-long v4, v4, v22

    if-eqz v4, :cond_4

    .line 1113
    sub-int v4, v9, v8

    not-int v4, v4

    ushr-int/lit8 v4, v4, 0x1f

    const/16 v5, 0x8

    rsub-int/lit8 v4, v4, 0x8

    .line 1114
    .local v4, "bitCount$iv$iv$iv":I
    const/4 v10, 0x0

    .local v10, "j$iv$iv$iv":I
    :goto_1
    if-ge v10, v4, :cond_3

    .line 1115
    const-wide/16 v22, 0xff

    and-long v22, v16, v22

    .local v22, "value$iv$iv$iv$iv":J
    const/4 v11, 0x0

    .line 1116
    .local v11, "$i$f$isFull":I
    const-wide/16 v24, 0x80

    cmp-long v20, v22, v24

    if-gez v20, :cond_0

    const/4 v11, 0x1

    goto :goto_2

    :cond_0
    const/4 v11, 0x0

    .line 1115
    .end local v11    # "$i$f$isFull":I
    .end local v22    # "value$iv$iv$iv$iv":J
    :goto_2
    if-eqz v11, :cond_2

    .line 1117
    shl-int/lit8 v11, v9, 0x3

    add-int/2addr v11, v10

    .line 1118
    .local v11, "index$iv$iv$iv":I
    move/from16 v20, v11

    .local v20, "index$iv$iv":I
    const/16 v22, 0x0

    .line 1119
    .local v22, "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    aget-object v5, v12, v20

    .local v5, "scope":Ljava/lang/Object;
    const/16 v24, 0x0

    .line 631
    .local v24, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$rereadDerivedState$1":I
    nop

    .line 632
    nop

    .line 633
    nop

    .line 634
    nop

    .line 635
    invoke-virtual {v2, v5}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v25

    check-cast v25, Landroidx/collection/MutableObjectIntMap;

    if-nez v25, :cond_1

    move-object/from16 v26, v7

    .end local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v26, "this_$iv$iv":Landroidx/collection/ScatterSet;
    new-instance v7, Landroidx/collection/MutableObjectIntMap;

    move/from16 v27, v11

    move-object/from16 v28, v12

    move-object/from16 v29, v13

    const/4 v11, 0x0

    const/4 v12, 0x0

    const/4 v13, 0x1

    .end local v11    # "index$iv$iv$iv":I
    .end local v12    # "k$iv$iv":[Ljava/lang/Object;
    .end local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v27, "index$iv$iv$iv":I
    .local v28, "k$iv$iv":[Ljava/lang/Object;
    .local v29, "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    invoke-direct {v7, v12, v13, v11}, Landroidx/collection/MutableObjectIntMap;-><init>(IILkotlin/jvm/internal/DefaultConstructorMarker;)V

    move-object v11, v7

    .local v11, "it":Landroidx/collection/MutableObjectIntMap;
    const/4 v12, 0x0

    .line 636
    .local v12, "$i$a$-also-SnapshotStateObserver$ObservedScopeMap$rereadDerivedState$1$1":I
    invoke-virtual {v2, v5, v11}, Landroidx/collection/MutableScatterMap;->set(Ljava/lang/Object;Ljava/lang/Object;)V

    .line 637
    nop

    .end local v11    # "it":Landroidx/collection/MutableObjectIntMap;
    .end local v12    # "$i$a$-also-SnapshotStateObserver$ObservedScopeMap$rereadDerivedState$1$1":I
    sget-object v11, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    .line 635
    goto :goto_3

    .end local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v27    # "index$iv$iv$iv":I
    .end local v28    # "k$iv$iv":[Ljava/lang/Object;
    .end local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v11, "index$iv$iv$iv":I
    .local v12, "k$iv$iv":[Ljava/lang/Object;
    .restart local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    :cond_1
    move-object/from16 v26, v7

    move/from16 v27, v11

    move-object/from16 v28, v12

    move-object/from16 v29, v13

    .end local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v11    # "index$iv$iv$iv":I
    .end local v12    # "k$iv$iv":[Ljava/lang/Object;
    .end local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v27    # "index$iv$iv$iv":I
    .restart local v28    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    move-object/from16 v7, v25

    .line 631
    :goto_3
    invoke-direct {v0, v1, v3, v5, v7}, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->recordRead(Ljava/lang/Object;ILjava/lang/Object;Landroidx/collection/MutableObjectIntMap;)V

    .line 639
    nop

    .line 1119
    .end local v5    # "scope":Ljava/lang/Object;
    .end local v24    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$rereadDerivedState$1":I
    nop

    .line 1120
    nop

    .line 1118
    .end local v20    # "index$iv$iv":I
    .end local v22    # "$i$a$-forEachIndex-ScatterSet$forEach$2$iv$iv":I
    goto :goto_4

    .line 1115
    .end local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v27    # "index$iv$iv$iv":I
    .end local v28    # "k$iv$iv":[Ljava/lang/Object;
    .end local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v12    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    :cond_2
    move-object/from16 v26, v7

    move-object/from16 v28, v12

    move-object/from16 v29, v13

    .line 1121
    .end local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v12    # "k$iv$iv":[Ljava/lang/Object;
    .end local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v28    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    :goto_4
    const/16 v5, 0x8

    shr-long v16, v16, v5

    .line 1114
    add-int/lit8 v10, v10, 0x1

    move-object/from16 v7, v26

    move-object/from16 v12, v28

    move-object/from16 v13, v29

    goto :goto_1

    .end local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v28    # "k$iv$iv":[Ljava/lang/Object;
    .end local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v12    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    :cond_3
    move-object/from16 v26, v7

    move-object/from16 v28, v12

    move-object/from16 v29, v13

    .line 1123
    .end local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v10    # "j$iv$iv$iv":I
    .end local v12    # "k$iv$iv":[Ljava/lang/Object;
    .end local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v28    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    if-ne v4, v5, :cond_7

    goto :goto_5

    .line 1111
    .end local v4    # "bitCount$iv$iv$iv":I
    .end local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v28    # "k$iv$iv":[Ljava/lang/Object;
    .end local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v12    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    :cond_4
    move-object/from16 v26, v7

    move-object/from16 v28, v12

    move-object/from16 v29, v13

    .line 1109
    .end local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v12    # "k$iv$iv":[Ljava/lang/Object;
    .end local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .end local v16    # "slot$iv$iv$iv":J
    .restart local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v28    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    :goto_5
    if-eq v9, v8, :cond_6

    add-int/lit8 v9, v9, 0x1

    move-object/from16 v4, v18

    move/from16 v5, v19

    move/from16 v11, v21

    move-object/from16 v7, v26

    move-object/from16 v12, v28

    move-object/from16 v13, v29

    goto/16 :goto_0

    .end local v18    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v19    # "$i$f$forEachScopeOf":I
    .end local v21    # "$i$f$forEach":I
    .end local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v28    # "k$iv$iv":[Ljava/lang/Object;
    .end local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .local v4, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v5, "$i$f$forEachScopeOf":I
    .restart local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .local v11, "$i$f$forEach":I
    .restart local v12    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    :cond_5
    move-object/from16 v18, v4

    move/from16 v19, v5

    move-object/from16 v26, v7

    move/from16 v21, v11

    move-object/from16 v28, v12

    move-object/from16 v29, v13

    .line 1126
    .end local v4    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v5    # "$i$f$forEachScopeOf":I
    .end local v7    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v9    # "i$iv$iv$iv":I
    .end local v11    # "$i$f$forEach":I
    .end local v12    # "k$iv$iv":[Ljava/lang/Object;
    .end local v13    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v18    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v19    # "$i$f$forEachScopeOf":I
    .restart local v21    # "$i$f$forEach":I
    .restart local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .restart local v28    # "k$iv$iv":[Ljava/lang/Object;
    .restart local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    :cond_6
    nop

    .line 1127
    .end local v8    # "lastIndex$iv$iv$iv":I
    .end local v14    # "$i$f$forEachIndex":I
    .end local v15    # "m$iv$iv$iv":[J
    .end local v29    # "this_$iv$iv$iv":Landroidx/collection/ScatterSet;
    :cond_7
    nop

    .end local v21    # "$i$f$forEach":I
    .end local v26    # "this_$iv$iv":Landroidx/collection/ScatterSet;
    .end local v28    # "k$iv$iv":[Ljava/lang/Object;
    goto :goto_6

    .line 1128
    .end local v18    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v19    # "$i$f$forEachScopeOf":I
    .restart local v4    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v5    # "$i$f$forEachScopeOf":I
    :cond_8
    move-object/from16 v18, v4

    move/from16 v19, v5

    .end local v4    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v5    # "$i$f$forEachScopeOf":I
    .restart local v18    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v19    # "$i$f$forEachScopeOf":I
    move-object v4, v6

    .local v4, "scope":Ljava/lang/Object;
    const/4 v5, 0x0

    .line 631
    .local v5, "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$rereadDerivedState$1":I
    nop

    .line 632
    nop

    .line 633
    nop

    .line 634
    nop

    .line 635
    invoke-virtual {v2, v4}, Landroidx/collection/MutableScatterMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v7

    check-cast v7, Landroidx/collection/MutableObjectIntMap;

    if-nez v7, :cond_9

    new-instance v7, Landroidx/collection/MutableObjectIntMap;

    const/4 v8, 0x0

    const/4 v9, 0x0

    const/4 v10, 0x1

    invoke-direct {v7, v9, v10, v8}, Landroidx/collection/MutableObjectIntMap;-><init>(IILkotlin/jvm/internal/DefaultConstructorMarker;)V

    move-object v8, v7

    .local v8, "it":Landroidx/collection/MutableObjectIntMap;
    const/4 v9, 0x0

    .line 636
    .local v9, "$i$a$-also-SnapshotStateObserver$ObservedScopeMap$rereadDerivedState$1$1":I
    invoke-virtual {v2, v4, v8}, Landroidx/collection/MutableScatterMap;->set(Ljava/lang/Object;Ljava/lang/Object;)V

    .line 637
    nop

    .end local v8    # "it":Landroidx/collection/MutableObjectIntMap;
    .end local v9    # "$i$a$-also-SnapshotStateObserver$ObservedScopeMap$rereadDerivedState$1$1":I
    sget-object v8, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    .line 635
    nop

    .line 631
    :cond_9
    invoke-direct {v0, v1, v3, v4, v7}, Landroidx/compose/runtime/snapshots/SnapshotStateObserver$ObservedScopeMap;->recordRead(Ljava/lang/Object;ILjava/lang/Object;Landroidx/collection/MutableObjectIntMap;)V

    .line 639
    nop

    .line 1128
    .end local v4    # "scope":Ljava/lang/Object;
    .end local v5    # "$i$a$-forEachScopeOf-SnapshotStateObserver$ObservedScopeMap$rereadDerivedState$1":I
    goto :goto_6

    .line 1097
    .end local v18    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v19    # "$i$f$forEachScopeOf":I
    .local v4, "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .local v5, "$i$f$forEachScopeOf":I
    :cond_a
    move-object/from16 v18, v4

    move/from16 v19, v5

    .line 1131
    .end local v4    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v5    # "$i$f$forEachScopeOf":I
    .end local v6    # "value$iv":Ljava/lang/Object;
    .restart local v18    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .restart local v19    # "$i$f$forEachScopeOf":I
    :goto_6
    nop

    .line 640
    .end local v18    # "this_$iv":Landroidx/compose/runtime/collection/ScopeMap;
    .end local v19    # "$i$f$forEachScopeOf":I
    return-void
.end method
