.class final Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;
.super Lkotlin/jvm/internal/Lambda;
.source "BottomSheetScaffold.kt"

# interfaces
.implements Lkotlin/jvm/functions/Function2;


# annotations
.annotation system Ldalvik/annotation/EnclosingMethod;
    value = Landroidx/compose/material/BottomSheetScaffoldKt;->BottomSheetScaffoldLayout-KCBPh4w(Lkotlin/jvm/functions/Function2;Lkotlin/jvm/functions/Function3;Lkotlin/jvm/functions/Function3;Lkotlin/jvm/functions/Function2;Lkotlin/jvm/functions/Function2;FILkotlin/jvm/functions/Function0;Landroidx/compose/material/BottomSheetState;Landroidx/compose/runtime/Composer;I)V
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x18
    name = null
.end annotation

.annotation system Ldalvik/annotation/Signature;
    value = {
        "Lkotlin/jvm/internal/Lambda;",
        "Lkotlin/jvm/functions/Function2<",
        "Landroidx/compose/ui/layout/SubcomposeMeasureScope;",
        "Landroidx/compose/ui/unit/Constraints;",
        "Landroidx/compose/ui/layout/MeasureResult;",
        ">;"
    }
.end annotation

.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nBottomSheetScaffold.kt\nKotlin\n*S Kotlin\n*F\n+ 1 BottomSheetScaffold.kt\nandroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1\n+ 2 ListUtils.kt\nandroidx/compose/ui/util/ListUtilsKt\n*L\n1#1,812:1\n151#2,3:813\n33#2,4:816\n154#2,2:820\n38#2:822\n156#2:823\n151#2,3:824\n33#2,4:827\n154#2,2:831\n38#2:833\n156#2:834\n171#2,13:835\n151#2,3:848\n33#2,4:851\n154#2,2:855\n38#2:857\n156#2:858\n151#2,3:859\n33#2,4:862\n154#2,2:866\n38#2:868\n156#2:869\n171#2,13:870\n171#2,13:883\n151#2,3:896\n33#2,4:899\n154#2,2:903\n38#2:905\n156#2:906\n171#2,13:907\n171#2,13:920\n*S KotlinDebug\n*F\n+ 1 BottomSheetScaffold.kt\nandroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1\n*L\n695#1:813,3\n695#1:816,4\n695#1:820,2\n695#1:822\n695#1:823\n699#1:824,3\n699#1:827,4\n699#1:831,2\n699#1:833\n699#1:834\n701#1:835,13\n706#1:848,3\n706#1:851,4\n706#1:855,2\n706#1:857\n706#1:858\n709#1:859,3\n709#1:862,4\n709#1:866,2\n709#1:868\n709#1:869\n713#1:870,13\n714#1:883,13\n717#1:896,3\n717#1:899,4\n717#1:903,2\n717#1:905\n717#1:906\n718#1:907,13\n719#1:920,13\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000\u0014\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0002\u0010\u0000\u001a\u00020\u0001*\u00020\u00022\u0006\u0010\u0003\u001a\u00020\u0004H\n\u00a2\u0006\u0004\u0008\u0005\u0010\u0006"
    }
    d2 = {
        "<anonymous>",
        "Landroidx/compose/ui/layout/MeasureResult;",
        "Landroidx/compose/ui/layout/SubcomposeMeasureScope;",
        "constraints",
        "Landroidx/compose/ui/unit/Constraints;",
        "invoke-0kLqBqw",
        "(Landroidx/compose/ui/layout/SubcomposeMeasureScope;J)Landroidx/compose/ui/layout/MeasureResult;"
    }
    k = 0x3
    mv = {
        0x1,
        0x8,
        0x0
    }
    xi = 0x30
.end annotation


# instance fields
.field final synthetic $body:Lkotlin/jvm/functions/Function3;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function3<",
            "Landroidx/compose/foundation/layout/PaddingValues;",
            "Landroidx/compose/runtime/Composer;",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field final synthetic $bottomSheet:Lkotlin/jvm/functions/Function3;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function3<",
            "Ljava/lang/Integer;",
            "Landroidx/compose/runtime/Composer;",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field final synthetic $floatingActionButton:Lkotlin/jvm/functions/Function2;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function2<",
            "Landroidx/compose/runtime/Composer;",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field final synthetic $floatingActionButtonPosition:I

.field final synthetic $sheetOffset:Lkotlin/jvm/functions/Function0;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function0<",
            "Ljava/lang/Float;",
            ">;"
        }
    .end annotation
.end field

.field final synthetic $sheetPeekHeight:F

.field final synthetic $sheetState:Landroidx/compose/material/BottomSheetState;

.field final synthetic $snackbarHost:Lkotlin/jvm/functions/Function2;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function2<",
            "Landroidx/compose/runtime/Composer;",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field final synthetic $topBar:Lkotlin/jvm/functions/Function2;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function2<",
            "Landroidx/compose/runtime/Composer;",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method constructor <init>(Lkotlin/jvm/functions/Function2;Lkotlin/jvm/functions/Function2;Lkotlin/jvm/functions/Function2;Lkotlin/jvm/functions/Function3;Lkotlin/jvm/functions/Function3;FLkotlin/jvm/functions/Function0;ILandroidx/compose/material/BottomSheetState;)V
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Lkotlin/jvm/functions/Function2<",
            "-",
            "Landroidx/compose/runtime/Composer;",
            "-",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;",
            "Lkotlin/jvm/functions/Function2<",
            "-",
            "Landroidx/compose/runtime/Composer;",
            "-",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;",
            "Lkotlin/jvm/functions/Function2<",
            "-",
            "Landroidx/compose/runtime/Composer;",
            "-",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;",
            "Lkotlin/jvm/functions/Function3<",
            "-",
            "Ljava/lang/Integer;",
            "-",
            "Landroidx/compose/runtime/Composer;",
            "-",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;",
            "Lkotlin/jvm/functions/Function3<",
            "-",
            "Landroidx/compose/foundation/layout/PaddingValues;",
            "-",
            "Landroidx/compose/runtime/Composer;",
            "-",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;F",
            "Lkotlin/jvm/functions/Function0<",
            "Ljava/lang/Float;",
            ">;I",
            "Landroidx/compose/material/BottomSheetState;",
            ")V"
        }
    .end annotation

    iput-object p1, p0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$topBar:Lkotlin/jvm/functions/Function2;

    iput-object p2, p0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$floatingActionButton:Lkotlin/jvm/functions/Function2;

    iput-object p3, p0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$snackbarHost:Lkotlin/jvm/functions/Function2;

    iput-object p4, p0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$bottomSheet:Lkotlin/jvm/functions/Function3;

    iput-object p5, p0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$body:Lkotlin/jvm/functions/Function3;

    iput p6, p0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$sheetPeekHeight:F

    iput-object p7, p0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$sheetOffset:Lkotlin/jvm/functions/Function0;

    iput p8, p0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$floatingActionButtonPosition:I

    iput-object p9, p0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$sheetState:Landroidx/compose/material/BottomSheetState;

    const/4 v0, 0x2

    invoke-direct {p0, v0}, Lkotlin/jvm/internal/Lambda;-><init>(I)V

    return-void
.end method


# virtual methods
.method public bridge synthetic invoke(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;
    .locals 3
    .param p1, "p1"    # Ljava/lang/Object;
    .param p2, "p2"    # Ljava/lang/Object;

    .line 688
    move-object v0, p1

    check-cast v0, Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    move-object v1, p2

    check-cast v1, Landroidx/compose/ui/unit/Constraints;

    invoke-virtual {v1}, Landroidx/compose/ui/unit/Constraints;->unbox-impl()J

    move-result-wide v1

    invoke-virtual {p0, v0, v1, v2}, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->invoke-0kLqBqw(Landroidx/compose/ui/layout/SubcomposeMeasureScope;J)Landroidx/compose/ui/layout/MeasureResult;

    move-result-object v0

    return-object v0
.end method

.method public final invoke-0kLqBqw(Landroidx/compose/ui/layout/SubcomposeMeasureScope;J)Landroidx/compose/ui/layout/MeasureResult;
    .locals 34
    .param p1, "$this$SubcomposeLayout"    # Landroidx/compose/ui/layout/SubcomposeMeasureScope;
    .param p2, "constraints"    # J

    .line 689
    move-object/from16 v0, p0

    move-object/from16 v14, p1

    invoke-static/range {p2 .. p3}, Landroidx/compose/ui/unit/Constraints;->getMaxWidth-impl(J)I

    move-result v25

    .line 690
    .local v25, "layoutWidth":I
    invoke-static/range {p2 .. p3}, Landroidx/compose/ui/unit/Constraints;->getMaxHeight-impl(J)I

    move-result v12

    .line 691
    .local v12, "layoutHeight":I
    const/4 v3, 0x0

    const/4 v4, 0x0

    const/4 v5, 0x0

    const/4 v6, 0x0

    const/16 v7, 0xa

    const/4 v8, 0x0

    move-wide/from16 v1, p2

    invoke-static/range {v1 .. v8}, Landroidx/compose/ui/unit/Constraints;->copy-Zbe2FdA$default(JIIIIILjava/lang/Object;)J

    move-result-wide v4

    .line 693
    .local v4, "looseConstraints":J
    sget-object v1, Landroidx/compose/material/BottomSheetScaffoldLayoutSlot;->Sheet:Landroidx/compose/material/BottomSheetScaffoldLayoutSlot;

    new-instance v2, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$sheetPlaceables$1;

    iget-object v3, v0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$bottomSheet:Lkotlin/jvm/functions/Function3;

    invoke-direct {v2, v3, v12}, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$sheetPlaceables$1;-><init>(Lkotlin/jvm/functions/Function3;I)V

    const v3, -0xa2ca027

    const/4 v6, 0x1

    invoke-static {v3, v6, v2}, Landroidx/compose/runtime/internal/ComposableLambdaKt;->composableLambdaInstance(IZLjava/lang/Object;)Landroidx/compose/runtime/internal/ComposableLambda;

    move-result-object v2

    check-cast v2, Lkotlin/jvm/functions/Function2;

    invoke-interface {v14, v1, v2}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->subcompose(Ljava/lang/Object;Lkotlin/jvm/functions/Function2;)Ljava/util/List;

    move-result-object v1

    .line 695
    nop

    .local v1, "$this$fastMap$iv":Ljava/util/List;
    const/4 v2, 0x0

    .line 813
    .local v2, "$i$f$fastMap":I
    nop

    .line 814
    new-instance v3, Ljava/util/ArrayList;

    invoke-interface {v1}, Ljava/util/List;->size()I

    move-result v7

    invoke-direct {v3, v7}, Ljava/util/ArrayList;-><init>(I)V

    .line 815
    .local v3, "target$iv":Ljava/util/ArrayList;
    move-object v7, v1

    .local v7, "$this$fastForEach$iv$iv":Ljava/util/List;
    const/4 v8, 0x0

    .line 816
    .local v8, "$i$f$fastForEach":I
    nop

    .line 817
    const/4 v9, 0x0

    .local v9, "index$iv$iv":I
    invoke-interface {v7}, Ljava/util/List;->size()I

    move-result v10

    :goto_0
    if-ge v9, v10, :cond_0

    .line 818
    invoke-interface {v7, v9}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v11

    .line 819
    .local v11, "item$iv$iv":Ljava/lang/Object;
    move-object v13, v11

    .local v13, "it$iv":Ljava/lang/Object;
    const/4 v15, 0x0

    .line 820
    .local v15, "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    move-object v6, v3

    check-cast v6, Ljava/util/Collection;

    move-object/from16 v16, v1

    .end local v1    # "$this$fastMap$iv":Ljava/util/List;
    .local v16, "$this$fastMap$iv":Ljava/util/List;
    move-object v1, v13

    check-cast v1, Landroidx/compose/ui/layout/Measurable;

    .local v1, "it":Landroidx/compose/ui/layout/Measurable;
    const/16 v17, 0x0

    .line 695
    .local v17, "$i$a$-fastMap-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$sheetPlaceables$2":I
    invoke-interface {v1, v4, v5}, Landroidx/compose/ui/layout/Measurable;->measure-BRTryo0(J)Landroidx/compose/ui/layout/Placeable;

    move-result-object v1

    .line 820
    .end local v1    # "it":Landroidx/compose/ui/layout/Measurable;
    .end local v17    # "$i$a$-fastMap-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$sheetPlaceables$2":I
    invoke-interface {v6, v1}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 821
    nop

    .line 819
    .end local v13    # "it$iv":Ljava/lang/Object;
    .end local v15    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    nop

    .line 817
    .end local v11    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v9, v9, 0x1

    move-object/from16 v1, v16

    const/4 v6, 0x1

    goto :goto_0

    .end local v16    # "$this$fastMap$iv":Ljava/util/List;
    .local v1, "$this$fastMap$iv":Ljava/util/List;
    :cond_0
    move-object/from16 v16, v1

    .line 822
    .end local v1    # "$this$fastMap$iv":Ljava/util/List;
    .end local v9    # "index$iv$iv":I
    .restart local v16    # "$this$fastMap$iv":Ljava/util/List;
    nop

    .line 823
    .end local v7    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v8    # "$i$f$fastForEach":I
    move-object v15, v3

    check-cast v15, Ljava/util/List;

    .line 695
    .end local v2    # "$i$f$fastMap":I
    .end local v3    # "target$iv":Ljava/util/ArrayList;
    .end local v16    # "$this$fastMap$iv":Ljava/util/List;
    nop

    .line 693
    nop

    .line 697
    .local v15, "sheetPlaceables":Ljava/util/List;
    iget-object v1, v0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$topBar:Lkotlin/jvm/functions/Function2;

    if-eqz v1, :cond_2

    move-object v3, v1

    .local v3, "it":Lkotlin/jvm/functions/Function2;
    const/4 v6, 0x0

    .line 698
    .local v6, "$i$a$-let-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$topBarPlaceables$1":I
    sget-object v7, Landroidx/compose/material/BottomSheetScaffoldLayoutSlot;->TopBar:Landroidx/compose/material/BottomSheetScaffoldLayoutSlot;

    invoke-interface {v14, v7, v1}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->subcompose(Ljava/lang/Object;Lkotlin/jvm/functions/Function2;)Ljava/util/List;

    move-result-object v1

    .line 699
    nop

    .restart local v1    # "$this$fastMap$iv":Ljava/util/List;
    const/4 v7, 0x0

    .line 824
    .local v7, "$i$f$fastMap":I
    nop

    .line 825
    new-instance v8, Ljava/util/ArrayList;

    invoke-interface {v1}, Ljava/util/List;->size()I

    move-result v9

    invoke-direct {v8, v9}, Ljava/util/ArrayList;-><init>(I)V

    .line 826
    .local v8, "target$iv":Ljava/util/ArrayList;
    move-object v9, v1

    .local v9, "$this$fastForEach$iv$iv":Ljava/util/List;
    const/4 v10, 0x0

    .line 827
    .local v10, "$i$f$fastForEach":I
    nop

    .line 828
    const/4 v11, 0x0

    .local v11, "index$iv$iv":I
    invoke-interface {v9}, Ljava/util/List;->size()I

    move-result v13

    :goto_1
    if-ge v11, v13, :cond_1

    .line 829
    invoke-interface {v9, v11}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v16

    .line 830
    .local v16, "item$iv$iv":Ljava/lang/Object;
    move-object/from16 v17, v16

    .local v17, "it$iv":Ljava/lang/Object;
    const/16 v18, 0x0

    .line 831
    .local v18, "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    move-object v2, v8

    check-cast v2, Ljava/util/Collection;

    move-object/from16 v19, v1

    .end local v1    # "$this$fastMap$iv":Ljava/util/List;
    .local v19, "$this$fastMap$iv":Ljava/util/List;
    move-object/from16 v1, v17

    check-cast v1, Landroidx/compose/ui/layout/Measurable;

    .local v1, "it":Landroidx/compose/ui/layout/Measurable;
    const/16 v20, 0x0

    .line 699
    .local v20, "$i$a$-fastMap-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$topBarPlaceables$1$1":I
    invoke-interface {v1, v4, v5}, Landroidx/compose/ui/layout/Measurable;->measure-BRTryo0(J)Landroidx/compose/ui/layout/Placeable;

    move-result-object v1

    .line 831
    .end local v1    # "it":Landroidx/compose/ui/layout/Measurable;
    .end local v20    # "$i$a$-fastMap-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$topBarPlaceables$1$1":I
    invoke-interface {v2, v1}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 832
    nop

    .line 830
    .end local v17    # "it$iv":Ljava/lang/Object;
    .end local v18    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    nop

    .line 828
    .end local v16    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v11, v11, 0x1

    move-object/from16 v1, v19

    goto :goto_1

    .end local v19    # "$this$fastMap$iv":Ljava/util/List;
    .local v1, "$this$fastMap$iv":Ljava/util/List;
    :cond_1
    move-object/from16 v19, v1

    .line 833
    .end local v1    # "$this$fastMap$iv":Ljava/util/List;
    .end local v11    # "index$iv$iv":I
    .restart local v19    # "$this$fastMap$iv":Ljava/util/List;
    nop

    .line 834
    .end local v9    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v10    # "$i$f$fastForEach":I
    move-object v1, v8

    check-cast v1, Ljava/util/List;

    .line 699
    .end local v7    # "$i$f$fastMap":I
    .end local v8    # "target$iv":Ljava/util/ArrayList;
    .end local v19    # "$this$fastMap$iv":Ljava/util/List;
    nop

    .line 697
    .end local v3    # "it":Lkotlin/jvm/functions/Function2;
    .end local v6    # "$i$a$-let-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$topBarPlaceables$1":I
    goto :goto_2

    :cond_2
    const/4 v1, 0x0

    :goto_2
    move-object/from16 v27, v1

    .line 701
    .local v27, "topBarPlaceables":Ljava/util/List;
    const/4 v1, 0x0

    if-eqz v27, :cond_6

    move-object/from16 v2, v27

    .local v2, "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v3, 0x0

    .line 835
    .local v3, "$i$f$fastMaxBy":I
    nop

    .line 836
    invoke-interface {v2}, Ljava/util/List;->isEmpty()Z

    move-result v6

    if-eqz v6, :cond_3

    const/4 v6, 0x0

    goto :goto_4

    .line 837
    :cond_3
    invoke-interface {v2, v1}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v6

    .line 838
    .local v6, "maxElem$iv":Ljava/lang/Object;
    move-object v7, v6

    check-cast v7, Landroidx/compose/ui/layout/Placeable;

    .local v7, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v8, 0x0

    .line 701
    .local v8, "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$topBarHeight$1":I
    invoke-virtual {v7}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v7

    .line 838
    .end local v7    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v8    # "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$topBarHeight$1":I
    nop

    .line 839
    .local v7, "maxValue$iv":I
    const/4 v8, 0x1

    .local v8, "i$iv":I
    invoke-static {v2}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v9

    if-gt v8, v9, :cond_5

    .line 840
    :goto_3
    invoke-interface {v2, v8}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v10

    .line 841
    .local v10, "e$iv":Ljava/lang/Object;
    move-object v11, v10

    check-cast v11, Landroidx/compose/ui/layout/Placeable;

    .local v11, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v13, 0x0

    .line 701
    .local v13, "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$topBarHeight$1":I
    invoke-virtual {v11}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v11

    .line 841
    .end local v11    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v13    # "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$topBarHeight$1":I
    nop

    .line 842
    .local v11, "v$iv":I
    if-ge v7, v11, :cond_4

    .line 843
    move-object v6, v10

    .line 844
    move v7, v11

    .line 839
    .end local v10    # "e$iv":Ljava/lang/Object;
    .end local v11    # "v$iv":I
    :cond_4
    if-eq v8, v9, :cond_5

    add-int/lit8 v8, v8, 0x1

    goto :goto_3

    .line 847
    .end local v8    # "i$iv":I
    :cond_5
    nop

    .line 701
    .end local v2    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v3    # "$i$f$fastMaxBy":I
    .end local v6    # "maxElem$iv":Ljava/lang/Object;
    .end local v7    # "maxValue$iv":I
    :goto_4
    check-cast v6, Landroidx/compose/ui/layout/Placeable;

    if-eqz v6, :cond_6

    invoke-virtual {v6}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v2

    goto :goto_5

    :cond_6
    move v2, v1

    :goto_5
    move/from16 v28, v2

    .line 703
    .local v28, "topBarHeight":I
    const/16 v18, 0x0

    const/16 v19, 0x0

    const/16 v20, 0x0

    sub-int v21, v12, v28

    const/16 v22, 0x7

    const/16 v23, 0x0

    move-wide/from16 v16, v4

    invoke-static/range {v16 .. v23}, Landroidx/compose/ui/unit/Constraints;->copy-Zbe2FdA$default(JIIIIILjava/lang/Object;)J

    move-result-wide v2

    .line 704
    .local v2, "bodyConstraints":J
    sget-object v6, Landroidx/compose/material/BottomSheetScaffoldLayoutSlot;->Body:Landroidx/compose/material/BottomSheetScaffoldLayoutSlot;

    new-instance v7, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$bodyPlaceables$1;

    iget-object v8, v0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$body:Lkotlin/jvm/functions/Function3;

    iget v9, v0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$sheetPeekHeight:F

    invoke-direct {v7, v8, v9}, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$bodyPlaceables$1;-><init>(Lkotlin/jvm/functions/Function3;F)V

    const v8, -0x1734e5d2

    const/4 v9, 0x1

    invoke-static {v8, v9, v7}, Landroidx/compose/runtime/internal/ComposableLambdaKt;->composableLambdaInstance(IZLjava/lang/Object;)Landroidx/compose/runtime/internal/ComposableLambda;

    move-result-object v7

    check-cast v7, Lkotlin/jvm/functions/Function2;

    invoke-interface {v14, v6, v7}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->subcompose(Ljava/lang/Object;Lkotlin/jvm/functions/Function2;)Ljava/util/List;

    move-result-object v6

    .line 706
    nop

    .local v6, "$this$fastMap$iv":Ljava/util/List;
    const/4 v7, 0x0

    .line 848
    .local v7, "$i$f$fastMap":I
    nop

    .line 849
    new-instance v8, Ljava/util/ArrayList;

    invoke-interface {v6}, Ljava/util/List;->size()I

    move-result v9

    invoke-direct {v8, v9}, Ljava/util/ArrayList;-><init>(I)V

    .line 850
    .local v8, "target$iv":Ljava/util/ArrayList;
    move-object v9, v6

    .restart local v9    # "$this$fastForEach$iv$iv":Ljava/util/List;
    const/4 v10, 0x0

    .line 851
    .local v10, "$i$f$fastForEach":I
    nop

    .line 852
    const/4 v11, 0x0

    .local v11, "index$iv$iv":I
    invoke-interface {v9}, Ljava/util/List;->size()I

    move-result v13

    :goto_6
    if-ge v11, v13, :cond_7

    .line 853
    invoke-interface {v9, v11}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v16

    .line 854
    .restart local v16    # "item$iv$iv":Ljava/lang/Object;
    move-object/from16 v17, v16

    .restart local v17    # "it$iv":Ljava/lang/Object;
    const/16 v18, 0x0

    .line 855
    .restart local v18    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    move-object v1, v8

    check-cast v1, Ljava/util/Collection;

    move-object/from16 v20, v6

    .end local v6    # "$this$fastMap$iv":Ljava/util/List;
    .local v20, "$this$fastMap$iv":Ljava/util/List;
    move-object/from16 v6, v17

    check-cast v6, Landroidx/compose/ui/layout/Measurable;

    .local v6, "it":Landroidx/compose/ui/layout/Measurable;
    const/16 v21, 0x0

    .line 706
    .local v21, "$i$a$-fastMap-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$bodyPlaceables$2":I
    invoke-interface {v6, v2, v3}, Landroidx/compose/ui/layout/Measurable;->measure-BRTryo0(J)Landroidx/compose/ui/layout/Placeable;

    move-result-object v6

    .line 855
    .end local v6    # "it":Landroidx/compose/ui/layout/Measurable;
    .end local v21    # "$i$a$-fastMap-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$bodyPlaceables$2":I
    invoke-interface {v1, v6}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 856
    nop

    .line 854
    .end local v17    # "it$iv":Ljava/lang/Object;
    .end local v18    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    nop

    .line 852
    .end local v16    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v11, v11, 0x1

    move-object/from16 v6, v20

    const/4 v1, 0x0

    goto :goto_6

    .end local v20    # "$this$fastMap$iv":Ljava/util/List;
    .local v6, "$this$fastMap$iv":Ljava/util/List;
    :cond_7
    move-object/from16 v20, v6

    .line 857
    .end local v6    # "$this$fastMap$iv":Ljava/util/List;
    .end local v11    # "index$iv$iv":I
    .restart local v20    # "$this$fastMap$iv":Ljava/util/List;
    nop

    .line 858
    .end local v9    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v10    # "$i$f$fastForEach":I
    move-object v13, v8

    check-cast v13, Ljava/util/List;

    .line 706
    .end local v7    # "$i$f$fastMap":I
    .end local v8    # "target$iv":Ljava/util/ArrayList;
    .end local v20    # "$this$fastMap$iv":Ljava/util/List;
    nop

    .line 704
    nop

    .line 708
    .local v13, "bodyPlaceables":Ljava/util/List;
    iget-object v1, v0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$floatingActionButton:Lkotlin/jvm/functions/Function2;

    if-eqz v1, :cond_9

    .local v1, "fab":Lkotlin/jvm/functions/Function2;
    const/4 v6, 0x0

    .line 709
    .local v6, "$i$a$-let-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabPlaceable$1":I
    sget-object v7, Landroidx/compose/material/BottomSheetScaffoldLayoutSlot;->Fab:Landroidx/compose/material/BottomSheetScaffoldLayoutSlot;

    invoke-interface {v14, v7, v1}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->subcompose(Ljava/lang/Object;Lkotlin/jvm/functions/Function2;)Ljava/util/List;

    move-result-object v7

    .local v7, "$this$fastMap$iv":Ljava/util/List;
    const/4 v8, 0x0

    .line 859
    .local v8, "$i$f$fastMap":I
    nop

    .line 860
    new-instance v9, Ljava/util/ArrayList;

    invoke-interface {v7}, Ljava/util/List;->size()I

    move-result v10

    invoke-direct {v9, v10}, Ljava/util/ArrayList;-><init>(I)V

    .line 861
    .local v9, "target$iv":Ljava/util/ArrayList;
    move-object v10, v7

    .local v10, "$this$fastForEach$iv$iv":Ljava/util/List;
    const/4 v11, 0x0

    .line 862
    .local v11, "$i$f$fastForEach":I
    nop

    .line 863
    const/16 v16, 0x0

    move-object/from16 v17, v1

    .end local v1    # "fab":Lkotlin/jvm/functions/Function2;
    .local v16, "index$iv$iv":I
    .local v17, "fab":Lkotlin/jvm/functions/Function2;
    invoke-interface {v10}, Ljava/util/List;->size()I

    move-result v1

    move-wide/from16 v20, v2

    move/from16 v2, v16

    .end local v16    # "index$iv$iv":I
    .local v2, "index$iv$iv":I
    .local v20, "bodyConstraints":J
    :goto_7
    if-ge v2, v1, :cond_8

    .line 864
    invoke-interface {v10, v2}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v3

    .line 865
    .local v3, "item$iv$iv":Ljava/lang/Object;
    move-object/from16 v16, v3

    .local v16, "it$iv":Ljava/lang/Object;
    const/16 v18, 0x0

    .line 866
    .restart local v18    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    move/from16 v22, v1

    move-object v1, v9

    check-cast v1, Ljava/util/Collection;

    move-object/from16 v23, v3

    .end local v3    # "item$iv$iv":Ljava/lang/Object;
    .local v23, "item$iv$iv":Ljava/lang/Object;
    move-object/from16 v3, v16

    check-cast v3, Landroidx/compose/ui/layout/Measurable;

    .local v3, "it":Landroidx/compose/ui/layout/Measurable;
    const/16 v24, 0x0

    .line 710
    .local v24, "$i$a$-fastMap-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabPlaceable$1$1":I
    invoke-interface {v3, v4, v5}, Landroidx/compose/ui/layout/Measurable;->measure-BRTryo0(J)Landroidx/compose/ui/layout/Placeable;

    move-result-object v3

    .line 866
    .end local v3    # "it":Landroidx/compose/ui/layout/Measurable;
    .end local v24    # "$i$a$-fastMap-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabPlaceable$1$1":I
    invoke-interface {v1, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 867
    nop

    .line 865
    .end local v16    # "it$iv":Ljava/lang/Object;
    .end local v18    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    nop

    .line 863
    .end local v23    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v2, v2, 0x1

    move/from16 v1, v22

    goto :goto_7

    .line 868
    .end local v2    # "index$iv$iv":I
    :cond_8
    nop

    .line 869
    .end local v10    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v11    # "$i$f$fastForEach":I
    move-object v1, v9

    check-cast v1, Ljava/util/List;

    .line 709
    .end local v7    # "$this$fastMap$iv":Ljava/util/List;
    .end local v8    # "$i$f$fastMap":I
    .end local v9    # "target$iv":Ljava/util/ArrayList;
    nop

    .line 708
    .end local v6    # "$i$a$-let-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabPlaceable$1":I
    .end local v17    # "fab":Lkotlin/jvm/functions/Function2;
    goto :goto_8

    .end local v20    # "bodyConstraints":J
    .local v2, "bodyConstraints":J
    :cond_9
    move-wide/from16 v20, v2

    .end local v2    # "bodyConstraints":J
    .restart local v20    # "bodyConstraints":J
    const/4 v1, 0x0

    :goto_8
    move-object/from16 v29, v1

    .line 713
    .local v29, "fabPlaceable":Ljava/util/List;
    if-eqz v29, :cond_d

    move-object/from16 v1, v29

    .local v1, "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v2, 0x0

    .line 870
    .local v2, "$i$f$fastMaxBy":I
    nop

    .line 871
    invoke-interface {v1}, Ljava/util/List;->isEmpty()Z

    move-result v3

    if-eqz v3, :cond_a

    const/4 v6, 0x0

    goto :goto_a

    .line 872
    :cond_a
    const/4 v3, 0x0

    invoke-interface {v1, v3}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v6

    .line 873
    .local v6, "maxElem$iv":Ljava/lang/Object;
    move-object v3, v6

    check-cast v3, Landroidx/compose/ui/layout/Placeable;

    .local v3, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v7, 0x0

    .line 713
    .local v7, "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabWidth$1":I
    invoke-virtual {v3}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v3

    .line 873
    .end local v3    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v7    # "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabWidth$1":I
    nop

    .line 874
    .local v3, "maxValue$iv":I
    const/4 v7, 0x1

    .local v7, "i$iv":I
    invoke-static {v1}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v8

    if-gt v7, v8, :cond_c

    .line 875
    :goto_9
    invoke-interface {v1, v7}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v9

    .line 876
    .local v9, "e$iv":Ljava/lang/Object;
    move-object v10, v9

    check-cast v10, Landroidx/compose/ui/layout/Placeable;

    .local v10, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v11, 0x0

    .line 713
    .local v11, "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabWidth$1":I
    invoke-virtual {v10}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v10

    .line 876
    .end local v10    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v11    # "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabWidth$1":I
    nop

    .line 877
    .local v10, "v$iv":I
    if-ge v3, v10, :cond_b

    .line 878
    move-object v6, v9

    .line 879
    move v3, v10

    .line 874
    .end local v9    # "e$iv":Ljava/lang/Object;
    .end local v10    # "v$iv":I
    :cond_b
    if-eq v7, v8, :cond_c

    add-int/lit8 v7, v7, 0x1

    goto :goto_9

    .line 882
    .end local v7    # "i$iv":I
    :cond_c
    nop

    .line 713
    .end local v1    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v2    # "$i$f$fastMaxBy":I
    .end local v3    # "maxValue$iv":I
    .end local v6    # "maxElem$iv":Ljava/lang/Object;
    :goto_a
    check-cast v6, Landroidx/compose/ui/layout/Placeable;

    if-eqz v6, :cond_d

    invoke-virtual {v6}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v1

    move v6, v1

    goto :goto_b

    :cond_d
    const/4 v6, 0x0

    .line 714
    .local v6, "fabWidth":I
    :goto_b
    if-eqz v29, :cond_11

    move-object/from16 v1, v29

    .restart local v1    # "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v2, 0x0

    .line 883
    .restart local v2    # "$i$f$fastMaxBy":I
    nop

    .line 884
    invoke-interface {v1}, Ljava/util/List;->isEmpty()Z

    move-result v3

    if-eqz v3, :cond_e

    const/4 v7, 0x0

    goto :goto_d

    .line 885
    :cond_e
    const/4 v3, 0x0

    invoke-interface {v1, v3}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v7

    .line 886
    .local v7, "maxElem$iv":Ljava/lang/Object;
    move-object v3, v7

    check-cast v3, Landroidx/compose/ui/layout/Placeable;

    .local v3, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v8, 0x0

    .line 714
    .local v8, "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabHeight$1":I
    invoke-virtual {v3}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v3

    .line 886
    .end local v3    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v8    # "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabHeight$1":I
    nop

    .line 887
    .local v3, "maxValue$iv":I
    const/4 v8, 0x1

    .local v8, "i$iv":I
    invoke-static {v1}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v9

    if-gt v8, v9, :cond_10

    .line 888
    :goto_c
    invoke-interface {v1, v8}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v10

    .line 889
    .local v10, "e$iv":Ljava/lang/Object;
    move-object v11, v10

    check-cast v11, Landroidx/compose/ui/layout/Placeable;

    .local v11, "it":Landroidx/compose/ui/layout/Placeable;
    const/16 v16, 0x0

    .line 714
    .local v16, "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabHeight$1":I
    invoke-virtual {v11}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v11

    .line 889
    .end local v11    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v16    # "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$fabHeight$1":I
    nop

    .line 890
    .local v11, "v$iv":I
    if-ge v3, v11, :cond_f

    .line 891
    move-object v7, v10

    .line 892
    move v3, v11

    .line 887
    .end local v10    # "e$iv":Ljava/lang/Object;
    .end local v11    # "v$iv":I
    :cond_f
    if-eq v8, v9, :cond_10

    add-int/lit8 v8, v8, 0x1

    goto :goto_c

    .line 895
    .end local v8    # "i$iv":I
    :cond_10
    nop

    .line 714
    .end local v1    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v2    # "$i$f$fastMaxBy":I
    .end local v3    # "maxValue$iv":I
    .end local v7    # "maxElem$iv":Ljava/lang/Object;
    :goto_d
    check-cast v7, Landroidx/compose/ui/layout/Placeable;

    if-eqz v7, :cond_11

    invoke-virtual {v7}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v1

    move v8, v1

    goto :goto_e

    :cond_11
    const/4 v8, 0x0

    .line 716
    .local v8, "fabHeight":I
    :goto_e
    sget-object v1, Landroidx/compose/material/BottomSheetScaffoldLayoutSlot;->Snackbar:Landroidx/compose/material/BottomSheetScaffoldLayoutSlot;

    iget-object v2, v0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$snackbarHost:Lkotlin/jvm/functions/Function2;

    invoke-interface {v14, v1, v2}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->subcompose(Ljava/lang/Object;Lkotlin/jvm/functions/Function2;)Ljava/util/List;

    move-result-object v1

    .line 717
    nop

    .local v1, "$this$fastMap$iv":Ljava/util/List;
    const/4 v2, 0x0

    .line 896
    .local v2, "$i$f$fastMap":I
    nop

    .line 897
    new-instance v3, Ljava/util/ArrayList;

    invoke-interface {v1}, Ljava/util/List;->size()I

    move-result v7

    invoke-direct {v3, v7}, Ljava/util/ArrayList;-><init>(I)V

    .line 898
    .local v3, "target$iv":Ljava/util/ArrayList;
    move-object v7, v1

    .local v7, "$this$fastForEach$iv$iv":Ljava/util/List;
    const/4 v9, 0x0

    .line 899
    .local v9, "$i$f$fastForEach":I
    nop

    .line 900
    const/4 v10, 0x0

    .local v10, "index$iv$iv":I
    invoke-interface {v7}, Ljava/util/List;->size()I

    move-result v11

    :goto_f
    if-ge v10, v11, :cond_12

    .line 901
    invoke-interface {v7, v10}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v16

    .line 902
    .local v16, "item$iv$iv":Ljava/lang/Object;
    move-object/from16 v17, v16

    .local v17, "it$iv":Ljava/lang/Object;
    const/16 v18, 0x0

    .line 903
    .restart local v18    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    move-object/from16 v22, v1

    .end local v1    # "$this$fastMap$iv":Ljava/util/List;
    .local v22, "$this$fastMap$iv":Ljava/util/List;
    move-object v1, v3

    check-cast v1, Ljava/util/Collection;

    move/from16 v23, v2

    .end local v2    # "$i$f$fastMap":I
    .local v23, "$i$f$fastMap":I
    move-object/from16 v2, v17

    check-cast v2, Landroidx/compose/ui/layout/Measurable;

    .local v2, "it":Landroidx/compose/ui/layout/Measurable;
    const/16 v24, 0x0

    .line 717
    .local v24, "$i$a$-fastMap-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$snackbarPlaceables$1":I
    invoke-interface {v2, v4, v5}, Landroidx/compose/ui/layout/Measurable;->measure-BRTryo0(J)Landroidx/compose/ui/layout/Placeable;

    move-result-object v2

    .line 903
    .end local v2    # "it":Landroidx/compose/ui/layout/Measurable;
    .end local v24    # "$i$a$-fastMap-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$snackbarPlaceables$1":I
    invoke-interface {v1, v2}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 904
    nop

    .line 902
    .end local v17    # "it$iv":Ljava/lang/Object;
    .end local v18    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    nop

    .line 900
    .end local v16    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v10, v10, 0x1

    move-object/from16 v1, v22

    move/from16 v2, v23

    goto :goto_f

    .end local v22    # "$this$fastMap$iv":Ljava/util/List;
    .end local v23    # "$i$f$fastMap":I
    .restart local v1    # "$this$fastMap$iv":Ljava/util/List;
    .local v2, "$i$f$fastMap":I
    :cond_12
    move-object/from16 v22, v1

    move/from16 v23, v2

    .line 905
    .end local v1    # "$this$fastMap$iv":Ljava/util/List;
    .end local v2    # "$i$f$fastMap":I
    .end local v10    # "index$iv$iv":I
    .restart local v22    # "$this$fastMap$iv":Ljava/util/List;
    .restart local v23    # "$i$f$fastMap":I
    nop

    .line 906
    .end local v7    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v9    # "$i$f$fastForEach":I
    move-object/from16 v17, v3

    check-cast v17, Ljava/util/List;

    .line 717
    .end local v3    # "target$iv":Ljava/util/ArrayList;
    .end local v22    # "$this$fastMap$iv":Ljava/util/List;
    .end local v23    # "$i$f$fastMap":I
    nop

    .line 716
    nop

    .line 718
    .local v17, "snackbarPlaceables":Ljava/util/List;
    move-object/from16 v1, v17

    .local v1, "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v2, 0x0

    .line 907
    .local v2, "$i$f$fastMaxBy":I
    nop

    .line 908
    invoke-interface {v1}, Ljava/util/List;->isEmpty()Z

    move-result v3

    if-eqz v3, :cond_13

    const/4 v7, 0x0

    goto :goto_11

    .line 909
    :cond_13
    const/4 v3, 0x0

    invoke-interface {v1, v3}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v7

    .line 910
    .local v7, "maxElem$iv":Ljava/lang/Object;
    move-object v3, v7

    check-cast v3, Landroidx/compose/ui/layout/Placeable;

    .local v3, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v9, 0x0

    .line 718
    .local v9, "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$snackbarWidth$1":I
    invoke-virtual {v3}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v3

    .line 910
    .end local v3    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v9    # "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$snackbarWidth$1":I
    nop

    .line 911
    .local v3, "maxValue$iv":I
    const/4 v9, 0x1

    .local v9, "i$iv":I
    invoke-static {v1}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v10

    if-gt v9, v10, :cond_15

    .line 912
    :goto_10
    invoke-interface {v1, v9}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v11

    .line 913
    .local v11, "e$iv":Ljava/lang/Object;
    move-object/from16 v16, v11

    check-cast v16, Landroidx/compose/ui/layout/Placeable;

    .local v16, "it":Landroidx/compose/ui/layout/Placeable;
    const/16 v18, 0x0

    .line 718
    .local v18, "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$snackbarWidth$1":I
    invoke-virtual/range {v16 .. v16}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v16

    .line 913
    .end local v16    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v18    # "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$snackbarWidth$1":I
    move/from16 v18, v16

    .line 914
    .local v18, "v$iv":I
    move-object/from16 v16, v1

    move/from16 v1, v18

    .end local v18    # "v$iv":I
    .local v1, "v$iv":I
    .local v16, "$this$fastMaxBy$iv":Ljava/util/List;
    if-ge v3, v1, :cond_14

    .line 915
    move-object v7, v11

    .line 916
    move v3, v1

    .line 911
    .end local v1    # "v$iv":I
    .end local v11    # "e$iv":Ljava/lang/Object;
    :cond_14
    if-eq v9, v10, :cond_16

    add-int/lit8 v9, v9, 0x1

    move-object/from16 v1, v16

    goto :goto_10

    .end local v16    # "$this$fastMaxBy$iv":Ljava/util/List;
    .local v1, "$this$fastMaxBy$iv":Ljava/util/List;
    :cond_15
    move-object/from16 v16, v1

    .line 919
    .end local v1    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v9    # "i$iv":I
    .restart local v16    # "$this$fastMaxBy$iv":Ljava/util/List;
    :cond_16
    nop

    .line 718
    .end local v2    # "$i$f$fastMaxBy":I
    .end local v3    # "maxValue$iv":I
    .end local v7    # "maxElem$iv":Ljava/lang/Object;
    .end local v16    # "$this$fastMaxBy$iv":Ljava/util/List;
    :goto_11
    check-cast v7, Landroidx/compose/ui/layout/Placeable;

    if-eqz v7, :cond_17

    invoke-virtual {v7}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v1

    move v9, v1

    goto :goto_12

    :cond_17
    const/4 v9, 0x0

    .line 719
    .local v9, "snackbarWidth":I
    :goto_12
    move-object/from16 v1, v17

    .restart local v1    # "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v2, 0x0

    .line 920
    .restart local v2    # "$i$f$fastMaxBy":I
    nop

    .line 921
    invoke-interface {v1}, Ljava/util/List;->isEmpty()Z

    move-result v3

    if-eqz v3, :cond_18

    const/4 v2, 0x0

    goto :goto_14

    .line 922
    :cond_18
    const/4 v3, 0x0

    invoke-interface {v1, v3}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v7

    .line 923
    .restart local v7    # "maxElem$iv":Ljava/lang/Object;
    move-object v10, v7

    check-cast v10, Landroidx/compose/ui/layout/Placeable;

    .local v10, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v11, 0x0

    .line 719
    .local v11, "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$snackbarHeight$1":I
    invoke-virtual {v10}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v10

    .line 923
    .end local v10    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v11    # "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$snackbarHeight$1":I
    nop

    .line 924
    .local v10, "maxValue$iv":I
    const/4 v11, 0x1

    .local v11, "i$iv":I
    invoke-static {v1}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v3

    if-gt v11, v3, :cond_1a

    .line 925
    :goto_13
    invoke-interface {v1, v11}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v16

    .line 926
    .local v16, "e$iv":Ljava/lang/Object;
    move-object/from16 v18, v16

    check-cast v18, Landroidx/compose/ui/layout/Placeable;

    .local v18, "it":Landroidx/compose/ui/layout/Placeable;
    const/16 v22, 0x0

    .line 719
    .local v22, "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$snackbarHeight$1":I
    invoke-virtual/range {v18 .. v18}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v18

    .line 926
    .end local v18    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v22    # "$i$a$-fastMaxBy-BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$snackbarHeight$1":I
    move/from16 v22, v18

    .line 927
    .local v22, "v$iv":I
    move-object/from16 v18, v1

    move/from16 v1, v22

    .end local v22    # "v$iv":I
    .local v1, "v$iv":I
    .local v18, "$this$fastMaxBy$iv":Ljava/util/List;
    if-ge v10, v1, :cond_19

    .line 928
    move-object/from16 v7, v16

    .line 929
    move v10, v1

    .line 924
    .end local v1    # "v$iv":I
    .end local v16    # "e$iv":Ljava/lang/Object;
    :cond_19
    if-eq v11, v3, :cond_1b

    add-int/lit8 v11, v11, 0x1

    move-object/from16 v1, v18

    goto :goto_13

    .end local v18    # "$this$fastMaxBy$iv":Ljava/util/List;
    .local v1, "$this$fastMaxBy$iv":Ljava/util/List;
    :cond_1a
    move-object/from16 v18, v1

    .line 932
    .end local v1    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v11    # "i$iv":I
    .restart local v18    # "$this$fastMaxBy$iv":Ljava/util/List;
    :cond_1b
    move-object v2, v7

    .line 719
    .end local v2    # "$i$f$fastMaxBy":I
    .end local v7    # "maxElem$iv":Ljava/lang/Object;
    .end local v10    # "maxValue$iv":I
    .end local v18    # "$this$fastMaxBy$iv":Ljava/util/List;
    :goto_14
    check-cast v2, Landroidx/compose/ui/layout/Placeable;

    if-eqz v2, :cond_1c

    invoke-virtual {v2}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v1

    move v11, v1

    goto :goto_15

    :cond_1c
    const/4 v11, 0x0

    .line 721
    .local v11, "snackbarHeight":I
    :goto_15
    move-object/from16 v19, v14

    check-cast v19, Landroidx/compose/ui/layout/MeasureScope;

    const/16 v22, 0x0

    new-instance v23, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$1;

    move-object/from16 v1, v23

    iget-object v2, v0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$sheetOffset:Lkotlin/jvm/functions/Function0;

    move-wide/from16 v30, v20

    .end local v20    # "bodyConstraints":J
    .local v30, "bodyConstraints":J
    iget v3, v0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$floatingActionButtonPosition:I

    iget v7, v0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$sheetPeekHeight:F

    iget-object v10, v0, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1;->$sheetState:Landroidx/compose/material/BottomSheetState;

    move-wide/from16 v32, v4

    .end local v4    # "looseConstraints":J
    .local v32, "looseConstraints":J
    move-object/from16 v4, p1

    move/from16 v5, v25

    move/from16 v26, v12

    .end local v12    # "layoutHeight":I
    .local v26, "layoutHeight":I
    move-object/from16 v14, v27

    move-object/from16 v16, v29

    move/from16 v18, v28

    invoke-direct/range {v1 .. v18}, Landroidx/compose/material/BottomSheetScaffoldKt$BottomSheetScaffoldLayout$1$1$1;-><init>(Lkotlin/jvm/functions/Function0;ILandroidx/compose/ui/layout/SubcomposeMeasureScope;IIFIILandroidx/compose/material/BottomSheetState;IILjava/util/List;Ljava/util/List;Ljava/util/List;Ljava/util/List;Ljava/util/List;I)V

    move-object/from16 v1, v23

    check-cast v1, Lkotlin/jvm/functions/Function1;

    const/16 v23, 0x4

    const/16 v24, 0x0

    move-object/from16 v18, v19

    move/from16 v19, v25

    move/from16 v20, v26

    move-object/from16 v21, v22

    move-object/from16 v22, v1

    invoke-static/range {v18 .. v24}, Landroidx/compose/ui/layout/MeasureScope;->layout$default(Landroidx/compose/ui/layout/MeasureScope;IILjava/util/Map;Lkotlin/jvm/functions/Function1;ILjava/lang/Object;)Landroidx/compose/ui/layout/MeasureResult;

    move-result-object v1

    return-object v1
.end method
